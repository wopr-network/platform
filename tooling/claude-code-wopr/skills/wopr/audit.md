...
Audit a WOPR repo with four parallel specialist agents. Produces a consolidated gap report with an option to discuss findings and file GitHub issues.
...
### Agent names
`correctness-auditor`, `completeness-auditor`, `practices-auditor`, `test-auditor`
...
Wait for all four agents. Shutdown all:
```
