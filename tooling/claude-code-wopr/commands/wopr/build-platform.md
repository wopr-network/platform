# WOPR Build Platform (local)

Build `ghcr.io/wopr-network/wopr-platform:local` from source and push to GHCR.
...
```bash
docker push ghcr.io/wopr-network/wopr-platform:local
```

### 5. Report

Print the pushed digest. Watchtower will pick it up within ~60s and restart `wopr-vps-platform-api` in the local DinD stack.
