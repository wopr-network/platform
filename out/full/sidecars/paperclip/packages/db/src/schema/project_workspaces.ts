...
export const projectWorkspaces = pgTable(
  ...
  (table) => ({
    ...
    companySharedKeyIdx: index("project_workspaces_company_shared_key_idx").on(
      table.companyId,
      table.sharedWorkspaceKey,
    ),
    projectRemoteRefIdx: uniqueIndex("project_workspaces_project_remote_ref_idx").on(
      table.projectId,
      table.remoteProvider,
      table.remoteWorkspaceRef,
    ),
  }),
);
