...
async function mockDashboardAPI(
  ...
  await page.route(
    (url) => url.href.includes(PLATFORM_BASE_URL) && url.pathname === "/api/activity",
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    (url) => url.href.includes(PLATFORM_BASE_URL) && url.pathname === "/api/fleet/resources",
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    (url) => url.href.includes(PLATFORM_BASE_URL) && url.pathname === "/api/billing/dividend/stats",
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    (url) => url.href.includes(PLATFORM_BASE_URL) && url.pathname.includes("/fleet/events"),
    async (route) => {
      await route.fulfill({
        status: 200,
...
test.describe("Dashboard: Command Center", () => {
  test("dashboard with active bot shows status, health, and activity", async ({ authedPage: page }) => {
  ...
  test("dashboard with no bots shows empty state", async ({ authedPage: page }) => {
    await mockDashboardAPI(page, {
      bots: [],
      activity: [],
    });
  ...
  test("dashboard with offline bot shows stopped status", async ({ authedPage: page }) => {
    await mockDashboardAPI(page, {
      bots: [
        {
          id: "bot-2",
