...
test.describe("Fleet overview and health", () => {
  test("fleet overview redirects to /fleet/health and lists instances with status indicators", async ({
    ...
    // Verify heading
    await expect(page.getByRole("heading", { name: "Fleet Health" })).toBeVisible({ timeout: 10000 });
  ...
  test("fleet health page shows ALL SYSTEMS NOMINAL when all instances are healthy", async ({ authedPage: page }) => {
    const state = createFleetMockState();
    const now = new Date().toISOString();
    state.bots.push(
      {
    ...
    await expect(page.getByText("ALL SYSTEMS NOMINAL").first()).toBeVisible({ timeout: 5000 });
  ...
  test("fleet health page shows empty state when no instances deployed", async ({ authedPage: page }) => {
    const state = createFleetMockState();
    ...
    await expect(page.getByText("FLEET EMPTY").first()).toBeVisible({ timeout: 5000 });
    await expect(page.getByText("NO PLATFORM UNITS DEPLOYED").first()).toBeVisible();
