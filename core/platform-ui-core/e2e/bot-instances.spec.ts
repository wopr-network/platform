...
test.describe("Bot instance lifecycle", () => {
  ...
  test("instance detail — header, status badge, overview metrics render", async ({ authedPage: page }) => {
    ...
    state.installedPlugins.set("e2e-bot-detail", [{ pluginId: "discord", enabled: true }]);
    await mockFleetAPI(page, state);
    ...
    state.bots.push(
      {
        ...
        uptime: now,
        createdAt: now,
        stats: { cpuPercent: 5, memoryUsageMb: 64, memoryLimitMb: 512, memoryPercent: 12.5 },
      },
      { id: "bot-2", name: "beta-bot", state: "exited", env: {}, uptime: now, createdAt: now, stats: null },
      {
        ...
        uptime: now,
        createdAt: now,
        stats: { cpuPercent: 30, memoryUsageMb: 200, memoryLimitMb: 512, memoryPercent: 39 },
      },
    );
    await mockFleetAPI(page, state);
