...
describe.skipIf(!canRun)("Stripe payment failure handling (real test-mode calls)", () => {
  ...
  async function expectPaymentIntentFailure(cardToken: string): Promise<Stripe.errors.StripeCardError> {
    try {
  ...
  it("rejects insufficient funds (tok_chargeDeclinedInsufficientFunds)", async () => {
    const err = await expectPaymentIntentFailure("tok_chargeDeclinedInsufficientFunds");
  ...
  it("rejects an expired card (tok_chargeDeclinedExpiredCard)", async () => {
    const err = await expectPaymentIntentFailure("tok_chargeDeclinedExpiredCard");
  ...
  it("rejects with processing error (tok_chargeDeclinedProcessingError)", async () => {
    const err = await expectPaymentIntentFailure("tok_chargeDeclinedProcessingError");
  ...
  it("detects CVC check failure (tok_cvcCheckFail)", async () => {
    ...
    const fullCharge = typeof charge === "string" ? await stripe.charges.retrieve(charge) : (charge as Stripe.Charge);

    const cvcCheck = fullCharge.payment_method_details?.card?.checks?.cvc_check;
  ...
  it("error shape includes type, code, and statusCode on all card errors", async () => {
    ...
    expect(typeof err.decline_code === "string" || err.decline_code === undefined).toBe(true);
  }, 30_000);
});
