...
describe("E2E: runtime billing cron — daily bot cost & suspension", () => {
  ...
  it("is idempotent — second run on same date skips already-billed tenants", async () => {
    const tenantId = `tenant-${randomUUID().slice(0, 8)}`;
    const idemBotId = randomUUID();
    await botBilling.registerBot(idemBotId, tenantId, "idem-bot");
    await botInstanceRepo.startBilling(idemBotId);
  ...
  it("charges resource tier surcharge on top of base cost for pro-tier bot", async () => {
    ...
    const totalExpected = dailyBotCost(TODAY).toCents() + proSurcharge;
    const startBalance = Math.ceil(totalExpected) + 50;
    ...
    const balance = await ledger.balance(tenantId);
    // Use closeTo to handle nano-dollar precision rounding
    expect(balance.toCents()).toBeCloseTo(startBalance - totalExpected, 5);
