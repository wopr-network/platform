...
describe("RateStore - Filter Coverage", () => {
  ...
  it("respects limit and offset for sell rates", async () => {
    for (let i = 0; i < 5; i++)
      await store.createSellRate({ capability: `lim-${i}`, displayName: `R${i}`, unit: "tok", priceUsd: i });
  ...
  it("respects limit and offset for provider costs", async () => {
    for (let i = 0; i < 5; i++)
      await store.createProviderCost({ capability: `plim-${i}`, adapter: `a${i}`, unit: "tok", costUsd: i });
...
describe("RateStore - Partial Updates", () => {
  ...
  it("updateProviderCost detects NULL model uniqueness conflict", async () => {
    ...
    const cost2 = await store.createProviderCost({
      capability: "pdup",
      adapter: "a1",
      unit: "tok",
      costUsd: 2,
