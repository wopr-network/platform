#!/bin/bash
# NemoClaw Platform — Full automated provisioning
#
...
#
# Usage:
#   cd nemoclaw-platform && bash vps/provision.sh
#
...
# nemopod.com Cloudflare zone ID — update if different
CF_ZONE="${CF_ZONE_NEMOPOD:?Set CF_ZONE_NEMOPOD (nemopod.com zone ID from Cloudflare dashboard)}"
DROPLET_NAME="nemoclaw-platform"
REGION="sfo2"
SIZE="s-2vcpu-4gb"
IMAGE="ubuntu-24-04-x64"
SSH_KEY_IDS="54912818,52980537"
DOMAINS=("nemopod.com" "api.nemopod.com" "app.nemopod.com")
...
if [ "${DESTROY_FIRST:-}" = "true" ]; then
  ...
  EXISTING=$(do_api GET "/droplets?tag_name=nemoclaw" | jq -r '.droplets[] | select(.name=="'"$DROPLET_NAME"'") | .id')
...
if [ ! -f "$ENV_FILE" ]; then
  echo "ERROR: $ENV_FILE not found. Copy .env.example to vps/.env.production and fill in secrets."
  exit 1
fi
...
heredoc = \"cat > /opt/nemoclaw-platform/.env << 'ENVEOF'\n\" + env.strip() + \"\nENVEOF\"
...
DROPLET_ID=$(do_api POST "/droplets" \
  -d "$(jq -n \
    ...
    '{name:$name,region:$region,size:$size,image:$image,ssh_keys:$ssh_keys,user_data:$user_data,tags:["nemoclaw","platform","production"]}'
  )" | jq -r '.droplet.id')
...
for i in $(seq 1 40); do
  STATUS=$(ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 root@"$IP" \
    'cat /var/log/cloud-init-nemoclaw.log 2>/dev/null | head -1' 2>/dev/null || true)
  if [[ "$STATUS" == NEMOCLAW_PLATFORM_READY* ]]; then
...
echo "$IP" | gh secret set PROD_HOST --repo wopr-network/nemoclaw-platform
echo "$DEPLOY_KEY" | gh secret set PROD_SSH_KEY --repo wopr-network/nemoclaw-platform
echo "$IP" | gh secret set PROD_HOST --repo wopr-network/nemoclaw-platform-ui
echo "$DEPLOY_KEY" | gh secret set PROD_SSH_KEY --repo wopr-network/nemoclaw-platform-ui
echo "    Secrets set on both repos"

# --- Step 7: Verify ---
echo "==> Verifying endpoints..."
sleep 30  # Let Caddy finish TLS provisioning
for domain in "${DOMAINS[@]}"; do
  CODE=$(curl -sf -o /dev/null -w "%{http_code}" "https://$domain" 2>/dev/null || echo "000")
  echo "    https://$domain → HTTP $CODE"
done

HEALTH=$(curl -sf "https://api.nemopod.com/health" 2>/dev/null || echo "unreachable")
echo "    Health: $HEALTH"

# --- Done ---
echo ""
echo "=== NemoClaw Platform Deployed ==="
echo "  Droplet: $DROPLET_ID ($IP)"
echo "  DNS:     nemopod.com, api.nemopod.com, app.nemopod.com"
