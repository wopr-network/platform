...
export function setFleetRouterDeps(d: FleetRouterDeps): void {
  _deps = d;
}

function deps(): FleetRouterDeps {
  if (!_deps) throw new Error("FleetRouter deps not initialized — call setFleetRouterDeps() first");
  return _deps;
}
...
export const fleetRouter = router({
  ...
  getInstance: protectedProcedure.input(z.object({ id: z.string().min(1) })).query(async ({ input, ctx }) => {
    ...
    const { WOPR_PROVISION_SECRET, BETTER_AUTH_SECRET, DATABASE_URL, GATEWAY_KEY, PAPERCLIP_GATEWAY_KEY, ...safeEnv } =
      profile.env;
    return { ...status, env: safeEnv };
  }),

  /** Create a new Managed instance. Requires admin role when orgId is provided. */
  createInstance: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const platformDomain = pc.product.domain ?? process.env.PLATFORM_DOMAIN ?? "nemopod.com";
      ...
      // Build env vars for the managed container.
      // DATA_HOME=/data — FleetManager mounts the volume at /data.
      ...
      const instanceDbName = `instance_${input.name.replace(/-/g, "_")}`;
      ...
      // managed's hostname allowlist must include it, plus the tenant subdomain.
      ...
      const env: Record<string, string> = {
        ...
        ALLOWED_HOSTNAMES: allowedHostnames.join(","),
        ...(gatewayKey ? { GATEWAY_KEY: gatewayKey } : {}),
      ...
      // FleetManager mounts volumeName at /data; DATA_HOME=/data above
      // tells the managed app to use that path for instance state.
      const volumeName = `platform-${input.name}`;
      const createdInstance = await fleet.create({
        tenantId: tenant,
        name: input.name,
        description: input.description ?? `Managed instance: ${input.name}`,
      ...
      // the sandbox user in the platform image) so the non-root container
      // can write to it. Uses the platform image itself (already pulled).
      try {
      ...
      if (healthy) {
        try {
          ...
          const userEmail = dbEmail ?? `${input.name}@nemopod.com`;
          ...
          provisionResult = await provisionContainer(containerUrl, provisionSecret, {
            tenantId: tenant,
          ...
          // Persist managed company ID in profile so member provisioning can resolve it
          if (provisionResult.tenantEntityId) {
            const currentProfile = await store.get(profile.id);
            if (currentProfile) {
              currentProfile.env = { ...currentProfile.env, COMPANY_ID: provisionResult.tenantEntityId };
