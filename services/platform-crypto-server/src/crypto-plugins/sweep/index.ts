...
async function poolReplenish() {
  ...
  const _result = (await res.json()) as { added: number; total: number };

  // Check pool status
  const statusRes = await fetch(`${CRYPTO_SERVICE_URL}/admin/pool/status?chain=${chain}`, { headers });
  if (statusRes.ok) {
    const _status = (await statusRes.json()) as { available: number; claimed: number; total: number };
  }
}
