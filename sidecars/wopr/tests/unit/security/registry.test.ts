...
describe("SecurityRegistry", () => {
  ...
  describe("permissions", () => {
    ...
    it("rejects invalid permission format", () => {
      const reg = getSecurityRegistry();
      expect(() => reg.registerPermission("INVALID", "p")).toThrow(/Invalid permission format/);
      expect(() => reg.registerPermission("", "p")).toThrow(/Invalid permission format/);
    });
    ...
    it("different plugin cannot overwrite existing permission", () => {
      ...
      expect(() => reg.registerPermission("foo.bar", "plugin-b")).toThrow(/is already registered by plugin "plugin-a"/);
    });
  ...
  describe("injection sources", () => {
    ...
    it("different plugin cannot overwrite existing source", () => {
      ...
      expect(() => reg.registerInjectionSource("webhook", "owner", "plugin-b")).toThrow(
  ...
  describe("tool capabilities", () => {
    ...
    it("rejects empty capability", () => {
      const reg = getSecurityRegistry();
      expect(() => reg.registerToolCapability("my_tool", "", "my-plugin")).toThrow(
        /Invalid capability: cannot be empty/,
      );
      expect(() => reg.registerToolCapability("my_tool", "   ", "my-plugin")).toThrow(
        /Invalid capability: cannot be empty/,
      );
    ...
    it("different plugin cannot overwrite existing tool capability", () => {
      ...
      expect(() => reg.registerToolCapability("my_tool", "plugin-b.perm", "plugin-b")).toThrow(
        /is already registered by plugin "plugin-a"/,
      );
    });
  });
