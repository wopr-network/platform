...
describe("capability-catalog hostedDefaults baseUrl", () => {
  ...
  it("uses WOPR_CAPABILITY_CATALOG_URL when set (takes precedence over WOPR_API_BASE_URL)", async () => {
    ...
    const { CAPABILITY_CATALOG } = await import("../../src/core/capability-catalog.js");
    const voice = CAPABILITY_CATALOG.find((c: any) => c.id === "voice");
    expect(voice?.plugins.every((p: any) => p?.hostedConfig?.baseUrl === "http://catalog:8080")).toBe(true);
  });
  ...
  it("falls back to WOPR_API_BASE_URL when WOPR_CAPABILITY_CATALOG_URL is whitespace-only", async () => {
    ...
    const { CAPABILITY_CATALOG } = await import("../../src/core/capability-catalog.js");
    const voice = CAPABILITY_CATALOG.find((c: any) => c.id === "voice");
    expect(voice?.plugins.every((p: any) => p?.hostedConfig?.baseUrl === "http://api-base:1234")).toBe(true);
  });
  ...
  it("uses WOPR_API_BASE_URL when set", async () => {
    ...
    const { CAPABILITY_CATALOG } = await import("../../src/core/capability-catalog.js");
    const voice = CAPABILITY_CATALOG.find((c: any) => c.id === "voice");
    expect(voice?.plugins.every((p: any) => p?.hostedConfig?.baseUrl === "http://localhost:9999")).toBe(true);
  });
  ...
  it("falls back to https://api.wopr.bot when env var absent", async () => {
    ...
    const { CAPABILITY_CATALOG } = await import("../../src/core/capability-catalog.js");
    const voice = CAPABILITY_CATALOG.find((c: any) => c.id === "voice");
    expect(voice?.plugins.every((p: any) => p?.hostedConfig?.baseUrl === "https://api.wopr.bot")).toBe(true);
  });
});
