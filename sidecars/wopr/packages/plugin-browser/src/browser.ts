...
function parseIPv4(host: string): number[] | null {
	...
	if (
		nums.some(
			(n) => Number.isNaN(n) || n < 0 || n > 255 || !Number.isInteger(n),
		)
	)
		return null;
	return nums;
...
function isPrivateIPv6(normalized: string): boolean {
	...
	const v4HexMatch = normalized.match(
...
async function persistProfile(
	...
	try {
		...
		log.warn(
			`[browser] Failed to persist profile "${instance.profileName}": ${err}`,
		);
	}
}
...
export function buildBrowserA2ATools(
	log: PluginLogger,
	headless: boolean,
): A2AServerConfig {
	return {
		...
		tools: [
			// ----- browser_navigate -----
			{
				...
				inputSchema: {
					...
					properties: {
						...
						profile: {
							type: "string",
							description:
								"Browser profile name for session persistence (default: 'default')",
						},
						waitFor: {
							type: "string",
				...
				async handler(args): Promise<A2AToolResult> {
					...
					const waitFor =
						(args.waitFor as "load" | "domcontentloaded" | "networkidle") ??
						"domcontentloaded";
					...
					if (!urlCheck.safe) {
						return {
							content: [
								{
									type: "text",
									text: `Navigation blocked: ${urlCheck.reason}`,
								},
							],
							isError: true,
						};
					...
					try {
						...
						return {
							content: [
								{ type: "text", text: `Navigation failed: ${message}` },
							],
							isError: true,
						};
					}
			...
			// ----- browser_click -----
			{
				...
				inputSchema: {
					...
					properties: {
						...
						timeout: {
							type: "number",
							description: "Timeout in ms to wait for element (default: 5000)",
						},
					},
					required: ["selector"],
				},
				async handler(args): Promise<A2AToolResult> {
					...
					try {
						...
						return {
							content: [
								{ type: "text", text: `Clicked "${selector}" on ${pageUrl}` },
							],
						};
					} catch (err: unknown) {
						const message = err instanceof Error ? err.message : String(err);
					...
					try {
						...
						return {
							content: [
								{
									type: "text",
									text: `Typed into "${selector}"${pressEnter ? " and pressed Enter" : ""}`,
								},
							],
						};
					} catch (err: unknown) {
					...
					try {
						...
						if (selector) {
							...
							if (!element) {
								return {
									content: [
										{ type: "text", text: `Element not found: ${selector}` },
									],
									isError: true,
								};
							}
							...
							buffer = await instance.page.screenshot({
								type: "png",
								fullPage,
							});
						}
				...
				inputSchema: {
					...
					properties: {
						expression: {
							type: "string",
							description:
								"JavaScript expression to evaluate in the browser page",
						},
						profile: {
							type: "string",
							description: "Browser profile name (default: 'default')",
