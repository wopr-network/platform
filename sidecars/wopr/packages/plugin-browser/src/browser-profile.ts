 ...
 */
export async function initBrowserProfileStorage(
...
function getStorage(): StorageApi {
	if (!storage) {
		throw new Error(
			"Browser profile storage not initialized. Call initBrowserProfileStorage() first.",
		);
	}
	return storage;
}
...
export async function loadProfile(name: string): Promise<BrowserProfile> {
	...
	const lsRepo = s.getRepository<BrowserLocalStorageRow>(
		"browser",
		"localStorage",
	);
...
export async function saveProfile(profile: BrowserProfile): Promise<void> {
	...
	await s.transaction(async (txStorage: StorageApi) => {
		const cookieRepo: Repository<BrowserCookieRow> =
			txStorage.getRepository<BrowserCookieRow>("browser", "cookies");
		...
		const lsRepo: Repository<BrowserLocalStorageRow> =
			txStorage.getRepository<BrowserLocalStorageRow>(
				"browser",
				"localStorage",
			);
