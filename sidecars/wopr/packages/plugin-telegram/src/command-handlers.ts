...
export function registerCallbackHandlers(
	...
	botInstance.on("callback_query:data", async (grammyCtx) => {
		...
		if (isFriendRequestCallback(data)) {
			...
			if (
				!config.ownerChatId ||
				String(chatId) !== String(config.ownerChatId)
			) {
			...
			if (!pending) {
				await grammyCtx.answerCallbackQuery({
					text: "Friend request expired or already handled.",
				});
				return;
			}
			...
			if (parsed.action === "accept") {
				try {
					...
					logger.info(
						`[telegram] Friend request from ${pending.requestFrom} accepted via button`,
					);
				} catch (err) {
					logger.error(
						`[telegram] Failed to accept friend request from ${pending.requestFrom}:`,
						err,
					);
					await sendMessage(
						botInstance,
						logger,
						chatId,
						`Failed to accept friend request: ${String(err)}`,
					);
				}
			} else {
				try {
					...
					if (p2pExt?.denyFriendRequest) {
						await p2pExt.denyFriendRequest(
							pending.requestFrom,
							pending.signature,
						);
					}
					...
					logger.info(
						`[telegram] Friend request from ${pending.requestFrom} denied via button`,
					);
				} catch (err) {
					logger.error(
						`[telegram] Failed to deny friend request from ${pending.requestFrom}:`,
						err,
					);
					await sendMessage(
						botInstance,
						logger,
						chatId,
						`Failed to deny friend request: ${String(err)}`,
					);
				}
			}
			return;
