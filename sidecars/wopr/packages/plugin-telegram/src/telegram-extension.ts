	...
	return {
		...
		sendNotification: async (
			...
			if (!config.ownerChatId) {
				getLogger?.()?.warn(
					"No ownerChatId configured — friend request notification not sent",
				);
				return false;
			}

			const storeResult = storePendingFriendRequest(
				requestFrom,
				pubkey,
				encryptPub,
				channelId,
			...
			if (typeof storeResult === "string") {
				getLogger?.()?.warn(
					`Friend request rejected: invalid keys from ${requestFrom}: ${storeResult}`,
				);
				return false;
			}

			try {
				const text = formatFriendRequestMessage(
					requestFrom,
					pubkey,
					channelName,
				);
				const keyboard = buildFriendRequestKeyboard(storeResult.id);
				const sent = await currentBot.api.sendMessage(
					String(config.ownerChatId),
					text,
					{
						parse_mode: "HTML",
				...
				getLogger?.()?.info(
					`Friend request notification sent to owner for request from ${requestFrom}`,
				);
				return true;
			} catch (err) {
				getLogger?.()?.error(
					`Failed to send friend request notification: ${String(err)}`,
				);
				return false;
			}
		},
