...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			placeholder: "r8_...",
			description:
				"Your own API key for the provider (optional — uses hosted credits if empty)",
			secret: true,
			setupFlow: "paste",
		},
	],
...
async function handleVideoCommand(
	...
	if (
		!confirmation ||
		!["yes", "y"].includes(confirmation.trim().toLowerCase())
	) {
	...
	try {
		...
		if (result.error) {
			if (result.error === "insufficient_credits") {
				await cmdCtx.reply(
					"You don't have enough credits for video generation. Add credits to continue.",
				);
			} else {
...
const plugin: WOPRPlugin = {
				...
				tools: [
					{
						...
						inputSchema: {
							...
							properties: {
								...
								model: {
									type: "string",
									description:
										"Video model to use (minimax-video, wan-2.1, kling-1.6, luma-ray2)",
								},
								duration: {
									type: "number",
						...
						async handler(
							...
							try {
								...
								const raw = await pluginCtx.inject(
									"__capability__",
									capabilityRequest,
									{ silent: true },
								);
								...
								return {
									content: [
										{ type: "text", text: result.url ?? "No URL returned" },
									],
								};
							} catch (error: unknown) {
					...
					},
					{
						...
						description:
							"List available video generation models and their capabilities",
						inputSchema: {
							type: "object",
							properties: {},
						},
					...
					},
					{
						...
						async handler(): Promise<A2AToolResult> {
							const currentConfig =
								pluginCtx?.getConfig<VideoGenConfig>() ?? {};
							return {
								content: [
									{
										type: "text",
		...
		const unsubProviderEvent = ctx.events.on("*", async (payload) => {
			...
			for (const provider of providers) {
				if (!registeredProviderIds.includes(provider.id)) {
					...
					registeredProviderIds.push(provider.id);
					pluginCtx.log.info(
						`Registered /video command on late-joining ${provider.id} channel`,
					);
				}
			}
		});
	...
	async shutdown() {
		...
		pluginCtx.unregisterCapabilityProvider(
			"video-generation",
			"videogen-replicate",
		);
