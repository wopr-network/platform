...
const { mockMessageCreate, mockClient, mockWSClient, mockEventDispatcher } = vi.hoisted(() => {
...
vi.mock("@larksuiteoapi/node-sdk", () => {
  ...
  return {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: vitest mock constructor requires function keyword
    CardActionHandler: vi.fn(function () {
      return mockCardHandler;
    } as any),
    adaptDefault: vi.fn(() => (_req: unknown, _res: unknown) => Promise.resolve()),
  };
});
...
describe("sendNotification", () => {
  ...
  it("sends interactive card for friend-request notification", async () => {
    ...
    const actions = content.elements.find((el: { tag: string }) => el.tag === "action");
  ...
  it("silently returns for non-friend-request types", async () => {
    ...
    await provider.sendNotification("oc_test", { type: "unknown-type" }, {});
  ...
  it("silently returns when client is null (no credentials)", async () => {
    ...
    await provider.sendNotification("oc_test", { type: "friend-request", from: "bob" }, { onAccept: vi.fn() });
...
describe("handleCardAction with pending callbacks", () => {
  it("fires onAccept callback when accept button is clicked", async () => {
    vi.resetModules();
    const { handleCardAction, storePendingCallbacks, removePendingCallbacks } = await import("../../src/index.js");
  ...
  it("fires onDeny callback when deny button is clicked", async () => {
    vi.resetModules();
    const { handleCardAction, storePendingCallbacks } = await import("../../src/index.js");
...
describe("cleanupExpiredNotifications", () => {
  it("removes entries older than TTL", async () => {
    vi.resetModules();
    const { storePendingCallbacks, cleanupExpiredNotifications, getPendingCallbacks } = await import(
      "../../src/index.js"
    );
