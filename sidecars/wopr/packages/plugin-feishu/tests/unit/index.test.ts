...
describe("extractTextFromContent", () => {
  ...
  it("extracts text from 'post' message with en_us locale when zh_cn missing", () => {
    const content = JSON.stringify({
      en_us: {
        title: "Title",
        content: [[{ tag: "text", text: "Hello from en_us" }], [{ tag: "text", text: "Second paragraph" }]],
      },
    });
    expect(extractTextFromContent("post", content)).toBe("Hello from en_us Second paragraph");
  });
...
describe("resolveCredentials", () => {
  ...
  it("throws when neither config nor env vars are set", () => {
    expect(() => resolveCredentials({})).toThrow("Feishu appId and appSecret are required");
  });
});
...
describe("plugin lifecycle", () => {
  ...
  it("init() registers config schema, channel provider, and skips bot start when credentials missing", async () => {
    ...
    expect(mockCtx.registerChannelProvider).toHaveBeenCalledWith(expect.objectContaining({ id: "feishu" }));
  });
