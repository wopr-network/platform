...
const state = {
	rateLimitRepo: null as Repository<{
		id: string;
		count: number;
		resetAt: number;
	}> | null,
...
const plugin: WOPRPlugin = {
	...
	async init(pluginCtx: WOPRPluginContext) {
		...
		const logger: Logger = {
			info: (msg) =>
				pluginCtx.log.info(typeof msg === "string" ? msg : JSON.stringify(msg)),
			warn: (msg) =>
				pluginCtx.log.warn(typeof msg === "string" ? msg : JSON.stringify(msg)),
			error: (msg) =>
				pluginCtx.log.error(
		...
		const config = pluginCtx.getMainConfig?.("webhooks") as
			| WebhooksConfig
			| undefined;
		if (!config?.enabled) {
			logger.info(
		...
		try {
			const mainConfig = pluginCtx.getMainConfig?.() as
		...
		try {
			const funnel = pluginCtx.getExtension?.("funnel") as
				| FunnelExtension
				| undefined;
		...
		const extension: WebhooksExtension = {
			...
			async handleWebhook(
				...
				const handlerCtx: WebhookHandlerContext = {
					...
					logMessage: (session, message, options) => {
						pluginCtx.logMessage(session, message, {
							from: "webhook",
							...options,
						});
					},
		...
		const agentResponseHandler = async (woprEvent: {
			type: string;
			payload: unknown;
		}) => {
			...
			const p = (
				woprEvent.payload && typeof woprEvent.payload === "object"
					? woprEvent.payload
					: {}
