...
export function updateEditorContext(
	...
	if (ctx.cursorPosition !== undefined)
		existing.cursorPosition = ctx.cursorPosition;
...
export function formatEditorContext(
	...
	if (cursorPosition) {
		parts.push(
			`Cursor: ${cursorPosition.path}:${cursorPosition.line}:${cursorPosition.column}`,
		);
	}
	...
	if (selection) {
		parts.push(
			`Selected text in ${selection.path} (lines ${selection.startLine}-${selection.endLine}):`,
		);
		parts.push("```");
