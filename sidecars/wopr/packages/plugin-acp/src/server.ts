...
export class AcpServer {
	...
	private async handleLine(line: string): Promise<void> {
		...
		if (msg.jsonrpc !== "2.0") {
			this.send(
				createError(
					msg.id as string | number | undefined,
					RPC_INVALID_REQUEST,
		...
		try {
			switch (method) {
				...
				default:
					this.send(
						createError(id, RPC_METHOD_NOT_FOUND, `Unknown method: ${method}`),
					);
			}
		} catch (err: unknown) {
	...
	private async handleInitialize(
		...
		if (!result.success) {
			this.send(
				createError(id, RPC_INVALID_PARAMS, "Invalid initialize params"),
			);
			return;
		}
	...
	private async handleChatMessage(
		...
		if (!result.success) {
			this.send(
				createError(id, RPC_INVALID_PARAMS, "Invalid chat/message params"),
			);
			return;
		}
		...
		const fullMessage = editorContext
			? `${editorContext}\n\n${params.message}`
			: params.message;
	...
	private async handleChatCancel(
		msg: Record<string, unknown>,
		id: string | number | undefined,
	): Promise<void> {
		if (!this.initialized) {
		...
		if (!result.success) {
			this.send(
				createError(id, RPC_INVALID_PARAMS, "Invalid chat/cancel params"),
			);
			return;
		}
	...
	private async handleContextUpdate(
		...
		if (!result.success) {
			this.send(
				createError(id, RPC_INVALID_PARAMS, "Invalid context/update params"),
			);
			return;
		}
