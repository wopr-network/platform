...
export function initNotification(
	getOwnerNumber: () => string | undefined,
): void {
...
export async function sendFriendRequestNotification(
	...
	try {
		...
		logger.info({
			msg: "Friend request notification sent to owner",
			requestFrom,
			ownerNumber,
		});
		return true;
		...
		logger.error({
			msg: "Failed to send friend request notification",
			error: String(err),
		});
		return false;
	}
...
export async function handleOwnerReply(
	...
	if (!oldest) {
		logger.info({
			msg: "Owner replied ACCEPT/DENY but no pending friend requests found",
		});
	...
	try {
		if (normalised === "ACCEPT") {
			if (p2p?.acceptFriendRequest) {
				...
				const replyMsg =
					...
					`Friend request from ${oldest.requestFrom} accepted.`;
				await sendMessageInternal(toJid(ownerNumber), replyMsg);
				logger.info({
					msg: "Friend request accepted",
					from: oldest.requestFrom,
				});
			} else {
			...
			if (p2p?.denyFriendRequest) {
				...
				await sendMessageInternal(
					toJid(ownerNumber),
					`Friend request from ${oldest.requestFrom} denied.`,
				);
			} else {
				await sendMessageInternal(
					toJid(ownerNumber),
		...
		logger.error({
			msg: "Failed to handle friend request",
			error: String(err),
		});
		await sendMessageInternal(
			toJid(ownerNumber),
			`Error handling friend request from ${oldest.requestFrom}: ${err}`,
		);
	} finally {
