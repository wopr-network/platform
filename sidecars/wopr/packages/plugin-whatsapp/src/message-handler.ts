...
let _getRetryConfig: () => Record<string, unknown> | undefined = () =>
	undefined;
let _handleOwnerReply: (fromJid: string, text: string) => Promise<boolean> =
	async () => false;
...
export async function sendReactionInternal(
	chatJid: string,
	messageId: string,
	emoji: string,
): Promise<void> {
...
export async function handleIncomingMessage(
	...
	if (text && !isGroup) {
		try {
			...
			logger.warn(
				`Owner reply handler failed; continuing normal flow: ${String(err)}`,
			);
		}
	}
