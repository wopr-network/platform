...
export class ObsidianClient {
	...
	async read(path: string): Promise<ObsidianNote> {
		...
		if (!res.ok)
			throw new Error(
				`Failed to read note "${path}": ${res.status} ${res.statusText}`,
			);
	...
	async write(path: string, content: string): Promise<void> {
		...
		if (!res.ok)
			throw new Error(
				`Failed to write note "${path}": ${res.status} ${res.statusText}`,
			);
	}
	...
	async append(path: string, content: string): Promise<void> {
		...
		if (!res.ok)
			throw new Error(
				`Failed to append to note "${path}": ${res.status} ${res.statusText}`,
			);
	}
	...
	async list(folder = ""): Promise<string[]> {
		...
		if (!res.ok)
			throw new Error(
				`Failed to list folder "${folder}": ${res.status} ${res.statusText}`,
			);
