...
const configSchema: ConfigSchema = {
	title: "Obsidian",
	description:
		"Connect to your local Obsidian vault via the Local REST API plugin",
	fields: [
		{
			name: "apiKey",
			type: "password",
			...
			secret: true,
			description:
				"Found in Obsidian → Community Plugins → Local REST API → Show API key",
			setupFlow: "paste",
		},
		{
			name: "port",
			...
			default: 27123,
			description:
				"Port the Obsidian Local REST API listens on (default: 27123)",
			setupFlow: "none",
		},
		{
			name: "vaultPath",
			...
			default: "WOPR",
			description:
				"Folder inside your vault where WOPR stores session archives",
			setupFlow: "none",
		},
		{
			name: "injectContext",
			...
			options: [
				{
					value: "always",
					label: "Always — inject relevant notes before every message",
				},
				{
					value: "on-demand",
					label: "On demand — available via A2A tools only",
				},
		...
		},
		{
			...
			options: [
				{
					value: "true",
					label: "Yes — write session summaries to vault on end",
				},
...
const manifest: PluginManifest = {
	...
	version: "1.0.0",
	description:
		"Obsidian vault integration — search, read, write notes, inject vault context into conversations",
	author: "wopr-network",
	license: "MIT",
	...
	install: [
		{
			kind: "brew",
			formula: "obsidian",
			label: "Install Obsidian (macOS via Homebrew)",
		},
		{
			kind: "script",
			url: "https://obsidian.md/download",
			label: "Download Obsidian (Linux/Windows)",
		},
		{
			kind: "manual",
	...
	commands: [
		{
			...
			async handler(cmdCtx, args) {
				...
				if (sub === "setup") {
					...
					console.info(
						"Step 2: Install the Local REST API plugin inside Obsidian",
					);
					console.info(
					...
					if (!ok) {
						console.error(
							"✗ Could not connect. Make sure Obsidian is running and the API key is correct.",
						);
						process.exit(1);
					}

					await cmdCtx.saveConfig({
						...cmdCtx.getConfig<ObsidianConfig>(),
						apiKey: apiKey.trim(),
						port,
					});
					console.info(
						"✓ Connected and config saved. Run `wopr daemon restart` to activate.",
					);
					process.exit(0);
				}
	...
	async init(context: WOPRPluginContext) {
		...
		healthTimer = setInterval(async () => {
			...
			if (now !== lastConnected) {
				lastConnected = now;
				ctx?.log[now ? "info" : "warn"](
					`Obsidian vault ${now ? "connected" : "disconnected"}`,
				);
			}
		}, 30_000);
				...
				try {
					...
					return {
						...
						metadata: {
							source: "obsidian",
							priority: 30,
							noteCount: top.length,
						},
					};
				} catch (error: unknown) {
					ctx?.log.warn(
						`Obsidian context fetch failed: ${error instanceof Error ? error.message : String(error)}`,
					);
					return null;
				}
			},
		...
		const memUnsub = ctx.events.on(
			"memory:search",
			async (payload: MemorySearchEvent) => {
		...
		const sessionUnsub = ctx.events.on(
			"session:destroy",
			async (payload: SessionDestroyEvent) => {
				const { session, history } = payload;
				const cfg = getCtx().getConfig<ObsidianConfig>();
				if (
					cfg.sessionArchive !== true &&
					String(cfg.sessionArchive) !== "true"
				)
					return;
				if (!client?.isConnected()) return;
