...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		if (ctx.registerA2AServer) {
			cleanups.push(() =>
				(
					ctx as unknown as { unregisterA2AServer?: (name: string) => void }
				)?.unregisterA2AServer?.("imagegen"),
			);
				...
				tools: [
					{
						...
						inputSchema: {
							...
							properties: {
								...
								model: {
									type: "string",
									description:
										"Model to use (flux, sdxl, dall-e). Defaults to plugin config.",
								},
								size: {
									type: "string",
									description:
						...
						async handler(
							...
							if (typeof prompt !== "string" || !prompt) {
								return {
									content: [
										{
											type: "text",
											text: "prompt is required and must be a non-empty string",
										},
									],
									isError: true,
								};
							...
							if (sizeArg !== undefined && !isValidSize(sizeArg)) {
								return {
									content: [
										{
											type: "text",
											text: `Invalid size format: "${sizeArg}". Use WxH format, e.g. 1024x1024`,
										},
									],
									isError: true,
								};
							...
							try {
								...
								if (parsed.imageUrl) {
									return {
										content: [
											...
											{
												type: "text",
												text: `Generated image: ${parsed.imageUrl}`,
											},
										],
									};
								}
								...
								return {
									content: [
										{ type: "text", text: `Image generation failed: ${error}` },
									],
									isError: true,
								};
							}
		...
		if (ctx.events) {
			const unsub = ctx.events.on("plugin:afterInit", () => {
				...
				for (const provider of currentProviders) {
					const p = provider as ChannelProvider;
					if (
						"registerCommand" in p &&
						typeof p.registerCommand === "function"
					) {
						...
						if (!registeredProviderIds.includes(providerId)) {
							...
							ctx.log.info(
								`Late-registered /imagine on channel provider: ${providerId}`,
							);
						}
					}
				}
		...
		if (ctx) {
			...
			for (const provider of providers) {
				try {
					const p = provider as ChannelProvider;
					if (
						"unregisterCommand" in p &&
						typeof p.unregisterCommand === "function"
					) {
