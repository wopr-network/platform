...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			default: "open",
			description:
				"How to handle direct messages: open (anyone), allowlist, or disabled",
		},
		{
			name: "allowFrom",
			type: "array",
			...
			description:
				"Phone numbers or emails allowed to DM (for allowlist policy)",
		},
		{
			name: "groupPolicy",
			type: "select",
		...
		},
		{
			...
			description:
				"Senders allowed to trigger in groups (for allowlist policy)",
		},
		{
			name: "mediaMaxMb",
			type: "number",
		...
		},
		{
			...
			default: true,
			description:
				"Send acknowledgment reaction on incoming messages (requires Private API)",
		},
		{
...
export function isAllowed(senderAddress: string, isGroup: boolean): boolean {
	if (isGroup) {
		...
		return (
			allowed.includes("*") ||
			allowed.some((a) => a.toLowerCase() === senderAddress.toLowerCase())
		);
	} else {
...
export async function handleNewMessage(message: BBMessage): Promise<void> {
		...
		for (const attachment of message.attachments) {
			...
			try {
				...
				if (!data || data.length === 0) {
					logger.error(
						`Attachment ${attachment.guid} download returned empty data`,
					);
					await sendResponse(
						chatGuid,
				...
				logger.error(
					`Failed to download attachment ${attachment.guid}:`,
					error,
				);
				await sendResponse(
					chatGuid,
	...
	const channelInfo: ChannelInfo = {
		...
		id: channelId,
		name: isGroup
			? message.chats[0].displayName || "Group Chat"
			: "BlueBubbles DM",
	};
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext): Promise<void> {
		...
		try {
			...
			if (!alive) {
				logger.error(
					"BlueBubbles server ping failed. Check server URL and password.",
				);
				bbClient = null;
				return;
			}
		...
		try {
			...
			if (privateApiAvailable) {
				logger.info(
					"BlueBubbles Private API is enabled -- reactions and read receipts active",
				);
			} else {
				logger.warn(
					"BlueBubbles Private API not enabled -- reactions and read receipts disabled",
				);
			}
		} catch (error: unknown) {
