	...
	for (let attempt = 0; attempt <= maxRetries; attempt++) {
		try {
			...
			const isNetworkError =
				...
				RETRYABLE_NETWORK_CODES.has(errObj.code);
			const isRetryable =
...
const msteamsChannelProvider: ChannelProvider = {
	...
	async sendNotification(
		...
		const convId = channelId.startsWith("msteams:")
			? channelId.slice(8)
			: channelId;
		const ref = conversationReferences.get(convId);
		if (!ref || !adapter) {
			logger?.info(
				`No conversation reference for ${convId}, cannot send notification`,
			);
			return;
		}
...
async function processActivity(turnContext: TurnContext): Promise<void> {
	...
	if (activity.type === "invoke") {
		...
		if (
			senderId &&
			!isAllowed(senderId, senderConversationType || "personal")
		) {
