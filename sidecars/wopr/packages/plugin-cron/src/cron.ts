 ...
 */
export async function executeCronScript(
	script: CronScript,
	...
	try {
		...
		return {
			...
			exitCode: 0,
			stdout:
				stdout.length > MAX_SCRIPT_OUTPUT
					? `${stdout.substring(0, MAX_SCRIPT_OUTPUT)}\n... (truncated)`
					: stdout,
			stderr:
				stderr.length > MAX_SCRIPT_OUTPUT
					? `${stderr.substring(0, MAX_SCRIPT_OUTPUT)}\n... (truncated)`
					: stderr,
			durationMs,
		};
	} catch (err: unknown) {
 ...
 */
export async function executeCronScripts(
	scripts: CronScript[],
): Promise<CronScriptResult[]> {
 ...
 */
export function resolveScriptTemplates(
	message: string,
	results: CronScriptResult[],
): string {
