...
export function createCronTickLoop(
	...
	return async () => {
		...
		for (const cron of crons) {
			...
			if (cron.runAt) {
				...
				if (currentMinute > lastMinute && shouldRunCron(cron.schedule, now))
					shouldExecute = true;
			}
				...
				try {
					...
					if (cron.scripts && cron.scripts.length > 0) {
						...
						if (!cfg?.cronScriptsEnabled) {
							...
							ctx.log.info(
								`Executing ${cron.scripts.length} script(s) for ${cron.name}`,
							);
							scriptResults = await executeCronScripts(cron.scripts);
							resolvedMessage = resolveScriptTemplates(
								cron.message,
								scriptResults,
							);
							...
							if (failedScripts.length > 0) {
								ctx.log.warn(
									`${failedScripts.length} script(s) failed for ${cron.name}`,
								);
							}
						}
					}
					...
					await Promise.race([
						ctx.inject(cron.session, resolvedMessage, {
							from: "cron",
							silent: true,
							source: CRON_SOURCE,
						}),
						new Promise<never>((_, reject) => {
							timeoutId = setTimeout(
								() =>
									reject(
										new Error(
											`Cron job '${cron.name}' timed out after ${CRON_TIMEOUT_MS / 1000}s`,
										),
									),
								CRON_TIMEOUT_MS,
							);
