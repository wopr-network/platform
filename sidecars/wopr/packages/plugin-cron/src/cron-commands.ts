...
export async function cronCommandHandler(
	...
	switch (subcommand) {
		case "add": {
			...
			if (filtered.length < 4) {
				console.error(
					"Usage: wopr cron add <name> <schedule> <session> <message> [--scripts-file <path>]",
				);
				process.exit(1);
			}
			...
			if (flags.scriptsFile) {
				try {
					...
					console.error(
						`Failed to read scripts file: ${err instanceof Error ? err.message : String(err)}`,
					);
					process.exit(1);
				}
			}
			...
			console.log(
				`Added cron: ${filtered[0]}${scripts ? ` (${scripts.length} script(s))` : ""}`,
			);
			if (flags.now) {
		...
		case "list": {
			...
			if (crons.length === 0) {
				...
				for (const c of crons) {
					if (c.runAt) {
						console.log(
							`  ${c.name}: once @ ${new Date(c.runAt).toLocaleString()}`,
						);
					} else {
						console.log(
							`  ${c.name}: ${c.schedule}${c.once ? " (one-time)" : ""}`,
						);
					}
					console.log(`    -> ${c.session}: "${c.message}"`);
					if (c.scripts && c.scripts.length > 0) {
						console.log(
							`    scripts: ${c.scripts.map((s: CronScript) => s.name).join(", ")}`,
						);
					}
				}
			}
