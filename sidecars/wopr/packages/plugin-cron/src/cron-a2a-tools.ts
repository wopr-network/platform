		...
		tools: [
			{
				...
				inputSchema: {
					...
					properties: {
						...
						message: {
							type: "string",
							description:
								"Message to inject. Use {{script_name}} for script output placeholders.",
						},
						scripts: {
							type: "array",
							items: {
				...
				async handler(args) {
					...
					const scriptInfo = scripts?.length
					...
					return {
						content: [
							{
								type: "text",
								text: `Cron job '${name}' scheduled: ${schedule} -> ${session}${scriptInfo}`,
							},
						],
					};
				},
			},
			{
				...
				inputSchema: {
					...
					properties: {
						time: {
							type: "string",
							description:
								"When to run: '+5m', '+1h', '14:30', or ISO timestamp",
						},
				...
				async handler(args) {
					const { time, session, message } = args as {
						time: string;
						session: string;
						message: string;
					};
			...
			},
			{
				...
				async handler() {
					...
					return {
						content: [
							{ type: "text", text: `Scheduled cron jobs:\n${formatted}` },
						],
					};
				},
			},
			{
				...
				async handler(args) {
					...
					return {
						content: [{ type: "text", text: `Cron job '${name}' cancelled` }],
					};
				},
			},
			{
				...
				inputSchema: {
					...
					properties: {
						...
						session: {
							type: "string",
							description: "Filter by target session",
						},
						limit: {
							type: "number",
						...
						since: {
							type: "number",
							description: "Only show entries after this timestamp (ms)",
						},
						successOnly: {
							type: "boolean",
				...
				async handler(args) {
					...
					if (result.total === 0)
						return {
							content: [
								{
									type: "text",
									text: "No cron history found matching filters.",
								},
							],
						};
					const lines: string[] = [
						`Cron History (showing ${result.entries.length} of ${result.total} entries):`,
						"",
					];
					for (const entry of result.entries) {
						...
						lines.push(
							`  Status: ${entry.status.toUpperCase()} | Duration: ${entry.durationMs}ms`,
						);
