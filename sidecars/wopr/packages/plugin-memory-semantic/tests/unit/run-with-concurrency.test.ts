...
describe("runWithConcurrency", () => {
  it("should continue processing tasks when one rejects", async () => {
    const { results } = await runWithConcurrency(
      [() => Promise.resolve("a"), () => Promise.reject(new Error("fail")), () => Promise.resolve("c")],
      2,
      () => {},
    );
  ...
  it("should call onError for each rejected task", async () => {
    ...
    await runWithConcurrency(
      [() => Promise.resolve("ok"), () => Promise.reject(new Error("boom")), () => Promise.reject(new Error("bang"))],
      3,
      (err) => errors.push(err),
    );
  ...
  it("should handle all tasks rejecting", async () => {
    const errors: unknown[] = [];
    const { results } = await runWithConcurrency(
      [() => Promise.reject(new Error("e1")), () => Promise.reject(new Error("e2"))],
      2,
      (err) => errors.push(err),
    );
  ...
  it("should respect concurrency limit even with failures", async () => {
    ...
    await runWithConcurrency([task("a"), task("b", true), task("c"), task("d"), task("e", true)], 2, () => {});
  ...
  it("should preserve undefined results from successful tasks", async () => {
    const { results, hadErrors } = await runWithConcurrency<string | undefined>(
      [() => Promise.resolve("a"), () => Promise.resolve(undefined), () => Promise.resolve("c")],
      3,
    );
    expect(hadErrors).toBe(false);
