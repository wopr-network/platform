...
describe("registerMemoryTools duck-type warning", () => {
  it("should warn when ctx lacks registerTool", () => {
    ...
    expect(ctx.log.warn).toHaveBeenCalledWith(expect.stringContaining("A2A memory tools will not be registered"));
  });
...
describe("unregisterMemoryTools duck-type warning", () => {
  it("should warn when ctx lacks unregisterTool", () => {
    ...
    expect(ctx.log.warn).toHaveBeenCalledWith(expect.stringContaining("unregisterTool"));
  });
