...
export class EmbeddingQueue {
	...
	enqueue(entries: PendingEntry[], source: string): void {
		...
		this.log.info(
			`[queue] enqueued ${added} entries from ${source} (${this.queue.length} total pending)`,
		);
	...
	private async drain(): Promise<void> {
		if (this.processing || this.queue.length === 0 || !this.searchManager)
			return;
		...
		try {
			while (this.queue.length > 0 && !this.stopped) {
				...
				this.log.info(
					`[queue] processing batch: ${batch.length} entries (${this.queue.length} remaining)`,
				);
				try {
					...
					this.log.error(
						`[queue] batch failed: ${err instanceof Error ? err.message : err}`,
					);
					if (this.stopped) break;
					...
					if (retriable.length > 0) {
						...
						const maxRetry = Math.max(
							...retriable.map((e) => e._retryCount ?? 1),
						);
						const backoffMs = BASE_BACKOFF_MS * 2 ** (maxRetry - 1);
						this.log.info(
							`[queue] re-queued ${retriable.length} entries, backoff ${backoffMs}ms`,
						);
		...
		if (this.drainPromise) {
			await Promise.race([
				this.drainPromise.catch((err) => {
					this.log.error(
						`[queue] drain error during clear: ${err instanceof Error ? err.message : err}`,
					);
				}),
