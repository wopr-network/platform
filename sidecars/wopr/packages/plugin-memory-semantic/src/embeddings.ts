...
export async function createGeminiEmbeddingProvider(
	...
	const embedQuery = async (text: string): Promise<number[]> => {
		...
		if (!res.ok) {
			...
			throw new Error(
				`Gemini embeddings failed: ${res.status} ${sanitizeErrorPayload(payload)}`,
			);
		}
	...
	const embedBatch = async (texts: string[]): Promise<number[][]> => {
		...
		if (!res.ok) {
			const payload = await res.text();
			throw new Error(
				`Gemini batch embeddings failed: ${res.status} ${sanitizeErrorPayload(payload)}`,
			);
		}
...
export async function createOllamaEmbeddingProvider(
	...
	return {
		...
		embedQuery: async (text) => {
			...
			log.info(
				`embedQuery: ${data.embeddings[0].length}-dim in ${ms}ms (~${tokens} tokens)`,
			);
		...
		embedBatch: async (texts) => {
			...
			const totalTokens = texts.reduce(
				(sum, t) => sum + Math.round(t.length / 4),
				0,
			);
			log.info(
				`embedBatch: ${texts.length} texts, ~${totalTokens} tokens, requesting...`,
			);
			...
			if (data.embeddings.length !== texts.length) {
				throw new Error(
					`Ollama embedding count mismatch: expected ${texts.length}, got ${data.embeddings.length}`,
				);
			}
