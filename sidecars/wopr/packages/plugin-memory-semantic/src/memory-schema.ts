 ...
 */
export function createMemoryPluginSchema(
	resolvedInstanceId: string | undefined,
): MigratablePluginSchema {
	return {
		...
		migrate: async (
			...
			if (fromVersion < 2 && toVersion >= 2) {
				await storage
					.raw(`ALTER TABLE memory_chunks ADD COLUMN instance_id TEXT`)
					.catch(() => {
						/* non-fatal: column may already exist */
					});
				await storage
					.raw(
						`CREATE INDEX IF NOT EXISTS idx_memory_chunks_instance_id ON memory_chunks(instance_id)`,
					)
					.catch(() => {
				...
				if (resolvedInstanceId) {
					await storage.raw(
						`UPDATE memory_chunks SET instance_id = ? WHERE instance_id IS NULL`,
						[resolvedInstanceId],
					);
				}
			}
...
export const memoryPluginSchema: MigratablePluginSchema =
	createMemoryPluginSchema(undefined);

/**
 * Migrate data from legacy $WOPR_HOME/memory/index.sqlite to the new Storage API
 */
async function migrateFromLegacyIndexSqlite(
	storage: StorageApi,
): Promise<void> {
