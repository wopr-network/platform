...
function validateAuth(auth: AuthContext): void {
	if (
		...
		throw new Error(
			"Auth token is required. Provide a valid token in auth.token.",
		);
	}
...
function validateInstanceId(id: string): string {
	if (id.length > MAX_INSTANCE_ID_LENGTH) {
		throw new Error(
			`instanceId exceeds maximum length of ${MAX_INSTANCE_ID_LENGTH} characters`,
		);
	}
	return id;
}
	...
	registry.register({
		...
		handler: async (params: Record<string, unknown>, auth: AuthContext) => {
			...
			const authInstanceId =
				typeof auth.instanceId === "string" ? auth.instanceId.trim() : "";
			const resolvedInstanceId =
				authInstanceId.length > 0 ? authInstanceId : instanceId;
