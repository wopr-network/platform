...
const MEMORY_APPEND_SEPARATOR_BYTES = Buffer.byteLength(
	MEMORY_APPEND_SEPARATOR,
	"utf-8",
);
...
function assertWithinBase(baseDir: string, filePath: string): void {
	...
	const baseReal = existsSync(resolvedBase)
	...
	if (existsSync(resolvedTarget)) {
		...
		const parentReal = existsSync(parentDir)
...
export function registerMemoryTools(
	...
	const GLOBAL_IDENTITY_DIR =
		process.env.WOPR_GLOBAL_IDENTITY || "/data/identity";
	...
	const api = ctx as ContextWithTools & {
	...
	api.registerTool({
		...
		inputSchema: {
			...
			properties: {
				file: {
					type: "string",
					description: "Filename to read (e.g., 'SELF.md', '2026-01-24.md')",
				},
				from: {
					type: "number",
		...
		handler: async (
			args: { file?: string; from?: number; lines?: number; days?: number },
			context: any,
		) => {
			try {
				...
				if (!file) {
					const files: string[] = listAllMemoryFiles(
						sessionDir,
						GLOBAL_MEMORY_DIR,
					);
				...
				if (file === "recent" || file === "daily") {
					...
					const collectDailyFromDir = (
						baseDir: string,
						dest: typeof dailyFiles,
					) => {
						if (!existsSync(baseDir)) return;
						for (const f of readdirSync(baseDir).filter((f: string) =>
					...
					if (recent.length === 0)
						return {
							content: [{ type: "text", text: "No daily memory files yet." }],
						};
	...
	api.registerTool({
		...
		inputSchema: {
			...
			properties: {
				file: {
					type: "string",
					description: "Filename (e.g., 'today' for today's log, 'SELF.md')",
				},
				content: { type: "string", description: "Content to write or append" },
				append: {
					type: "boolean",
					description: "If true, append instead of replacing",
				},
			},
			required: ["file", "content"],
		},
		handler: async (
			args: { file: string; content: string; append?: boolean },
			context: any,
		) => {
			try {
				...
				const maxBytes =
					context.config?.maxWriteBytes ?? MEMORY_WRITE_MAX_BYTES;
				...
				if (contentBytes > maxBytes) {
					return {
						content: [
							{
								type: "text",
								text: `Content exceeds maximum size of ${maxBytes} bytes (got ${contentBytes})`,
							},
						],
						isError: true,
					};
				...
				const filePath = ROOT_FILES.includes(filename)
					? join(sessionDir, filename)
					: join(memoryDir, filename);
				assertWithinBase(
					ROOT_FILES.includes(filename) ? sessionDir : memoryDir,
					filePath,
				);
				...
				if (shouldAppend && existsSync(filePath)) {
					...
					const combinedBytes =
						existingBytes + MEMORY_APPEND_SEPARATOR_BYTES + contentBytes;
					...
					writeFileSync(filePath, `${MEMORY_APPEND_SEPARATOR}${content}`, {
						flag: "a",
					});
				} else {
				...
				return {
					content: [
						{
							type: "text",
							text: `${shouldAppend ? "Appended to" : "Wrote"} ${filename}`,
						},
					],
				};
			} catch (err) {
	...
	api.registerTool({
		...
		inputSchema: {
			...
			properties: {
				...
				temporal: {
					type: "string",
					description:
						'Time filter: relative ("24h", "7d") or date range ("2026-01-01", "2026-01-01 to 2026-01-05")',
				},
			},
			required: ["query"],
		},
		handler: async (
			...
			const safeRaw =
				Number.isFinite(rawMax) && rawMax > 0 ? Math.floor(rawMax) : 10;
			const maxResults = Math.min(safeRaw, MAX_SEARCH_RESULTS);
			const parsedTemporal = temporalExpr
				? parseTemporalFilter(temporalExpr)
				: null;
			...
			try {
				...
				if (results.length === 0) {
					const temporalNote = temporalExpr
						? ` within time range "${temporalExpr}"`
						: "";
					return {
						content: [
							{
								type: "text",
								text: `No matches found for "${query}"${temporalNote}`,
							},
						],
					};
				}
				...
				return {
					content: [
						{
							type: "text",
							text: `Found ${results.length} results${temporalNote}:\n\n${formatted}`,
						},
					],
				};
			} catch (err) {
	...
	api.registerTool({
		...
		inputSchema: {
			...
			properties: {
				path: {
					type: "string",
					description: "Relative path from search results",
				},
				from: {
					type: "number",
					description: "Starting line number (1-indexed)",
	...
	api.registerTool({
		...
		inputSchema: {
			...
			properties: {
				...
				section: {
					type: "string",
					description: "Section header (default: today's date)",
				},
			},
		},
		handler: async (
			args: { reflection?: string; tattoo?: string; section?: string },
			context: any,
		) => {
			try {
				...
				if (!reflection && !tattoo) {
					return {
						content: [
							{ type: "text", text: "Provide 'reflection' or 'tattoo'" },
						],
						isError: true,
					};
				}
				...
				if (
					reflection &&
					Buffer.byteLength(reflection, "utf-8") > SELF_REFLECT_MAX_BYTES
				) {
					return {
						content: [
							{
								type: "text",
								text: `reflection exceeds maximum allowed size of ${SELF_REFLECT_MAX_BYTES} bytes`,
							},
						],
						isError: true,
					};
				...
				if (
					tattoo &&
					Buffer.byteLength(tattoo, "utf-8") > SELF_REFLECT_MAX_BYTES
				) {
					return {
						content: [
							{
								type: "text",
								text: `tattoo exceeds maximum allowed size of ${SELF_REFLECT_MAX_BYTES} bytes`,
							},
						],
						isError: true,
					};
				...
				if (
					section &&
					Buffer.byteLength(section, "utf-8") > SELF_REFLECT_SECTION_MAX_BYTES
				) {
					return {
						content: [
							{
				...
				if (tattoo) {
					...
					const tattooSection = lines.findIndex((l: string) =>
						l.includes("## Tattoos"),
					);
					if (tattooSection === -1) {
						const titleLine = lines.findIndex((l: string) =>
							l.startsWith("# "),
						);
						writeFileSync(
							selfPath,
						...
						const insertPoint = afterTattoo.findIndex((l: string) =>
							l.startsWith("## "),
						);
						...
						writeFileSync(
							selfPath,
							[...beforeTattoo, ...afterTattoo].join("\n"),
						);
					}
					return {
						content: [{ type: "text", text: `Tattoo added: "${tattoo}"` }],
					};
				}
				...
				if (reflection) {
					...
					return {
						content: [
							{
								type: "text",
								text: `Reflection added under "${sectionHeader}"`,
							},
						],
					};
				}
...
export function unregisterMemoryTools(ctx: WOPRPluginContext): void {
	if (typeof (ctx as ContextWithTools).unregisterTool !== "function") {
		ctx.log.warn(
			"[memory-semantic] ctx.unregisterTool not available — A2A memory tools cannot be unregistered.",
		);
		return;
	}
	const api = ctx as ContextWithTools & {
