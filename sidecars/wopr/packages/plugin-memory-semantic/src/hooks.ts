 ...
 */
export async function handleBeforeInject(
	state: HookState,
	log: HookLogger,
	payload: any,
): Promise<void> {
	if (
		!state.initialized ||
		!state.searchManager ||
		!state.config.autoRecall.enabled
	) {
		return;
	}
...
export async function handleAfterInject(
	...
	try {
		...
		if (payload.message && payload.message.trim().length > 10) {
			indexText(
				payload.message,
				`rt-${contentHash(`${sessionName}:user:${payload.message}`)}`,
				"realtime-user",
			);
		}
		...
		if (indexedCount > 0) {
			log.info(
				`[semantic-memory] Real-time indexed ${indexedCount} entries from session ${sessionName}`,
			);
		}
		...
		if (state.config.autoCapture.enabled) {
			...
			if (candidates.length > 0) {
				log.info(
					`[semantic-memory] Found ${candidates.length} capture candidates`,
				);
				...
				embeddingQueue.enqueue(
					captureEntries,
					`auto-capture(${candidates.length})`,
				);

				log.info(
					`[semantic-memory] Queued ${candidates.length} capture memories`,
				);
			}
		}
	} catch (err) {
