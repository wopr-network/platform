...
export function registerIdentityTools(ctx: WOPRPluginContext): void {
	if (typeof (ctx as ContextWithTools).registerTool !== "function") {
		ctx.log.warn(
			"[memory-semantic] ctx.registerTool not available — identity tools will not be registered.",
		);
		return;
	}
	...
	if (!sessionApi) {
		ctx.log.warn(
			"[memory-semantic] ctx.session not available — identity tools will not be registered.",
		);
		return;
	}
					...
					content: [
						{
							type: "text",
							text:
								err instanceof PathTraversalError ? err.message : String(err),
						},
					],
					isError: true,
				};
	...
	api.registerTool({
		...
		inputSchema: {
			...
			properties: {
				...
				sectionContent: {
					type: "string",
					description: "Content for custom section",
				},
			},
		},
		handler: async (
			...
			return {
				content: [
					{ type: "text", text: `Identity updated: ${updates.join(", ")}` },
				],
			};
		},
	});
...
export function unregisterIdentityTools(ctx: WOPRPluginContext): void {
	if (typeof (ctx as ContextWithTools).unregisterTool !== "function") {
		ctx.log.warn(
			"[memory-semantic] ctx.unregisterTool not available — identity tools cannot be unregistered.",
		);
		return;
	}
	const api = ctx as ContextWithTools & {
