...
export async function initialize(
		...
		chunking: {
			...
			multiScale: userConfig?.chunking?.multiScale
				? {
						...
						scales:
							userConfig.chunking.multiScale.scales ??
							DEFAULT_CONFIG.chunking.multiScale?.scales ??
							[],
					}
				: DEFAULT_CONFIG.chunking.multiScale,
		},
	...
	state.instanceId =
		state.config.instanceId || process.env.WOPR_INSTANCE_ID || undefined;
	...
	if (!state.instanceId) {
		...
		api.log.info(
			`[semantic-memory] Instance isolation enabled: instanceId=${state.instanceId}`,
		);
	}
    ...
    `);
		...
		const unsubSessionDestroy = api.events.on(
			"session:destroy",
			async (payload: any) => {
		...
		api.log.info(
			`[semantic-memory] Embedding provider: ${state.embeddingProvider.id}`,
		);
			...
			? async (query: string, limit: number, instanceId?: string) => {
					const results = await state.memoryManager?.search(query, {
						...
						excludeLegacyEntries: !!(
							instanceId && state.config.search.excludeLegacyEntries
						),
					});
		...
		api.log.info(
			`[semantic-memory] Initialized with ${loadedVectors} persisted vectors`,
		);

		// Bootstrap: if HNSW is empty but we have chunk metadata, embed them now
		if (loadedVectors === 0 && chunkMetadata.size > 0) {
			api.log.info(
				`[semantic-memory] Bootstrap: ${chunkMetadata.size} chunks need embedding, starting async...`,
			);
			bootstrapEmbeddings(chunkMetadata, state, embeddingQueue, log).catch(
				(err) => {
					api.log.error(
						`[semantic-memory] Bootstrap failed: ${err instanceof Error ? err.message : err}`,
					);
				},
			);
		} else {
			api.log.info(
				`[semantic-memory] Initialized — waiting for memory:filesChanged events from core`,
			);
		}
	} catch (err) {
		api.log.error(
...
export async function bootstrapEmbeddings(
	...
	log.info(
		`Bootstrap: ${entries.length} chunks via ${state.embeddingProvider.id} (heap=${heapMB()}MB)`,
	);
