...
export function ensureEmbeddingColumn(db: any, log: PersistenceLogger): void {
	...
	try {
		...
		log.warn(
			`ensureEmbeddingColumn failed: ${err instanceof Error ? err.message : err}`,
		);
	}
}
...
export async function openDbForRead(
	...
	try {
		...
		log.warn(
			`Cannot open DB directly: ${err instanceof Error ? err.message : err}`,
		);
		return null;
	}
}
...
export async function loadChunkMetadata(
	...
	try {
		const stmt = db.prepare(
			`SELECT id, path, start_line, end_line, source, text, instance_id FROM chunks`,
		);
...
export function getHnswPath(api: PluginContextLike): string | undefined {
	...
	const db = getDb(api);
	if (db) {
		const dbPath: unknown =
			typeof db.location === "function" ? db.location() : db.name;
...
export function persistNewEntryToDb(
	...
	try {
       ...
       ON CONFLICT(id) DO UPDATE SET
		...
		log.warn(
			`persistNewEntryToDb failed for ${id}: ${err instanceof Error ? err.message : err}`,
		);
	}
}
 ...
 */
export function backfillLegacyInstanceId(
	api: PluginContextLike,
	instanceId: string,
	log: PersistenceLogger,
): number {
	...
	try {
		const result = db
			.prepare(`UPDATE chunks SET instance_id = ? WHERE instance_id IS NULL`)
			.run(instanceId);
		const count = typeof result.changes === "number" ? result.changes : 0;
		log.info(
			`Backfilled ${count} legacy entries with instanceId=${instanceId}`,
		);
		return count;
	} catch (err) {
		log.error(
			`backfillLegacyInstanceId failed: ${err instanceof Error ? err.message : err}`,
		);
		return 0;
	}
}
