...
export async function handleMemorySearch(
	...
	if (!state.initialized || !state.searchManager) {
		log.info(
			`[semantic-memory] Not initialized, skipping (initialized=${state.initialized})`,
		);
		return;
	}
	...
	try {
		log.info(
			`[semantic-memory] Starting semantic search (instanceId=${state.instanceId ?? "none"})...`,
		);
		const results = await state.searchManager.search(
			payload.query,
			payload.maxResults,
			state.instanceId,
		);
		...
		log.error(
			`[semantic-memory] Search failed: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
}
