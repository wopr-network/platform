...
export async function syncSessionFiles(params: {
	...
	if (!params.sessionApi) {
		params.log.warn(
			"[sync-sessions] ctx.session not available — skipping session sync",
		);
		return;
	}
	...
	const tasks = sessionNames.map((sessionName) => async () => {
		...
		const entry = await buildSessionEntryFromSql(
			sessionName,
			params.sessionApi!,
			params.log,
		);
	...
	const staleRows = (await params.storage.raw(
	...
	for (const stale of staleRows) {
		...
		if (params.ftsEnabled && params.ftsAvailable) {
			try {
				await params.storage.raw(
					`DELETE FROM ${params.ftsTable} WHERE path = ? AND source = ? AND model = ?`,
					[stale.path, "sessions", params.model],
				);
			} catch (err) {
				params.log.warn(
					`[sync-sessions] FTS delete failed for ${stale.path}: ${err}`,
				);
			}
		}
	}
