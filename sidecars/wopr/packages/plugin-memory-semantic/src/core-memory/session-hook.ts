	...
	return async (sessionName: string, _reason: string) => {
		try {
			if (!params.sessionApi) {
				params.log.warn(
					`[session-hook] ctx.session not available — skipping memory save for ${sessionName}`,
				);
				return;
			}
			...
			const existing = await params.sessionApi.getContext(
				sessionName,
				filename,
			);
			...
			await params.sessionApi.setContext(
				sessionName,
				filename,
				content,
				"session",
			...
			params.log.info(
				`[session-hook] Saved session ${sessionName} to ${today}.md`,
			);
		} catch (err) {
			params.log.warn(
