 ...
 */
export async function discoverSessionMemoryDirs(
	sessionsBase: string,
): Promise<string[]> {
	const dirs: string[] = [];
	try {
...
export class MemoryIndexManager {
	...
	private async searchKeyword(
		...
		const instanceFilter: { sql: string; params: (string | number)[] } =
			instanceId
				? excludeLegacyEntries
					? { sql: ` AND c.instance_id = ?`, params: [instanceId] }
					: {
							sql: ` AND (c.instance_id = ? OR c.instance_id IS NULL)`,
							params: [instanceId],
						}
				: { sql: "", params: [] };
      ...
      `;
		...
		try {
			...
			this.log.warn(
				`FTS search failed: ${err instanceof Error ? err.message : String(err)}`,
			);
			return [];
		}
	}
	...
	private async runSync(_params?: { force?: boolean }): Promise<void> {
		...
		this.log.info(
			`[memory-sync] start (heap: ${heapMB()}MB, fullReindex: ${needsFullReindex})`,
		);
		...
		this.log.info(
			`[memory-sync] global: ${globalChanges.length} changes (heap: ${heapMB()}MB)`,
		);
		...
		this.log.info(
			`[memory-sync] session dirs: ${sessionMemoryDirs.length} (heap: ${heapMB()}MB)`,
		);
		...
		this.log.info(
			`[memory-sync] session: ${sessionChanges.length} changes (heap: ${heapMB()}MB)`,
		);
		...
		if (this.config.sync.indexSessions !== false) {
			this.log.info(
				`[memory-sync] starting transcript streaming (heap: ${heapMB()}MB)`,
			);
			transcriptErrors =
	 ...
	 */
	private async syncSessionTranscriptsStreaming(
					...
					changes: [
						{
							...
							chunks: chunks.map((chunk) => ({
								id: hashText(
									`sessions:${entry.path}:${chunk.startLine}:${chunk.endLine}:${chunk.hash}`,
								),
								text: chunk.text,
								hash: chunk.hash,
	...
	private async scanMemoryFiles(params: {
		...
		for (const dir of params.dirs) {
			...
			for (const entry of fileEntries) {
				...
				const records = (await this.storage.raw(
					`SELECT hash FROM memory_files WHERE path = ? AND source = ?`,
					[entry.path, params.source],
				...
				changes.push({
					...
					chunks: chunks.map((chunk) => ({
						id: hashText(
							`${params.source}:${entry.path}:${chunk.startLine}:${chunk.endLine}:${chunk.hash}`,
						),
						text: chunk.text,
						hash: chunk.hash,
						startLine: chunk.startLine,
		...
		const staleRows = (await this.storage.raw(
			`SELECT path FROM memory_files WHERE source = ?`,
			[params.source],
		)) as Array<{
			path: string;
		}>;
	...
	async handleFilesChanged(event: {
		changes: MemoryFileChange[];
	}): Promise<void> {
		const heapMB = () =>
			Math.round(process.memoryUsage().heapUsed / 1024 / 1024);
		const totalChunks = event.changes.reduce(
			(sum, c) => sum + (c.chunks?.length || 0),
			0,
		);
		this.log.debug(
			...
			for (const change of event.changes) {
				if (change.action === "delete") {
					await this.storage.raw(
						...
						[change.path, change.source],
					);
					await this.storage.raw(
						`DELETE FROM memory_chunks WHERE path = ? AND source = ?`,
						[change.path, change.source],
					);
					if (this.fts.available) {
						try {
							...
							this.log.warn(
								`[memory] FTS delete failed for ${change.path}: ${err}`,
							);
						}
					}
					continue;
				...
				for (const chunk of change.chunks) {
					const chunkId = hashText(
						`${change.source}:${change.path}:${chunk.startLine}:${chunk.endLine}:${chunk.text}`,
					);
					const now = Date.now();
	...
	async status(): Promise<{
		...
		const filesRows = (await this.storage.raw(
			`SELECT COUNT(*) as c FROM memory_files`,
		)) as Array<{ c: number }>;
		const chunksRows = (await this.storage.raw(
