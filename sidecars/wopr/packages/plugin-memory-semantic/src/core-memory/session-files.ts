...
export async function listSessionNames(
	...
	try {
		const rows = (await storage.raw(
			`SELECT name FROM sessions WHERE status = ?`,
			["active"],
		)) as Array<{
			name: string;
		}>;
		...
		log.warn(
			"[session-files] Failed to list active SQL-backed session names",
			error,
		);
		return [];
	}
...
export async function buildSessionEntryFromSql(
	...
	try {
		...
		log.warn(
			`[session-files] Failed to build session entry from SQL for "${sessionName}"`,
			error,
		);
		return null;
	}
...
export async function getRecentSessionContentFromSql(
	...
	try {
		...
		log.warn(
			`[session-files] Failed to get recent session content from SQL for "${sessionName}"`,
			error,
		);
		return null;
	}
