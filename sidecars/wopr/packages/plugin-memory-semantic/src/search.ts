...
export interface SemanticSearchManager {
	...
	addEntriesBatch(
		entries: Array<{ entry: Omit<VectorEntry, "embedding">; text: string }>,
	): Promise<number>;
	close(): Promise<void>;
...
export async function createSemanticSearchManager(
	...
	const saveIndexAsync = (): Promise<void> => {
		const doSave = async (): Promise<void> => {
			...
			try {
				...
				log.info(
					`Saved HNSW index to disk: ${metadata.size} vectors, ${hnswPath}`,
				);
			} catch (err) {
				log.warn(
					`Failed to save HNSW index: ${err instanceof Error ? err.message : err}`,
				);
				try {
		...
		if (initMapPath && existsSync(initMapPath)) {
			try {
				const saved = JSON.parse(
					readFileSync(initMapPath, "utf-8"),
				) as HnswMapFile;
				if (saved.dims) savedDims = saved.dims;
			} catch (err) {
				log.debug(
					`Failed to read saved HNSW map for dims: ${err instanceof Error ? err.message : err}`,
				);
			}
		}
		...
		if (
			...
			try {
				...
				if (!Array.isArray(saved.entries)) {
					throw new Error(
						"old map format (no entries), deleting and rebuilding",
					);
				}
				...
				if (index.size() !== map.entries.length) {
					throw new Error(
						`size mismatch: index=${index.size()}, map=${map.entries.length}`,
					);
				}
	...
	if (!loadedFromDisk) {
		log.info(
			"No persisted HNSW found — starting with empty index. Vectors arrive via events.",
		);
	}
	...
	const scheduleSave = () => {
		...
		saveTimeout = setTimeout(() => {
			saveIndexAsync().catch((err) => {
				log.warn(
					`Scheduled save failed: ${err instanceof Error ? err.message : err}`,
				);
			});
		}, 5000);
	};
	...
	const getEmbedding = async (text: string): Promise<number[]> => {
		...
		if (config.cache.enabled) {
			...
			if (
				...
				embeddingCache.size > config.cache.maxEntries
			) {
	...
	const vectorSearch = async (
		...
		const fetchLimit = Math.min(
			Math.floor(limit * overFetchMultiplier),
			metadata.size,
		);
		...
		for (let i = 0; i < results.keys.length; i++) {
			...
			if (
				...
				config.search.excludeLegacyEntries
			)
				continue;
			if (
				instanceId &&
			...
			scored.push({
				...
				vectorScore: Math.max(
					...
					Math.min(
						1,
						Number.isFinite(results.distances[i])
		...
		log.debug(
			`HNSW search: k=${k}, returned=${scored.length}, instanceId=${instanceId ?? "none"}, took=${elapsed}ms`,
		);
		return scored;
	};
	...
	return {
		async search(
			...
			const keywordResults = await keywordSearchFn(
				query,
				candidateLimit,
				instanceId,
			);
			...
			return merged
				.filter((r) => r.score >= config.search.minScore)
				.slice(0, limit);
		},
		...
		async addEntry(
			...
			if (!embedding || embedding.length !== dims) {
				log.warn(
					`Skipping entry ${entry.id}: expected ${dims}-dim embedding, got ${embedding?.length ?? 0}`,
				);
				return;
			}
			...
			log.debug(
				`Added entry id=${entry.id} as label=${label}, index size=${metadata.size}`,
			);
			scheduleSave();
		},

		async addEntriesBatch(
			entries: Array<{ entry: Omit<VectorEntry, "embedding">; text: string }>,
		): Promise<number> {
			// Filter out already indexed entries AND deduplicate within batch
			const seenInBatch = new Set<string>();
			const newEntries = entries.filter((e) => {
				if (existingIds.has(e.entry.id) || seenInBatch.has(e.entry.id))
					return false;
			...
			const isLocal =
				embeddingProvider.id === "ollama" || embeddingProvider.id === "local";
			...
			while (batchStart < texts.length) {
				...
				while (batchEnd < texts.length) {
					const textTokens = Math.ceil(
						texts[batchEnd].length / CHARS_PER_TOKEN,
					);
					if (
						batchTokens + textTokens > MAX_TOKENS_PER_REQUEST &&
						batchEnd > batchStart
					) {
					...
					try {
						...
						if (
							...
							const waitSecs = waitMatch
								? Math.ceil(parseFloat(waitMatch[1])) + 1
								: 2 ** retries * 5;
							log.warn(
								`Rate limited, waiting ${waitSecs}s before retry ${retries + 1}/${maxRetries}`,
							);
		...
		async close(): Promise<void> {
			...
			log.info(
				`Closed: saved index, cleared cache, final size=${metadata.size}`,
			);
		},
