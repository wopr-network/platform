 ...
 */
export function mapFlatConfigToNested(
	raw: Record<string, unknown>,
): Partial<SemanticMemoryConfig> {
	const config: Partial<SemanticMemoryConfig> = {};

	// Pass-through flat keys that are already flat in SemanticMemoryConfig
	if (raw.provider !== undefined)
		config.provider = raw.provider as SemanticMemoryConfig["provider"];
	if (raw.model !== undefined) config.model = raw.model as string;
	if (raw.baseUrl !== undefined) config.baseUrl = raw.baseUrl as string;
	if (raw.maxWriteBytes !== undefined)
		config.maxWriteBytes = raw.maxWriteBytes as number;
	if (raw.instanceId !== undefined)
		config.instanceId = raw.instanceId as string;

	// Map flat wizard keys → nested sub-objects
	if (raw.autoRecallEnabled !== undefined) {
		config.autoRecall = {
			enabled: raw.autoRecallEnabled as boolean,
		} as SemanticMemoryConfig["autoRecall"];
	}
	if (raw.autoCaptureEnabled !== undefined) {
		config.autoCapture = {
			enabled: raw.autoCaptureEnabled as boolean,
		} as SemanticMemoryConfig["autoCapture"];
	}
	if (raw.searchMaxResults !== undefined) {
		config.search = {
			maxResults: Math.round(raw.searchMaxResults as number),
		} as SemanticMemoryConfig["search"];
	}
	if (raw.searchHybridWeight !== undefined) {
		const vectorWeight = Math.max(
			0,
			Math.min(1, raw.searchHybridWeight as number),
		);
		config.hybrid = {
			vectorWeight,
			textWeight: 1 - vectorWeight,
		} as SemanticMemoryConfig["hybrid"];
	}
...
const plugin: WOPRPlugin & {
	...
	version: "1.0.0",
	description:
		"Semantic memory search with embeddings, auto-recall, and auto-capture",
	...
	async init(api: WOPRPluginContext) {
		...
		for (let i = cleanups.length - 1; i >= 0; i--) {
			try {
				...
				log.warn(
					`[semantic-memory] re-init cleanup[${i}] threw: ${e instanceof Error ? e.message : String(e)}`,
				);
			}
		}
		cleanups.length = 0;
		...
		cleanups.push(() =>
			cleanupCtx.unregisterConfigSchema("wopr-plugin-memory-semantic"),
		);
		...
		cleanups.push(() =>
			cleanupCtx.unregisterContextProvider("memory-semantic"),
		);
		...
		const storedConfig = rawConfig
			? mapFlatConfigToNested(rawConfig)
			: undefined;
		...
		if (!state.initialized) {
			log.error(
				"[semantic-memory] Initialization failed — plugin will not activate",
			);
			return;
		}
		...
		const unsubBeforeInject = ctx.events.on(
			"session:beforeInject",
			(payload: any) => handleBeforeInject(state, log, payload),
		);
		const unsubAfterInject = ctx.events.on(
			"session:afterInject",
			(payload: any) => handleAfterInject(state, log, embeddingQueue, payload),
		);

		// Subscribe to core's file change events for vector indexing
		const unsubFilesChanged = ctx.events.on(
			"memory:filesChanged",
			(payload: any) => handleFilesChanged(state, log, embeddingQueue, payload),
		);

		// Hook into memory:search to provide semantic results
		const unsubSearch = ctx.events.on("memory:search", (payload: any) =>
			handleMemorySearch(state, log, payload),
		);
		...
		log.info(
			"[semantic-memory] Plugin initialized - memory_search enhanced with semantic search",
		);
	},
	...
	async shutdown() {
		...
		for (let i = cleanups.length - 1; i >= 0; i--) {
			try {
				...
				shutdownLog.warn(
					`[semantic-memory] cleanup[${i}] threw: ${e instanceof Error ? e.message : String(e)}`,
				);
			}
		}
		cleanups.length = 0;
