...
const configSchema: ConfigSchema = {
	...
	description:
		"Configure IRC bot integration for channels and private messages",
	fields: [
		{
			name: "server",
			type: "text",
		...
		},
		{
			...
			default: 512,
			description:
				"Maximum IRC line length in bytes (RFC 2812: 512). Protocol overhead is subtracted automatically.",
			setupFlow: "none",
		},
		{
			name: "commandPrefix",
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		if (
			!rawConfig?.server ||
			!rawConfig?.nick ||
			!rawConfig?.channels?.length
		) {
			logger.warn(
				"IRC plugin not configured (missing server, nick, or channels)",
			);
			return;
		}
		...
		const safeMaxLength = Math.max(
			1,
			currentConfig.maxMessageLength - IRC_PROTOCOL_OVERHEAD,
		);
...
function registerEventHandlers(
	ircClient: InstanceType<typeof IrcFramework.Client>,
	config: IrcPluginConfig,
): void {
	...
	ircClient.on("registered", (event: IrcRegisteredEvent) => {
		...
		for (const channel of config.channels) {
			if (!channel.startsWith("#") && !channel.startsWith("&")) {
				logger.warn({
					msg: "Invalid channel name (must start with # or &), skipping",
					channel,
				});
				continue;
			}
	...
	ircClient.on("kick", (event: IrcKickEvent) => {
		if (event.kicked === ircClient.user.nick) {
			logger.warn({
				msg: "Kicked from channel, rejoining",
	...
	ircClient.on("nick in use", () => {
		...
		logger.warn({
			msg: "Nick in use, trying alternative",
			from: currentNick,
			to: newNick,
		});
	...
	ircClient.on("nick", (event: IrcNickEvent) => {
		...
		if (
			event.nick !== ircClient.user.nick &&
			ircClient.user.nick !== config.nick
		) {
...
async function handleIncomingMessage(
	...
	const replyFn = (msg: string) => {
		...
		const safeLimit = Math.max(
			1,
			(currentConfig?.maxMessageLength ?? 512) - IRC_PROTOCOL_OVERHEAD,
		);
	...
	const handledByCommand = await handleRegisteredCommand(
		channel,
		sender,
		message,
	...
	const handledByParser = await handleRegisteredParsers(
		channel,
		sender,
		message,
		replyFn,
