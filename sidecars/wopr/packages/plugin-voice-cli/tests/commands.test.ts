...
describe("voice transcribe", () => {
  it("errors when no file argument given", async () => {
    ...
    expect(ctx.log.error).toHaveBeenCalledWith(expect.stringContaining("Usage"));
  });

  it("errors when file does not exist", async () => {
    const ctx = makeCtx();
    await voiceCmd.handler(ctx, ["transcribe", "/nonexistent/file.wav"]);
    expect(ctx.log.error).toHaveBeenCalledWith(expect.stringContaining("File not found"));
  });
  ...
  it("errors when no STT provider is available", async () => {
    ...
    try {
      ...
      expect(ctx.log.error).toHaveBeenCalledWith(expect.stringContaining("No STT provider"));
    } finally {
      unlinkSync(tmpFile);
    }
  });
...
describe("voice synthesize", () => {
  ...
  it("errors when no TTS provider is available", async () => {
    ...
    expect(ctx.log.error).toHaveBeenCalledWith(expect.stringContaining("No TTS provider"));
  });
});
...
describe("voice list", () => {
  ...
  it("lists voices when TTS available", async () => {
    ...
    const ctx = makeCtx({
      getCapabilityProviders: vi.fn((capability: string) => (capability === "tts" ? [ttsProvider] : [])),
    });
    await voiceCmd.handler(ctx, ["list"]);
  ...
  it("handles provider with malformed voice entries without throwing", async () => {
    ...
    const ctx = makeCtx({
      getCapabilityProviders: vi.fn((capability: string) => (capability === "tts" ? [malformedProvider] : [])),
    });
