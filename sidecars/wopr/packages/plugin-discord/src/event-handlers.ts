...
interface CtxWithSession {
	session?: {
		readConversationLog?: (
			sessionName: string,
		) => Promise<Array<{ channel?: { id: string; type: string } }>>;
	};
}
 ...
 */
export async function findChannelIdFromSession(
	...
	if (!ctxWithSession.session?.readConversationLog) {
		logger.warn({
			msg: "ctx.session.readConversationLog not available (WOP-1538 required)",
			sessionName,
		});
		return null;
	}
	...
	try {
		const entries =
			await ctxWithSession.session.readConversationLog(sessionName);
		...
		logger.debug({
			msg: "No Discord channel found in conversation log",
			sessionName,
		});
		return null;
	} catch (err) {
		logger.error({
			msg: "Error reading conversation log",
			sessionName,
			error: err,
		});
		return null;
...
export async function executeInjectInternal(
	...
	if (cancelToken.cancelled) {
		logger.info({
			msg: "executeInjectInternal - cancelled before start",
			sessionKey,
			streamKey,
		});
	...
	const channel = replyToMessage.channel as
		| TextChannel
		| ThreadChannel
		| DMChannel;
	...
	try {
		...
		await ctx.inject(sessionKey, messageContent, {
			...
			onStream: (msg: StreamMessage) => {
				...
				handleChunk(msg, streamKey).catch((e) =>
					logger.error({ msg: "Chunk error", streamKey, error: String(e) }),
				);
			},
		});
		logger.info({
			msg: "executeInjectInternal - inject complete",
			sessionKey,
			streamKey,
		});
		...
		if (isCancelled) {
			logger.info({
				msg: "executeInjectInternal - inject was cancelled",
				sessionKey,
				streamKey,
			});
			try {
				...
				logger.debug("Stream cleanup error (non-fatal)", {
					error: e,
					streamKey,
				});
			} finally {
			...
			try {
				...
				logger.debug("Reaction cleanup error (non-fatal)", {
					error: e,
					streamKey,
				});
			}
...
export async function handleMessage(
	...
	if (isBot) {
		...
		const botDisplayName =
			message.guild?.members.me?.displayName || client.user?.username || "WOPR";
		messageContent = messageContent
			.replace(new RegExp(`@${botDisplayName}\\s*`, "gi"), "")
			.trim();
	...
	if (isDirectlyMentioned || isDM) {
		...
		if (rateLimiter?.isRateLimited(message.author.id)) {
			...
			message.author
				.send(
					"You've hit the rate limit. Please wait before sending more requests.",
				)
				.catch((e: unknown) =>
					logger.debug("Rate limit DM error (non-fatal)", { error: e }),
				);
			return;
		}
		...
		if (message.attachments.size > 0) {
			...
			const allowedTypes = parsedAllowedTypes?.length
				? parsedAllowedTypes
				: undefined;
		...
		if (!messageContent) {
			...
			logger.info({
				msg: "Human @mention - empty message, using default",
				channelId,
			});
		}
...
export function subscribeSessionEvents(
	...
	const beforeHandler = async (payload: SessionInjectEvent) => {
		...
		try {
			...
			const notificationMsg = await (
				channel as TextChannel | ThreadChannel | DMChannel
			).send(`**[${sourceLabel}]** ${payload.message.slice(0, 1900)}`);
			logger.info({
	...
	const afterHandler = async (payload: SessionResponseEvent) => {
		if (!payload.session.startsWith("discord:")) return;
		if (
			(payload as { channel?: { type?: string } }).channel?.type === "discord"
		)
			return;
...
export function subscribeSessionCreateEvent(
	ctx: WOPRPluginContext,
	client: Client,
): () => void {
	if (!ctx.events) return () => {};
	...
	const handler = async (payload: SessionCreateEvent) => {
		...
		logger.info({
			msg: "Session create for Discord pattern",
			sessionName,
			guildName,
			channelName,
		});
		...
		if (!guild) {
			logger.warn({
				msg: "Guild not found for session",
				sessionName,
				guildName,
			});
			return;
		...
		let woprCategory = guild.channels.cache.find(
			(c) =>
				c.name.toLowerCase() === "wopr" && c.type === ChannelType.GuildCategory,
		);
		...
		if (!woprCategory) {
			try {
				...
				logger.info({
					msg: "Created WOPR category",
					categoryId: woprCategory.id,
				});
			} catch (err) {
				logger.error({
					msg: "Failed to create WOPR category",
					error: String(err),
				});
				return;
			}
		...
		try {
			...
			logger.error({
				msg: "Failed to create Discord channel",
				channelName,
				error: String(err),
			});
		}
