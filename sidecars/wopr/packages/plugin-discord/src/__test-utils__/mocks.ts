...
export function createMockMessage(
	...
	const message: any = {
		...
		mentions: {
			users: new Map(
				mentionedIds.map((uid) => [
					uid,
					{ id: uid, username: "bot", displayName: "bot" },
				]),
			),
			channels: new Map(),
			roles: new Map(),
		...
		reply: vi
			.fn()
			.mockResolvedValue({ id: "reply-1", edit: vi.fn(), delete: vi.fn() }),
...
export function createMockClient(
	overrides: { userId?: string; username?: string } = {},
) {
	const client: any = {
		user: {
...
export function createMockContext(
	...
	const ctx: any = {
		...
		cancelInject: vi
			.fn()
			.mockReturnValue(overrides.cancelInjectResult ?? false),
		events,
		hooks: {
			on: vi.fn(),
			off: vi.fn(),
...
export function createMockInteraction(
	...
	const interaction: any = {
		...
		options: {
			getString: vi.fn(
				(name: string, _required?: boolean) => optionValues[name] ?? null,
			),
			getBoolean: vi.fn(
				(name: string, _required?: boolean) => optionValues[name] ?? null,
			),
