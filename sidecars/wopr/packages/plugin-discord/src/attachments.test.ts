...
describe("saveAttachments", () => {
	...
	it("rejects attachment exceeding maxSizeBytes before download", async () => {
		...
		expect(logger.warn).toHaveBeenCalledWith(
			expect.objectContaining({ msg: "Attachment exceeds size limit" }),
		);
	});
	...
	it("limits number of attachments per message", async () => {
		...
		const msg = makeMessage([
			{
				name: "a.txt",
				url: "https://cdn.discord.com/a.txt",
				size: 100,
				contentType: "text/plain",
			},
			{
				name: "b.txt",
				url: "https://cdn.discord.com/b.txt",
				size: 100,
				contentType: "text/plain",
			},
			{
				name: "c.txt",
				url: "https://cdn.discord.com/c.txt",
				size: 100,
				contentType: "text/plain",
			},
		]);
		...
		expect(logger.warn).toHaveBeenCalledWith(
			expect.objectContaining({ msg: "Attachment limit reached" }),
		);
	});
	...
	it("falls back to default maxSizeBytes when NaN is supplied", async () => {
		...
		const msg = makeMessage([
			{
				name: "f.txt",
				url: "https://cdn.discord.com/f.txt",
				size: 100,
				contentType: "text/plain",
			},
		]);
		...
		await expect(
			saveAttachments(msg, { maxSizeBytes: NaN }),
	...
	it("falls back to default maxSizeBytes when Infinity is supplied", async () => {
		...
		await expect(
			saveAttachments(msg, { maxSizeBytes: Infinity }),
		).resolves.not.toThrow();
	});
	...
	it("falls back to default maxPerMessage when NaN is supplied", async () => {
		...
		await expect(
			saveAttachments(msg, { maxPerMessage: NaN }),
		).resolves.not.toThrow();
	});
	...
	it("rejects attachment with disallowed content type", async () => {
		...
		expect(result).toEqual([]);
		expect(logger.warn).toHaveBeenCalledWith(
			expect.objectContaining({ msg: "Attachment content type not allowed" }),
		);
	});
	...
	it("aborts body stream when timeout fires during download", async () => {
		...
		vi.mocked(mockPipeline).mockImplementationOnce((...args) => {
			const opts = args.find(
				(a) => a && typeof a === "object" && "signal" in a,
