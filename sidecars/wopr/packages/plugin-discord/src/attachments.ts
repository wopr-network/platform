...
export class AttachmentSizeLimitError extends Error {
	...
	constructor(bytesReceived: number, maxSize: number) {
		super(
			`Attachment stream exceeded size limit: ${bytesReceived} > ${maxSize}`,
		);
...
export async function saveAttachments(
	...
	const maxSize =
		Number.isFinite(rawMaxSize) && rawMaxSize > 0
			? rawMaxSize
	...
	for (const [, attachment] of message.attachments) {
		...
		try {
			...
			const baseContentType = (attachment.contentType ?? "")
				.split(";")[0]
				.trim()
				.toLowerCase();
			...
			const safeName =
				attachment.name?.replace(/[^a-zA-Z0-9._-]/g, "_") || "attachment";
...
async function downloadAttachment(
	...
	try {
		...
		await pipeline(nodeStream, sizeGuard, fileStream, {
