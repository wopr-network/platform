...
async function withRateLimitRetry<T>(
	...
	for (let attempt = 0; attempt <= RATE_LIMIT_MAX_RETRIES; attempt++) {
		try {
			...
			const isRateLimit =
				typedErr?.httpStatus === 429 || typedErr?.status === 429;
...
export class DiscordMessageUnit {
	...
	constructor(
		...
		logger.debug({
			msg: "DiscordMessageUnit created",
			unitId: this.unitId,
			isReply,
		});
	}
	...
	append(text: string): void {
		if (this.state.status === "finalized") {
			logger.debug({
				msg: "Unit.append ignored - finalized",
				unitId: this.unitId,
				textLen: text.length,
			});
			return;
		...
		if (this.state.status === "sending") {
			this.state = {
				...this.state,
				pendingWhileSending: this.state.pendingWhileSending + text,
			};
			logger.debug({
	...
	async flush(): Promise<"ok" | "split" | "skip"> {
		...
		if (this.state.status === "sent") {
			if (content.length === this.state.lastEditLength) {
				logger.debug({
					msg: "Unit.flush skip - no new content",
					unitId: this.unitId,
				});
				return "skip";
			}
	...
	private async sendInitial(content: string): Promise<"ok" | "split" | "skip"> {
		...
		logger.debug({
			msg: "Unit.sendInitial",
			unitId: this.unitId,
			contentLen: content.length,
			isReply: this.isReply,
		});

		const payload: SendPayload = this.useComponentsV2
			? textToComponentsV2(content)
			: content;
		const promise = withRateLimitRetry(
			() =>
				(this.isReply
					? this.replyTo.reply(payload)
					: this.channel.send(payload)) as Promise<Message>,
		...
		try {
			...
			const buffered =
				this.state.status === "sending" ? this.state.pendingWhileSending : "";
			const mergedContent = content + buffered;
			this.state = {
				status: "sent",
				content: mergedContent,
				discordMsg,
				lastEditLength: content.length,
			};
			if (buffered) {
				logger.debug({
	...
	private async editExisting(
		content: string,
	): Promise<"ok" | "split" | "skip"> {
		if (this.state.status !== "sent") return "skip";

		logger.debug({
			msg: "Unit.editExisting",
			unitId: this.unitId,
			contentLen: content.length,
		});
		const discordMsg = this.state.discordMsg;
		const payload: EditPayload = this.useComponentsV2
			? textToComponentsV2Edit(content)
			: content;
		await withRateLimitRetry(
			() => discordMsg.edit(payload),
			`editExisting:${this.unitId}`,
		);
	...
	private async handleOverflow(
		...
		if (this.state.status === "buffering") {
			const sendPayload: SendPayload = this.useComponentsV2
				? textToComponentsV2(toSend)
				: toSend;
			const promise = withRateLimitRetry(
				() =>
					(this.isReply
						? this.replyTo.reply(sendPayload)
						: this.channel.send(sendPayload)) as Promise<Message>,
			...
			try {
				...
				logger.debug({
					msg: "Unit.handleOverflow sent and finalized",
					unitId: this.unitId,
				});
			} catch (error) {
				const buffered =
				...
				this.state = { status: "buffering", content: content + buffered };
				logger.error({
					msg: "Unit.handleOverflow failed",
					unitId: this.unitId,
					error: String(error),
				});
				throw error;
			...
			const editPayload: EditPayload = this.useComponentsV2
				? textToComponentsV2Edit(toSend)
				: toSend;
			await withRateLimitRetry(
				() => sentMsg.edit(editPayload),
				`handleOverflow:edit:${this.unitId}`,
			);
			this.state = { status: "finalized" };
			logger.debug({
				msg: "Unit.handleOverflow edited and finalized",
				unitId: this.unitId,
			});
		}
	...
	async finalize(): Promise<void> {
		logger.debug({
			...
			status: this.state.status,
			contentLen:
				this.state.status !== "finalized" ? this.state.content.length : 0,
		});

		if (this.state.status === "finalized") {
			logger.debug({
				msg: "Unit.finalize skip - already finalized",
				unitId: this.unitId,
			});
			return;
		}
		...
		if (this.state.status === "sending") {
			logger.debug({
				msg: "Unit.finalize waiting for send",
				unitId: this.unitId,
			});
			try {
				...
				if (stateAfterSend.status === "sending") {
					...
					logger.debug({
						msg: "Unit.finalize already finalized during send",
						unitId: this.unitId,
					});
					return;
				}
				...
				logger.debug({
					msg: "Unit.finalize send completed",
					unitId: this.unitId,
					msgId: discordMsg.id,
				});
			} catch (error) {
				logger.error({
					msg: "Unit.finalize send failed",
					unitId: this.unitId,
					error: String(error),
				});
				this.state = { status: "finalized" };
		...
		if (!content) {
			logger.debug({
				msg: "Unit.finalize skip - empty content",
				unitId: this.unitId,
			});
			this.state = { status: "finalized" };
			return;
		...
		try {
			if (prevState.status === "sent") {
				logger.debug({
					msg: "Unit.finalize editing sent message",
					unitId: this.unitId,
					contentLen: content.length,
				});
				...
				const editPayload: EditPayload = this.useComponentsV2
					? textToComponentsV2Edit(finalContent)
					: finalContent;
				await withRateLimitRetry(
					() => sentMsg.edit(editPayload),
					`finalize:edit:${this.unitId}`,
				);
				logger.debug({
					msg: "Unit.finalize edit success",
					unitId: this.unitId,
				});
				...
				const finalContent = content.slice(0, DISCORD_LIMIT);
				const sendPayload: SendPayload = this.useComponentsV2
					? textToComponentsV2(finalContent)
					: finalContent;
				...
				logger.debug({
					msg: "Unit.finalize send success",
					unitId: this.unitId,
					msgId: msg.id,
				});
			}
...
export class DiscordMessageStream {
	...
	append(text: string): void {
		if (this.finalized) {
			logger.debug({
				msg: "Stream.append ignored - finalized",
				streamId: this.streamId,
				textLen: text.length,
			});
			return;
	...
	private async processPending(): Promise<void> {
		...
		try {
			const batch = this.pendingContent
				.splice(0, this.pendingContent.length)
				.join("");
			if (!batch) return;
			...
			if (
				timeSinceLast > IDLE_SPLIT_MS &&
				this.currentUnit.content.length > 0
			) {
				logger.info({
				...
				this.currentUnit = new DiscordMessageUnit(
					this.channel,
					this.replyTo,
					false,
					{
			...
			logger.debug({
				msg: "Stream.processPending complete",
				streamId: this.streamId,
	...
	async finalize(): Promise<void> {
		...
		if (this.finalized) {
			logger.debug({
				msg: "Stream.finalize skip - already finalized",
				streamId: this.streamId,
			});
			return;
		}
		...
		if (this.flushTimer) {
			...
			logger.debug({
				msg: "Stream.finalize stopped flush interval",
				streamId: this.streamId,
			});
		}
		...
		if (this.processing) {
			logger.info({
				msg: "Stream.finalize waiting for processing to complete",
				streamId: this.streamId,
			});
			let waitCount = 0;
			...
			if (this.processing) {
				logger.warn({
					msg: "Stream.finalize timed out waiting for processing",
					streamId: this.streamId,
				});
			} else {
				logger.debug({
					msg: "Stream.finalize processing completed",
					streamId: this.streamId,
					waitCount,
				});
			}
...
export async function handleChunk(
	...
	if (!stream) {
		logger.warn({
			msg: "handleChunk - no stream found",
			streamKey,
			msgType: msg.type,
		});
		return;
	...
	if (msg.type === "system" && msg.subtype === "compact_boundary") {
		const metadata = msg.metadata as
			| { pre_tokens?: number; trigger?: string }
			| undefined;
		logger.info({
			msg: "handleChunk - auto-compaction detected",
			streamKey,
			metadata,
		});
	...
	if (msg.type === "text" && msg.content) {
		...
		logger.debug({
			msg: "handleChunk - assistant content",
			streamKey,
			contentLen: textContent.length,
		});
	} else {
		logger.debug({
			msg: "handleChunk - skipping non-text",
			streamKey,
			msgType: msg.type,
		});
	}
