...
describe("sessionNameSchema", () => {
	...
	it("rejects empty string", () => {
		...
		if (!r.success)
			expect(r.error.issues[0].message).toBe("Session name cannot be empty");
	});
...
describe("modelInputSchema", () => {
	...
	it("accepts model with slashes, dots, colons", () => {
		expect(
			modelInputSchema.safeParse("anthropic/claude-3.5:latest").success,
		).toBe(true);
	});
...
describe("woprMessageSchema", () => {
	it("accepts normal message", () => {
		expect(woprMessageSchema.safeParse("Hello, how are you?").success).toBe(
			true,
		);
	});

	it("accepts message with special characters and unicode", () => {
		expect(woprMessageSchema.safeParse("Hello! @#$ \u{1F600}").success).toBe(
			true,
		);
	});
...
describe("autocompleteFocusedSchema", () => {
	...
	it("rejects string over 128 characters", () => {
		expect(autocompleteFocusedSchema.safeParse("a".repeat(129)).success).toBe(
			false,
		);
	});
});
...
describe("validateInput", () => {
	...
	it("works with enum schemas", () => {
		expect(validateInput(thinkLevelSchema, "high")).toEqual({
			success: true,
			data: "high",
		});
