...
function mockTextChannel(
	...
	return {
		...
		guild: {
			id: overrides.guildId ?? "guild-001",
...
function mockDMChannel(
	...
	return {
		recipient:
			id === null
				? null
				: { id: id ?? "user-001", username: username ?? "someuser" },
		isDMBased: () => true,
		isThread: () => false,
	} as any;
}
	...
	return {
		...
		parentId:
			overrides.parentId === null
				? null
				: (overrides.parentId ?? "channel-001"),
		parent:
			overrides.parentName === null
				? null
				: { name: overrides.parentName ?? "general" },
		isDMBased: () => false,
		isThread: () => true,
	} as any;
}
...
describe("discord-utils", () => {
	describe("getSessionKey", () => {
		describe("guild TextChannel", () => {
			...
			it("produces different keys for same-named channels in different guilds", () => {
				const ch1 = mockTextChannel({
					id: "ch-100",
					guildId: "g-aaa",
					name: "general",
					guildName: "My Server",
				});
				const ch2 = mockTextChannel({
					id: "ch-100",
					guildId: "g-bbb",
					name: "general",
					guildName: "My Server",
	...
	describe("getSessionKeyFromInteraction", () => {
		...
		it("uses getSessionKey when channel is instanceof TextChannel", () => {
			...
			expect(getSessionKeyFromInteraction(interaction)).toBe(
				"discord:g-100:#ch-789",
			);
		});
		...
		it("uses getSessionKey when channel is instanceof ThreadChannel", () => {
			...
			Object.defineProperties(channel, {
				...
				guild: {
					value: { id: "g-100", name: "Test Guild" },
					writable: true,
					configurable: true,
				},
		...
		it("uses getSessionKey when channel is instanceof DMChannel", () => {
			...
			Object.defineProperties(channel, {
				recipient: {
					value: { id: "user-alice", username: "alice" },
					writable: true,
					configurable: true,
				},
			...
			const message = {
				...
				guild: {
					members: {
						cache: {
							get: vi.fn((id: string) =>
								id === "user-1" ? { displayName: "Alice (Nickname)" } : null,
							),
						},
					},
				},
			...
			const message = {
				...
				mentions: {
					users: new Map([
						[
							"user-1",
							{ id: "user-1", username: "alice", displayName: undefined },
						],
					]),
					channels: new Map(),
					roles: new Map(),
