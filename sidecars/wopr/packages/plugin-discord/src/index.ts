...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			description:
				"Discord user ID that owns this bot (used for /claim command)",
		},
		{
			name: "emojiQueued",
			type: "text",
		...
		},
		{
			...
			default: false,
			description:
				"Enable Discord Components v2 message layout (Containers, Sections). Cannot mix with legacy embeds.",
		},
		{
			...
			description:
				"Maximum size in bytes for a single Discord attachment download (default 10MB)",
		},
		{
			...
			description:
				"Maximum number of attachments to download per message (default 5)",
		},
		{
		...
		},
		{
			...
			default: 10,
			description:
				"Maximum number of AI requests a single user can make within the rate limit window (default 10)",
		},
		{
			...
			default: 60000,
			description:
				"Sliding window duration in milliseconds for per-user rate limiting (default 60000 = 60s)",
		},
		{
			...
			default: "",
			description:
				"Discord user IDs allowed to invoke message-based /commands. Empty = deny all.",
		},
		{
...
const plugin: WOPRPlugin = {
	...
	manifest: {
		...
		provides: {
			capabilities: [
				{ type: "channel", id: "discord", displayName: "Discord" },
			],
		},
			...
			async handler(_context: WOPRPluginContext, args: string[]) {
				...
				if (subcommand === "claim") {
					...
					if (!code) {
						...
						// biome-ignore lint/suspicious/noConsole: CLI output
						console.log(
							"  Claim ownership of the Discord bot using a pairing code",
						);
						process.exit(1);
					}
					...
					try {
						...
						if (
							...
							console.error(
								`Invalid daemon auth token (contains newline characters): ${tokenPath}`,
							);
							process.exit(1);
						}
						...
						const response = await fetch(
							"http://localhost:7437/plugins/discord/claim",
							{
								method: "POST",
								headers,
						...
						if (!response.ok) {
							...
							let errorMsg: string;
							try {
								const errJson = rawBody
									? (JSON.parse(rawBody) as { error?: string })
									: {};
							...
							if (response.status === 401 || response.status === 403) {
								console.error(
									`Authentication failed (${response.status}): ${errorMsg}`,
								);
								if (!authToken) {
									console.error(
										`  Daemon auth token not found at: ${tokenPath}`,
									);
									console.error(
										`  Ensure WOPR daemon has written a token to that path.`,
									);
								}
							} else {
						...
						try {
							...
							result = {
								success: false,
								error: rawBody || "Invalid response from daemon",
							};
						}
					...
					// biome-ignore lint/suspicious/noConsole: CLI output
					console.log(
						"  wopr discord claim <code>  - Claim ownership using a pairing code",
					);
	...
	async init(context: WOPRPluginContext) {
		...
		logger.info({
			msg: "Checking ctx.events availability",
		...
		client.on(Events.InteractionCreate, async (interaction) => {
			if (interaction.isAutocomplete()) {
				await slashHandler
					...
					.catch((e) =>
						logger.error({ msg: "Autocomplete error", error: String(e) }),
					);
				return;
			}
		...
		client.on(Events.TypingStart, (typing) => {
			if (client && queueManager)
				handleTypingStart(typing, client, queueManager);
		});
