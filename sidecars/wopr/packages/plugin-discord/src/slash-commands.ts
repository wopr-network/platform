...
export const commands = [
	...
	new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName("name").setDescription("Session name").setRequired(true),
		),
	new SlashCommandBuilder()
		.setName("wopr")
...
function getAllModels(ctx: WOPRPluginContext): ResolvedModel[] {
	...
	for (const pid of providerIds) {
		const provider = (
			ctx as {
				getProvider?: (id: string) => { supportedModels?: string[] } | null;
			}
		)?.getProvider?.(pid);
...
export class SlashCommandHandler {
	...
	async handleAutocomplete(
		...
		if (interaction.commandName === "model") {
			...
			const focusedResult = validateInput(
				autocompleteFocusedSchema,
				rawFocused,
			);
			let focused = "";
	...
	async handle(interaction: ChatInputCommandInteraction): Promise<void> {
		...
		switch (commandName) {
			...
			case "verbose": {
				...
				await interaction.reply({
					content: enabled
						? "\u{1f50a} **Verbose mode enabled**"
						: "\u{1f507} **Verbose mode disabled**",
					ephemeral: true,
				});
				break;
			}
			...
			case "claim": {
				...
				const result = await this.claimOwnership(
					codeResult.data,
					interaction.user.id,
					interaction.user.id,
				);
			...
			default: {
				...
				if (registeredCmd) {
						...
						if (option.value !== undefined) {
							...
							if (mentionMatch && client) {
								try {
									...
									logger.warn({
										msg: "Failed to resolve mention to username",
										value,
										error: String(err),
									});
					...
					try {
						...
						logger.error({
							msg: "Channel command handler error",
							command: commandName,
							error: String(err),
						});
						if (!replied) {
	...
	private async handleWoprMessage(
		...
		try {
			...
			logger.error({
				msg: "Slash command inject failed",
				error: String(error),
			});
