...
export function setCommandAuthConfigGetter(
	getter: () => CommandAuthConfig,
): void {
	commandAuthConfigGetter = getter;
}

function getCommandAuthConfig(): CommandAuthConfig {
	return commandAuthConfigGetter
		? commandAuthConfigGetter()
		: commandAuthConfig;
}
...
export async function handleRegisteredCommand(
	...
	if (authConfig.allowedRoleIds.length > 0 && message.member) {
		...
		roleAllowed = authConfig.allowedRoleIds.some((roleId) =>
	...
	const args = rawArgs
		.map((a) => sanitize(a).slice(0, MAX_ARG_LENGTH))
		.filter((a) => a.length > 0);
	...
	try {
		...
		logger.error({
			msg: "Channel command error",
			cmd: cmdName,
			error: String(error),
		});
		try {
			await message.reply(
				`Error executing /${cmdName}. Please try again later.`,
			);
		} catch (replyErr) {
