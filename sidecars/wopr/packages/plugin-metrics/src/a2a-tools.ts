		...
		tools: [
			{
				...
				inputSchema: {
					...
					properties: {
						name: {
							type: "string",
							description: "Metric name (e.g., messages_processed)",
						},
				...
				handler: async (
					...
					if (typeof args.name !== "string" || args.name === "") {
						return {
							content: [
								{
									type: "text",
									text: "Error: name must be a non-empty string",
								},
							],
							isError: true,
						};
					...
					if (typeof args.value !== "number") {
						return {
							content: [
								{ type: "text", text: "Error: value must be a number" },
							],
							isError: true,
						};
					}
					...
					return {
						content: [
							{ type: "text", text: `Recorded metric: ${name}=${value}` },
						],
					};
				},
			},
			{
				...
				inputSchema: {
					...
					properties: {
						...
						since: {
							type: "number",
							description: "Timestamp (ms) to filter from",
						},
				...
				inputSchema: {
					...
					properties: {
						instance_id: {
							type: "string",
							description:
								"Instance ID for per-instance summary. Omit for platform summary.",
						},
					},
				},
				handler: async (
					...
					const instanceId = args.instance_id as string | undefined;
					const summary = instanceId
						? await store.getInstanceSummary(instanceId)
						: await store.getPlatformSummary();
					return {
