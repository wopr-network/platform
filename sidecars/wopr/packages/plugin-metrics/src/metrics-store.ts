...
export class MetricsStore {
	...
	async getSum(
		...
		let sql =
			"SELECT COALESCE(SUM(metric_value), 0) as total FROM metrics_rows WHERE metric_name = ?";
	...
	async getInstanceSummary(
		instanceId: string,
	): Promise<InstanceMetricsSummary> {
		const [
			messages_processed,
			tokens_consumed,
			active_sessions,
	...
	async query(options: {
		...
		let sql =
			"SELECT id, timestamp, metric_name, metric_value, instance_id, tags FROM metrics_rows WHERE 1=1";
