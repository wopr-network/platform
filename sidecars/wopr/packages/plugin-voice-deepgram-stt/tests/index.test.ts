...
function makeMockCtx(config: Record<string, unknown> = { apiKey: "test-key-123" }) {
  const ctx = {
...
describe("wopr-plugin-voice-deepgram-stt", () => {
  ...
  describe("init", () => {
    it("registers STT provider", async () => {
      ...
      expect(mockCtx.registerProvider).toHaveBeenCalledWith(expect.any(Object));
    });
    ...
    it("throws when API key is missing", async () => {
      ...
      await expect(plugin.init?.(mockCtx)).rejects.toThrow("DEEPGRAM_API_KEY is required");
    });
  });
  ...
  describe("transcribe (batch)", () => {
    it("calls Deepgram API and returns transcript", async () => {
      ...
      const result = await registeredProvider.transcribe(Buffer.from("fake-audio"));
    ...
    it("throws on HTTP error", async () => {
      ...
      await expect(registeredProvider.transcribe(Buffer.from("fake-audio"))).rejects.toThrow(
        "Deepgram transcription failed (HTTP 401)",
      );
    });
    ...
    it("throws when transcript is empty", async () => {
      ...
      await expect(registeredProvider.transcribe(Buffer.from("fake-audio"))).rejects.toThrow(
        "Deepgram response missing transcript",
      );
    });
  });
