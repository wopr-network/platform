...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			secret: true,
			description:
				"Bearer token for app-only endpoints (search, stream). Optional if using OAuth 1.0a.",
		},
		{
			name: "streamKeywords",
			type: "text",
...
const plugin: WOPRPlugin = {
	...
	manifest: {
		...
		provides: {
			capabilities: [
				{ type: "channel", id: "twitter", displayName: "Twitter/X" },
			],
		},
	...
	async init(context: WOPRPluginContext) {
		...
		if (ctx.registerSetupContextProvider) {
			ctx.registerSetupContextProvider(({ partialConfig }) => {
				...
				if (!hasApiKey) {
					...
					instructions +=
						'2. Create a new Project and App (choose "Free" or "Basic" tier)\n';
					instructions +=
						'3. Under "Keys and Tokens", generate your API Key and API Key Secret\n';
				...
				if (!hasAccessToken) {
					...
					instructions +=
						"4. Paste the Access Token and Access Token Secret here\n\n";
				} else {
					instructions +=
				...
				instructions +=
					"For streaming and search endpoints, a Bearer Token is recommended.\n";
				instructions += 'Find it under "Keys and Tokens" > "Bearer Token".\n\n';

				instructions += "### Step 4: (Optional) Stream Keywords\n";
				instructions +=
					"Enter comma-separated keywords to monitor in real-time via Twitter's filtered stream.\n";
		...
		try {
			...
			logger.info({
				msg: "Twitter client authenticated",
		...
		if (config.streamKeywords) {
			...
			await streamManager.connect((tweet) => {
				logger.info({
					msg: "Stream tweet received",
					id: tweet.data.id,
					text: tweet.data.text?.slice(0, 50),
				});
