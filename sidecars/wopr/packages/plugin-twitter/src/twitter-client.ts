...
export class TwitterClient {
	...
	async tweet(
		text: string,
		options?: { replyToId?: string; quoteId?: string; mediaIds?: string[] },
	): Promise<string> {
	...
	async search(query: string, maxResults = 10): Promise<TweetV2[]> {
		return this.withRetry("search", async () => {
			const result = await this.api.v2.search(query, {
	...
	async getTimeline(maxResults = 20): Promise<TweetV2[]> {
		return this.withRetry("timeline", async () => {
			const me = await this.api.v2.me();
			const timeline = await this.api.v2.userTimeline(me.data.id, {
				max_results: maxResults,
			});
	...
	private async withRetry<T>(
		...
		for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
			...
			try {
				...
				if (status === 429) {
					...
					const rateLimit = rateLimitErr?.rateLimit as
				...
				if (attempt < MAX_RETRIES - 1) {
					...
					logger.warn({
						msg: "Twitter API error, retrying",
						endpoint,
						attempt,
						error: String(err),
					});
				...
				logger.error({
					msg: "Twitter API error, exhausted retries",
					endpoint,
					error: String(err),
				});
				throw err;
		...
		throw new Error(
			`withRetry: exhausted ${MAX_RETRIES} retries for ${endpoint}`,
		);
	}
}
