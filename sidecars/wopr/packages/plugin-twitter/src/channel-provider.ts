...
export function setTwitterProviderClient(
	c: TwitterClient | null,
	username?: string,
	ownerId?: string,
): void {
	twitterClient = c;
...
const activeNotificationTimeouts: Set<ReturnType<typeof setTimeout>> =
	new Set();
...
export async function sendNotification(
	...
	if (!ownerUserId)
		throw new Error(
			"sendNotification: ownerUserId not set — cannot send notification",
		);
	...
	logger.info({
		msg: "Notification parser registered",
		parserId,
		from: payload.from,
	});
...
export function clearNotificationParsers(): void {
	...
	const toDelete = [...registeredParsers.keys()].filter((id) =>
		id.startsWith("notif-friend-"),
	);
