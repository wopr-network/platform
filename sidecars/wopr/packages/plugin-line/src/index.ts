...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			placeholder: "U1234567890abcdef...",
			description:
				"User IDs allowed to trigger in groups (for allowlist policy)",
		},
	],
};
...
export function getPendingNotification(
	notifId: string,
): ChannelNotificationCallbacks | undefined {
...
export function buildFriendRequestFlexMessage(
	fromName: string,
	notifId: string,
...
export async function sendReply(
	...
	const messages: messagingApi.TextMessage[] = chunkMessage(text).map(
		(chunk) => ({
			type: "text",
			text: chunk,
		}),
...
async function handlePostbackEvent(
	...
	const notifId = isAccept
		? data.slice("notif_accept:".length)
		: data.slice("notif_deny:".length);
	...
	const userId =
		source?.type === "user" ? (source as webhook.UserSource).userId : undefined; // Group/room sources don't reliably carry userId for reply context
	...
	if (!entry) {
		ctx?.log.warn(
			`Notification ${notifId} not found (expired or already handled)`,
		);
	...
	if (entry.expectedUserId && userId && userId !== entry.expectedUserId) {
		ctx?.log.warn(
			`Notification ${notifId} postback from unexpected user ${userId} (expected ${entry.expectedUserId})`,
		);
		return;
	}
...
const lineChannelProvider: ChannelProvider = {
	...
	async send(channelId: string, content: string): Promise<void> {
		...
		const messages: messagingApi.TextMessage[] = chunkMessage(content).map(
			(chunk) => ({
				type: "text",
				text: chunk,
			}),
	...
	async sendNotification(
		...
		const flexMessage = buildFriendRequestFlexMessage(
			payload.from ?? "",
			notifId,
		);
		...
		ctx?.log.info(
			`Notification sent for friend request from ${payload.from ?? "unknown"} (notifId=${notifId})`,
		);
	},
};
...
async function startWebhookServer(): Promise<void> {
	...
	app.post(
		...
		(req: express.Request, res: express.Response) => {
			...
			const events: webhook.Event[] =
				(req.body as { events: webhook.Event[] }).events ?? [];
	...
	server = app.listen(port, () => {
		ctx?.log.info(
			`LINE webhook server listening on port ${port} at ${webhookPath}`,
		);
	});
}
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		try {
			...
			ctx.log.error(
				"Failed to start LINE webhook server",
				err instanceof Error ? err.message : String(err),
			);
		}
	},
