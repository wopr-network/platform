...
export async function createMatrixClient(
	...
	if (!config.storageDir) {
		throw new Error(
			"Matrix plugin requires storageDir to be set — E2EE keys must not be stored in /tmp",
		);
	}
	...
	if (!accessToken) {
		if (!config.userId || !config.password) {
			throw new Error(
				"Matrix plugin requires either accessToken or userId+password",
			);
		}
	...
	const storageProvider = new SimpleFsStorageProvider(
		path.join(storageDir, "matrix-bot-storage.json"),
	);
	...
	if (config.enableEncryption !== false) {
		cryptoProvider = new RustSdkCryptoStorageProvider(
			path.join(storageDir, "crypto"),
			1 as any,
		);
		logger.info({
			msg: "E2EE crypto storage initialized",
			path: path.join(storageDir, "crypto"),
		});
	}
