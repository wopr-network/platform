		...
		await ctx.inject(sessionKey, messageContent, {
			...
			onStream: (msg: StreamMessage) => {
				...
				if (msg.type === "text" && msg.content) {
					...
					const content = (msg as unknown as { message: { content: unknown } })
						.message.content;
...
export async function handleRoomMessage(
	...
	const authorDisplayName = await getUserDisplayName(
		client,
		event.sender,
		roomId,
	);
	...
	if (isMentioned && !isDM) {
		messageContent = messageContent
			...
			.replace(
				new RegExp(botDisplayName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi"),
				"",
			)
			.trim();
	}
...
export function subscribeSessionEvents(
	...
	const unsubscribe = ctx.events.on(
		"session:afterInject",
		async (payload: SessionResponseEvent) => {
			if (!payload.session.startsWith("matrix:")) return;
			if (
				(payload as unknown as { channel?: { type: string } }).channel?.type ===
				"matrix"
			)
				return;
