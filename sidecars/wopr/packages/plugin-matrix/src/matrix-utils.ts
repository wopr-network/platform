...
export async function getSessionKey(
	...
	try {
		...
		logger.warn({
			msg: "Failed to resolve session key, using room ID",
			roomId,
			error: String(err),
		});
...
export async function getRoomName(
	...
	try {
		...
		if (state?.alias)
			return (state.alias as string).split(":")[0]?.replace("#", "") || roomId;
	} catch {
		// No alias either
	}
...
export async function getUserDisplayName(
	...
	try {
		if (roomId) {
			const memberEvent = await client.getRoomStateEvent(
				roomId,
				"m.room.member",
				userId,
			);
 ...
 */
export async function isDMRoom(
	client: MatrixClient,
	roomId: string,
): Promise<boolean> {
	try {
