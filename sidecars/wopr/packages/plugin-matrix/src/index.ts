...
const plugin: WOPRPlugin = {
	...
	manifest: {
		...
		version: "1.0.0",
		description:
			"Matrix channel plugin for WOPR - Enables AI conversations in Matrix rooms with E2EE support",
		author: "WOPR",
		license: "MIT",
	...
	async init(context: WOPRPluginContext) {
		...
		if (!config.accessToken && !(config.userId && config.password)) {
			logger.warn(
				"Matrix plugin not configured: need accessToken or userId+password",
			);
			return;
		}
		...
		try {
			...
			logger.error({
				msg: "Failed to create Matrix client",
				error: String(error),
			});
			throw error;
		}
		...
		client.on("room.message", (roomId: string, event: unknown) => {
			...
			handleRoomMessage(
				roomId,
				event as MatrixRoomEvent,
				client,
				ctx,
				queueManager,
			).catch((error: unknown) =>
				logger.error({
					msg: "Room message handling failed",
					error: String(error),
				}),
			);
		});
		...
		client.on("room.event", async (_roomId: string, event: unknown) => {
			...
			const botUserId = await client.getUserId();
			handleReactionEvent(evt as MatrixReactionEvent, botUserId).catch(
				(error: unknown) =>
					logger.error({
						msg: "Reaction handling failed",
						error: String(error),
					}),
			);
		});
		...
		client.on(
			...
			(roomId: string, _event: unknown, error: Error) => {
				logger.error({
		...
		if (!config.accessToken && client.accessToken) {
			try {
				...
				logger.warn({
					msg: "Failed to save access token to config",
					error: String(error),
				});
			}
		}
