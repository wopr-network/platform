...
describe("PiperTTSProvider", () => {
  ...
  describe("validateConfig", () => {
    ...
    it("accepts speed boundary values", () => {
      expect(() => new PiperTTSProvider({ speed: 0.5 }).validateConfig()).not.toThrow();
      expect(() => new PiperTTSProvider({ speed: 2.0 }).validateConfig()).not.toThrow();
    });

    it("accepts all valid sample rates", () => {
      for (const rate of [16000, 22050, 24000, 48000]) {
        expect(() => new PiperTTSProvider({ sampleRate: rate }).validateConfig()).not.toThrow();
      }
    });
  });
  ...
  describe("metadata", () => {
    ...
    it("requires piper docker image", () => {
      expect(provider.metadata.requires?.docker).toContain("rhasspy/piper:latest");
    });
