		...
		try {
			...
			const errorObj =
				typeof error === "object" && error !== null
					...
					: null;
			const status = (errorObj?.status ?? errorObj?.statusCode) as
				| number
				| undefined;
			const isRetryable =
...
const BASE_CONFIG_FIELDS: ConfigSchema["fields"] = [
	...
	},
	{
		...
		required: false,
		description:
			"Enable native speech-to-speech via OpenAI Realtime API (gpt-realtime)",
		default: false,
	},
	{
		name: "realtimeVoice",
...
const plugin: WOPRPlugin = {
	...
	async init(ctx: WOPRPluginContext) {
		...
		if (ctx.registerExtension) {
			const realtimeEnabled = !!ctx.getConfig?.<{ enableRealtime?: boolean }>()
				?.enableRealtime;
			if (realtimeEnabled) {
			...
			ctx.registerExtension("provider-openai", {
				...
				createRealtimeClient: (
					...
					if (!realtimeEnabled) {
						throw new Error(
							"Realtime voice is disabled. Enable `enableRealtime` in plugin config.",
						);
					}
