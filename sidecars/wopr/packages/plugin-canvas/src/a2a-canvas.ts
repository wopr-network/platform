		...
		tools: [
			{
				...
				inputSchema: {
					...
					properties: {
						...
						content: {
							type: "string",
							description:
								"The content body (HTML string, Markdown text, chart JSON, or form schema)",
						},
						title: {
							type: "string",
				...
				async handler(args) {
					...
					const item = await canvasPush(sessionName, type, content, {
						title,
						id,
					});
					return {
						...
						content: [
							{
								type: "text",
								text: removed
									? `Canvas item ${id} removed`
									: `Canvas item ${id} not found`,
							},
						],
					};
				},
			...
			},
			{
				...
				description:
					"Take a snapshot of the current canvas state and return all items.",
				inputSchema: {
					type: "object",
					properties: {},
				},
			...
			},
			{
				name: "canvas_get",
				description:
					"Get current canvas items for the session without emitting events.",
				inputSchema: {
					type: "object",
					properties: {},
				},
