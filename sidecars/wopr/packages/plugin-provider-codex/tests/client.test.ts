...
describe("CodexClient.query", () => {
  ...
  it("yields tool_use for command_execution events", async () => {
    ...
    const toolUse = results.find((e: any) => e.type === "assistant" && e.message?.content?.[0]?.type === "tool_use");
  ...
  it("yields system reasoning event for reasoning items", async () => {
    ...
    const reasoningEvent = results.find((e: any) => e.type === "system" && e.subtype === "reasoning");
  ...
  it("yields error result on turn.failed", async () => {
    ...
    const errorEvent = results.find((e: any) => e.type === "result" && e.subtype === "error");
  ...
  it("resumes thread when opts.resume is provided", async () => {
    ...
    expect(mockCodexInstance.resumeThread).toHaveBeenCalledWith("old-thread-id");
  });
...
describe("CodexClient.listModels", () => {
  ...
  it("handles flat array response", async () => {
    mockCodexInstance.listModels.mockResolvedValue([{ model: "model-a" }, { name: "model-b" }]);
