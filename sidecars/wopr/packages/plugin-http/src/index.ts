	...
	fields: [
		{
			...
			placeholder:
				"api.example.com, cdn.example.com (comma-separated, empty = all)",
			description:
				"Comma-separated list of allowed domains for http_fetch. Empty means all domains allowed.",
			setupFlow: "paste",
		},
		{
			name: "blockedDomains",
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		if (!context.registerA2AServer) {
			context.log.error(
				"registerA2AServer not available - cannot register tools",
			);
			return;
		}
