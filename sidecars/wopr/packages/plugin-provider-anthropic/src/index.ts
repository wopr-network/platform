...
class AnthropicClient implements ModelClient {
	...
	async *queryV2(
		...
		try {
			...
			await retryWithBackoff(
				() => active.session.send(opts.prompt),
	...
	async *query(opts: ModelQueryOptions): AsyncGenerator<unknown> {
		...
		throw new Error(
			`Anthropic query failed: ${lastError instanceof Error ? lastError.message : String(lastError)}`,
		);
	}
...
const plugin: WOPRPlugin & {
	...
	async onDrain() {
		logger.info(
			"[provider-anthropic] Draining — waiting for active sessions to complete",
		);
