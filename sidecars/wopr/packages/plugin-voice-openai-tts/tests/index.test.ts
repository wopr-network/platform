...
describe("plugin init", () => {
  ...
  it("logs error when API key is missing", async () => {
    ...
    expect(ctx.log.error).toHaveBeenCalledWith(expect.stringContaining("OPENAI_API_KEY required"));
  ...
  it("logs error on invalid voice", async () => {
    ...
    await plugin.init?.(ctx as never);
    expect(ctx.log.error).toHaveBeenCalledWith(expect.stringContaining("Invalid voice"));
  });
});
...
describe("plugin shutdown", () => {
  ...
  it("calls unregisterCapabilityProvider on shutdown", async () => {
    ...
    await plugin.init?.(ctx as never);
    await plugin.shutdown?.();
    expect(ctx.unregisterCapabilityProvider).toHaveBeenCalledWith("tts", "openai-tts");
  });
});
...
describe("synthesize", () => {
  ...
  it("throws on non-OK response", async () => {
    ...
    try {
      const registeredProvider = ctx.registerTTSProvider.mock.calls[0]?.[0];
      await expect(registeredProvider.synthesize("Hello")).rejects.toThrow("OpenAI TTS failed: 401");
    } finally {
      globalThis.fetch = origFetch;
    }
  });
