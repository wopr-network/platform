...
export function buildSoulA2ATools(
		...
		tools: [
			{
				...
				async handler() {
					...
					try {
						...
						const sessionContent = await sessionApi.getContext(
							sessionName,
							"SOUL.md",
						);
						if (sessionContent) {
						...
						const globalContent = await sessionApi.getContext(
							"__global__",
							"SOUL.md",
						);
						if (globalContent) {
			...
			},
			{
				...
				async handler(args: unknown) {
					...
					if (!sessionApi) {
						return {
							content: [
								{
									type: "text",
									text: "Provide 'content' or 'section'+'sectionContent'",
								},
							],
						};
					}
					...
					if (content !== undefined) {
						...
						return {
							content: [{ type: "text", text: "SOUL.md replaced entirely" }],
						};
					}

					if (section !== undefined && sectionContent !== undefined) {
						try {
							let existing = await sessionApi.getContext(
								sessionName,
								"SOUL.md",
							);
							if (!existing) {
						...
						return {
							content: [
								{ type: "text", text: `SOUL.md section "${section}" updated` },
							],
						};
					}
