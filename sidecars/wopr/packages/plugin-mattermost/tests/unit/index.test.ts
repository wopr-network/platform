...
describe("plugin.init", () => {
  ...
  it("calls registerChannelProvider on init", async () => {
    ...
    await plugin.init!(ctx as any);
    expect(ctx.registerChannelProvider).toHaveBeenCalledWith(expect.objectContaining({ id: "mattermost" }));
  });
  ...
  it("connects and gets bot user ID when token provided", async () => {
    ...
    await plugin.init!(ctx as any);
    expect(global.fetch).toHaveBeenCalledWith("https://mm.example.com/api/v4/users/me", expect.anything());
  });
  ...
  it("loads config from channels.mattermost nested path", async () => {
    ...
    await plugin.init!(ctx as any);
    expect(global.fetch).toHaveBeenCalledWith("https://nested.mm.example.com/api/v4/users/me", expect.anything());
  });
});
...
describe("sendNotification", () => {
  ...
  it("posts a friend-request notification message to the channel", async () => {
    ...
    const onDeny = vi.fn();
    await channelProv.sendNotification("ch1", { type: "friend-request", from: "alice" }, { onAccept, onDeny });
  ...
  it("works without callbacks (no-op on accept/deny)", async () => {
    ...
    await expect(channelProv.sendNotification("ch1", { type: "friend-request", from: "bob" })).resolves.not.toThrow();
  });
});
