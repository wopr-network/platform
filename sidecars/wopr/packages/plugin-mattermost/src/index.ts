...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			description:
				"Personal Access Token from Mattermost. Preferred auth method.",
			secret: true,
			setupFlow: "paste",
		},
		{
...
async function resolveClient(): Promise<MattermostClient> {
	if (!config.serverUrl) {
		throw new Error(
			"Mattermost serverUrl is required. Set channels.mattermost.serverUrl",
		);
	}
	...
	const envToken =
		process.env.MATTERMOST_TOKEN || process.env.MATTERMOST_ACCESS_TOKEN;
	if (envToken) {
		return new MattermostClient({
			serverUrl: config.serverUrl,
			token: envToken,
...
async function handlePostedEvent(event: MattermostWsEvent): Promise<void> {
	...
	const botMentioned = botUsername
		? post.message.includes(`@${botUsername}`)
		: false;
	...
	if (trimmedMsg === `${prefix}accept` || trimmedMsg === `${prefix}deny`) {
		...
		if (entry) {
			try {
				...
				logger?.error(
					`Error in notification ${isAccept ? "accept" : "deny"} callback:`,
					String(error),
				);
			} finally {
	...
	try {
		thinkingPost = await client.createPost(
			post.channel_id,
			"_Thinking..._",
			rootId,
		);
	...
	try {
		...
		const finalText =
			response.length > MM_MSG_LIMIT
				? `${response.substring(0, MM_MSG_LIMIT - 3)}...`
				: response;
...
const channelProvider: MattermostChannelProvider = {
	...
	async sendNotification(
		...
		if (!ownerUserId && callbacks) {
			throw new Error(
				`Cannot determine owner for channel ${channelId} — sendNotification requires a DM channel`,
			);
		}
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext): Promise<void> {
		...
		cleanups.push(() =>
			ctx?.unregisterConfigSchema?.("wopr-plugin-mattermost"),
		);

		// Load config — support nested (channels.mattermost) and flat patterns
		type FullConfig = {
			channels?: { mattermost?: MattermostConfig };
		} & MattermostConfig;
		const fullConfig = ctx.getConfig<FullConfig>();
		config =
			fullConfig?.channels?.mattermost ||
			(fullConfig as MattermostConfig) ||
			{};
