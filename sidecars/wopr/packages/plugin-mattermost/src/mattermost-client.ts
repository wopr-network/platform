...
export class MattermostClient {
	...
	private async request<T>(
		...
		if (!res.ok) {
			...
			throw new Error(
				`Mattermost API ${method} ${path} failed (${res.status}): ${text}`,
			);
		}
	...
	async getDirectChannel(
		...
		return this.request<MattermostChannel>("POST", "/channels/direct", [
			userId1,
			userId2,
		]);
	}
	...
	async uploadFile(
		...
		if (!res.ok) {
			throw new Error(
				`File upload failed (${res.status}): ${await res.text()}`,
			);
		}
