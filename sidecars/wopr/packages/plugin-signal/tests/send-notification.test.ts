...
describe("sendNotification", () => {
  ...
  it("throws for unsupported notification type", async () => {
    ...
    await expect(sendNotification("chan-1", { type: "unknown-type" }, {})).rejects.toThrow(
      "Unsupported notification type: unknown-type",
    );
  });

  it("throws when no account is configured", async () => {
    const { default: plugin, sendNotification } = await import("../src/index.js");
    const mockCtx = createMockContext({
      getConfig: vi.fn().mockReturnValue({}),
    });
    await plugin.init(mockCtx);

    await expect(sendNotification("chan-1", { type: "friend-request", from: "Alice" }, {})).rejects.toThrow(
      "no Signal account",
    );
  ...
  it("sends DM to owner and registers parser on friend-request", async () => {
    const { default: plugin, sendNotification, signalChannelProvider } = await import("../src/index.js");
    ...
    await sendNotification("chan-1", { type: "friend-request", from: "Bob" }, { onAccept, onDeny });
  ...
  it("parser calls onAccept when owner replies ACCEPT", async () => {
    ...
    await sendNotification("chan-1", { type: "friend-request", from: "Charlie" }, { onAccept, onDeny });
  ...
  it("parser calls onDeny when owner replies DENY", async () => {
    ...
    await sendNotification("chan-1", { type: "friend-request", from: "Dave" }, { onAccept, onDeny });
  ...
  it("ignores responses from non-owner senders", async () => {
    ...
    await sendNotification("chan-1", { type: "friend-request", from: "Eve" }, { onAccept });
