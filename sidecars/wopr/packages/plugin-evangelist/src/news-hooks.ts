...
const patterns: Array<{
	keywords: string[];
	angleTemplate: (product: ProductInfo) => string;
}> = [
	{
	...
	},
	{
		...
		angleTemplate: (p) =>
			`Acquisition news — position "${p.oneLiner}" as a neutral, independent alternative.`,
	},
	{
		keywords: ["outage", "down", "incident", "failure"],
		angleTemplate: (p) =>
			`Reliability incident — position "${p.oneLiner}" as resilient with provider fallback.`,
	},
];

export function mapHeadlineToAngle(
	headline: string,
	product: ProductInfo,
): string {
...
export function createNewsHook(
	headline: string,
	source: string,
	product: ProductInfo,
): NewsHook {
