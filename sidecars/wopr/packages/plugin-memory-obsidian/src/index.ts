...
import type {
	MemoryObsidianConfig,
	MemoryObsidianExtension,
	VaultClient,
} from "./types.js";
import {
	registerMemoryObsidianTools,
	unregisterMemoryObsidianTools,
	WEBMCP_MANIFEST,
} from "./webmcp.js";
...
const configSchema: ConfigSchema = {
	...
	description:
		"Auto-store and recall conversation memories in your Obsidian vault",
	fields: [
		{
			name: "autoStore",
			type: "select",
			...
			options: [
				...
				{
					value: "heuristic",
					label: "Smart — store exchanges with memorable content (recommended)",
				},
		...
		},
		{
			...
			options: [
				{
					value: "heuristic",
					label: "Heuristic — extract key facts and preferences (recommended)",
				},
				{
					value: "full-exchange",
					label: "Full exchange — store the whole Q&A verbatim",
				},
			],
			default: "heuristic",
			setupFlow: "none",
		},
		{
			...
			options: [
				{
					value: "always",
					label: "Always — inject relevant memories before every message",
				},
...
const manifest: PluginManifest = {
	...
	homepage:
		"https://github.com/wopr-network/wopr-plugin-memory-obsidian#readme",
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		if (!obsidian) {
			ctx.log.warn(
				"wopr-plugin-obsidian extension not found — vault I/O will be unavailable until it loads",
			);
		}
			...
			async getContext(_session: string, message: MessageInfo) {
				...
				try {
					...
					return {
						...
						metadata: {
							source: "memory-obsidian",
							priority: 20,
							memoryCount: valid.length,
						},
					};
				} catch (error: unknown) {
					ctx?.log.warn(
		...
		const afterUnsub = ctx.events.on(
			"session:afterInject",
			async (payload: unknown) => {
				const { session, message, response } = payload as {
					session: string;
					message: string;
					response: string;
				};
				...
				try {
					...
					ctx?.log.warn(
						`Auto-store failed: ${error instanceof Error ? error.message : String(error)}`,
					);
				}
			},
		);
		...
		const extension: MemoryObsidianExtension = {
			async store(sessionId, content, tags = []) {
				...
				await vault.write(
					path,
					buildNoteContent(id, sessionId, allTags, now, content),
				);
