	...
	return [
		{
			...
			inputSchema: {
				...
				properties: {
					sessionId: {
						type: "string",
						description: "Session ID to associate this memory with",
					},
					content: {
						type: "string",
			...
			async handler(args: unknown) {
				const { sessionId, content, tags } = args as {
					sessionId: string;
					content: string;
					tags?: string[];
				};
		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					sessionId: {
						type: "string",
						description: "Session ID to scope the recall",
					},
					query: { type: "string", description: "What to recall" },
					limit: {
						type: "number",
						description: "Max memories returned (default 5)",
					},
				},
			...
			async handler(args: unknown) {
				const {
					...
					query,
					limit = 5,
				} = args as { sessionId: string; query: string; limit?: number };
				try {
		...
		},
		{
			name: "memory.forget",
			description:
				"Remove a memory from the index by ID (vault note is preserved)",
			inputSchema: {
				type: "object",
				properties: {
		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					sessionId: {
						type: "string",
						description: "Filter by session ID (omit for all memories)",
					},
				},
				required: [],
			},
