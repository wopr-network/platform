...
const validators: Record<string, ValidatorFn> = {
	...
	openai: async (key) => {
		try {
			...
			return {
				valid: false,
				error: `OpenAI API returned ${res.status}: ${body}`,
			};
		} catch (err) {
			return { valid: false, error: `Network error: ${String(err)}` };
		}
	...
	discord: async (key) => {
		try {
			...
			return {
				valid: false,
				error: `Discord API returned ${res.status}: ${body}`,
			};
		} catch (err) {
			return { valid: false, error: `Network error: ${String(err)}` };
		}
	...
	telegram: async (key) => {
		try {
			...
			return {
				valid: false,
				error: `Telegram API returned ${res.status}: ${body}`,
			};
		} catch (err) {
			return { valid: false, error: `Network error: ${String(err)}` };
		}
...
export async function validateKey(
	...
	if (!fn) {
		return {
			valid: false,
			error: `Unknown provider: ${provider}. Supported: ${Object.keys(validators).join(", ")}`,
		};
	}
