...
const PLATFORM_BASE_URL =
	process.env.WOPR_PLATFORM_URL ?? "http://localhost:7437";
	...
	return [
		// 1. setup.ask
		{
			...
			async handler(args): Promise<A2AToolResult> {
				...
				if (!session)
					return textResult(`No active setup session: ${sessionId}`, true);
		...
		// 2. setup.validateKey
		{
			...
			async handler(args): Promise<A2AToolResult> {
				...
				if (result.valid) {
					return textResult(
						`Key validated successfully for provider: ${provider}`,
					);
				}
			...
			inputSchema: {
				...
				properties: {
					...
					pluginId: {
						type: "string",
						description:
							"Plugin package name to install (e.g. @wopr-network/wopr-plugin-discord)",
					},
				},
			...
			async handler(args): Promise<A2AToolResult> {
				...
				try {
					...
					if (!res.ok || !body.success) {
						return textResult(
							`Failed to install ${pluginId}: ${body.error ?? `HTTP ${res.status}`}`,
							true,
						);
					}
					...
					return textResult(
						`Failed to install ${pluginId}: ${String(err)}`,
						true,
					);
				}
			},
			...
			async handler(args): Promise<A2AToolResult> {
				...
				try {
					const res = await fetch(
						`${PLATFORM_BASE_URL}/plugins/${service}/health`,
					);
					if (res.ok) {
					...
					return textResult(
						`Connection test failed for ${service}: HTTP ${res.status} — ${body}`,
						true,
					);
				} catch (err) {
					return textResult(
						`Connection test failed for ${service}: ${String(err)}`,
						true,
					);
				}
			},
		...
		// 5. setup.saveConfig
		{
			...
			async handler(args): Promise<A2AToolResult> {
				...
				if (
					...
					return textResult(
						`Field "${key}" is required and cannot be empty.`,
						true,
					);
				}
				if (field.pattern) {
				...
				const resultPromise = saveConfigQueue.then(
					async (): Promise<A2AToolResult> => {
						try {
							const currentConfig =
								ctx.getConfig<Record<string, unknown>>() ?? {};
		...
		// 6. setup.complete
		{
			name: "setup.complete",
			description:
				"Mark setup as done, activate the plugin, and emit setup:complete event.",
			inputSchema: {
				type: "object",
				properties: {
					sessionId: { type: "string", description: "Active setup session ID" },
			...
			async handler(args): Promise<A2AToolResult> {
				...
				if (session.completed)
					return textResult("Setup already completed.", true);
				...
				return textResult(
					`Setup complete for ${session.pluginId}. Plugin is now active.`,
				);
			},
		},
		...
		// 7. setup.rollback
		{
			name: "setup.rollback",
			description:
				"Undo all saveConfig and installDependency calls from this session. Fully transactional.",
			inputSchema: {
				type: "object",
				properties: {
					sessionId: { type: "string", description: "Active setup session ID" },
				...
				for (const mutation of reversed) {
					try {
						if (mutation.type === "saveConfig") {
							...
							await ctx.saveConfig(currentConfig);
							logger.info({
								msg: "Rolled back config",
								key: mutation.key,
								sessionId,
							});
						} else if (mutation.type === "installDependency") {
							const res = await fetch(
								`${PLATFORM_BASE_URL}/plugins/uninstall`,
								{
									method: "POST",
									headers: { "Content-Type": "application/json" },
							...
							if (!res.ok) {
								errors.push(
									`Failed to uninstall ${mutation.pluginId}: HTTP ${res.status}`,
								);
							} else {
								logger.info({
