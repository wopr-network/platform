...
function mockContext(overrides: Partial<WOPRPluginContext> = {}): WOPRPluginContext {
  return {
    ...
    getSessions: () => [],
    getPeers: () => [],
    getConfig: () => ({ channels: { slack: { dm: { allowFrom: ["U_AUTH"] } } } }) as any,
    saveConfig: vi.fn(),
    getMainConfig: () => ({}),
    ...
    getProvider: vi.fn((id: string) => {
      if (id === "anthropic") {
        return {
          id: "anthropic",
          name: "Anthropic",
          supportedModels: ["claude-sonnet-4-20250514", "claude-opus-4-20250514", "claude-haiku-4-20250514"],
        };
      }
      if (id === "openai") {
        return {
...
describe("registerSlashCommands", () => {
  ...
  describe("/verbose", () => {
    ...
    it("enables verbose with 'on'", async () => {
      ...
      await handlers.get("/verbose")!({ command, ack, respond });
      expect(respond).toHaveBeenCalledWith(expect.objectContaining({ text: expect.stringContaining("enabled") }));
    });
    ...
    it("disables verbose with 'off'", async () => {
      ...
      await handlers.get("/verbose")!({ command, ack, respond });
      expect(respond).toHaveBeenCalledWith(expect.objectContaining({ text: expect.stringContaining("disabled") }));
    });
  });
  ...
  describe("/model", () => {
    ...
    it("switches to a model by exact ID", async () => {
      const { command, ack, respond } = makeCommand("C_MODEL", "U_AUTH", "claude-opus-4-20250514");
      await handlers.get("/model")!({ command, ack, respond });
      expect(respond).toHaveBeenCalledWith(
        expect.objectContaining({
      ...
      expect(ctx.setSessionProvider).toHaveBeenCalledWith("slack-channel-C_MODEL", "anthropic", {
    ...
    it("falls back to local state when setSessionProvider is unavailable", async () => {
      ...
      const { command, ack, respond } = makeCommand("C_LOCAL", "U_AUTH", "claude-opus-4-20250514");
    ...
    it("reports error when setSessionProvider throws", async () => {
      ...
      const { command, ack, respond } = makeCommand("C_ERR", "U_AUTH", "claude-opus-4-20250514");
      await mock.handlers.get("/model")!({ command, ack, respond });
      expect(respond).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe("/session", () => {
    ...
    it("switches to a named session", async () => {
      ...
      expect(getEffectiveSessionKey("C_SESS", "U_AUTH", false)).toBe("slack-channel-C_SESS/work");
    });
    ...
    it("resets to default session", async () => {
      ...
      expect(getEffectiveSessionKey("C_SESS_DEF", "U_AUTH", false)).toBe("slack-channel-C_SESS_DEF");
    });
...
describe("getEffectiveSessionKey", () => {
  it("returns DM key for DM channels", () => {
    expect(getEffectiveSessionKey("D_CHAN", "U_USER", true)).toBe("slack-dm-U_USER");
  });

  it("returns channel key for non-DM channels", () => {
    expect(getEffectiveSessionKey("C_CHAN", "U_USER", false)).toBe("slack-channel-C_CHAN");
  });
});
