 ...
 */
function mockContext(config: Record<string, any> = {}): WOPRPluginContext {
...
describe("pairing", () => {
  ...
  describe("claimPairingCode", () => {
    ...
    it("normalizes code to uppercase and trims whitespace", () => {
      ...
      const result = claimPairingCode(`  ${code.toLowerCase()}  `, "source6", "U_CLAIM_4");
      expect(result.request).not.toBeNull();
    });
  ...
  describe("approveUser", () => {
    ...
    it("throws when saveConfig fails", async () => {
      ...
      await expect(approveUser(ctx, "U_APPROVE_4")).rejects.toThrow("Failed to save config");
    });
  });
});
