...
function mockContext(configOverride: Record<string, any> = {}): WOPRPluginContext {
...
describe("plugin default export", () => {
  ...
  describe("init", () => {
    ...
    it("throws when socket mode missing appToken", async () => {
      ...
      await expect(plugin.init!(ctx)).rejects.toThrow("App Token required for Socket Mode");
    });
    ...
    it("throws when HTTP mode missing signingSecret", async () => {
      ...
      await expect(plugin.init!(ctx)).rejects.toThrow("Signing Secret required for HTTP mode");
    });
  ...
  describe("message handler behavior", () => {
    ...
    it("responds to DMs in open policy mode", async () => {
      ...
      expect(ctx.inject).toHaveBeenCalledWith("slack-dm-U_DM_1", "hi bot", expect.objectContaining({ from: "U_DM_1" }));
    });
    ...
    it("uses channel session key for channel messages", async () => {
      ...
      expect(ctx.inject).toHaveBeenCalledWith("slack-channel-C_SESS", "test session", expect.any(Object));
    });
