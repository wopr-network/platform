...
vi.mock("@slack/bolt", () => {
  class MockApp {
    ...
    client = {
      ...
      chat: {
        update: sharedBoltMocks.chatUpdate,
        postMessage: sharedBoltMocks.chatPostMessage,
        delete: sharedBoltMocks.chatDelete,
      },
    };
  }
  return {
...
describe("saveAttachments", () => {
  ...
  it("downloads files using bot token in Authorization header", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("https://files.slack.com/files-pri/T123/download/report.pdf", {
...
describe("message handler file attachment integration", () => {
  ...
  it("handles multiple file attachments", async () => {
    fetchMock.mockResolvedValueOnce(mockFetchOk("a")).mockResolvedValueOnce(mockFetchOk("b"));
