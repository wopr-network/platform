...
import { withRetry, isRateLimitError, isTransientError, calculateDelay } from "../src/retry.js";
...
describe("retry", () => {
  ...
  describe("isTransientError", () => {
    it("returns true for rate limit errors", () => {
      expect(isTransientError({ code: "slack_webapi_rate_limited_error" })).toBe(true);
    });
  ...
  describe("withRetry", () => {
    ...
    it("retries on 5xx server error", async () => {
      const fn = vi.fn().mockRejectedValueOnce({ statusCode: 502 }).mockResolvedValue("recovered");
    ...
    it("throws immediately on non-transient error", async () => {
      ...
      await expect(withRetry(fn, { maxRetries: 3, baseDelay: 1, maxDelay: 10 })).rejects.toThrow("bad request");
    ...
    it("throws after exhausting all retries", async () => {
      ...
      await expect(withRetry(fn, { maxRetries: 2, baseDelay: 1, maxDelay: 10 })).rejects.toBe(rateLimitErr);
