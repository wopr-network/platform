...
const slackChannelProvider: SlackChannelProvider = {
	...
	async sendNotification(
		...
		if (payload.type !== "friend-request") {
			logger.warn({
				msg: "SlackChannelProvider.sendNotification: unsupported payload type",
				type: payload.type,
			});
			return;
		}
		...
		const fromLabel = escapeMrkdwn(
			payload.from || payload.pubkey || "unknown peer",
		);
		...
		if (callbacks) {
			...
			pendingNotifications.set(nonce, {
				callbacks,
				channelId,
				createdAt: Date.now(),
			});
...
const plugin: WOPRPlugin = {
	...
	async init(context: WOPRPluginContext) {
		...
		ctx.registerChannelProvider(
			slackChannelProvider as import("@wopr-network/plugin-types").ChannelProvider,
		);
		...
		try {
			...
			slackApp.action("notification_accept", async ({ ack, body }) => {
				await ack();
				const nonce = (
					body as unknown as { actions?: Array<{ value?: string }> }
				).actions?.[0]?.value;
				const incomingChannelId = (
					body as unknown as { channel?: { id?: string } }
				).channel?.id;
				if (!nonce) return;
				const pending = pendingNotifications.get(nonce);
				if (pending) {
					if (incomingChannelId && pending.channelId !== incomingChannelId)
						return;
					...
					try {
						...
						logger.error({
							msg: "Error in notification_accept callback",
							error: String(error),
						});
					}
				}
			...
			slackApp.action("notification_deny", async ({ ack, body }) => {
				...
				if (pending) {
					...
					try {
						...
						logger.error({
							msg: "Error in notification_deny callback",
							error: String(error),
						});
					}
				}
