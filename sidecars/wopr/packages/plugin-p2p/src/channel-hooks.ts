					...
					const callbacks: ChannelNotificationCallbacks = {
						onAccept: async () => {
							...
							if (result) {
								...
								await msgCtx.reply(formatFriendAccept(accept));
								ctx.log.info(
									`[p2p] Friend request from @${request.from} accepted via notification`,
								);
							}
						},
						onDeny: async () => {
							...
							ctx.log.info(
								`[p2p] Friend request from @${request.from} denied via notification`,
							);
						},
					};
					...
					for (const provider of ctx.getChannelProviders()) {
						if (provider.sendNotification) {
							try {
								...
								ctx.log.info(
									`[p2p] Sent notification to ${provider.id} for friend request from @${request.from}`,
								);
							} catch (err) {
								ctx.log.warn(
