...
export class MCPBridge {
	...
	async connect(config: ServerConfig): Promise<void> {
		...
		const client = new Client(
			{ name: `wopr-mcp-${config.name}`, version: "0.1.0" },
			{ capabilities: {} },
		);
		...
		const a2aTools: A2AToolDefinition[] = tools.map((tool) => ({
			...
			handler: async (
				...
				return {
					content: (result.content as A2AToolResult["content"]) ?? [
	...
	async disconnect(name: string): Promise<void> {
		...
		const ctxExt = this.ctx as WOPRPluginContext & {
			unregisterA2AServer?: (name: string) => void;
		};
		if (ctxExt.unregisterA2AServer) {
			ctxExt.unregisterA2AServer(`mcp-${name}`);
		} else {
			logger.warn({
				msg: "unregisterA2AServer not available — A2A tools will persist until plugin reload",
				name,
			});
		}
