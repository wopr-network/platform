...
const configSchema: ConfigSchema = {
	...
	description:
		"Configure Twitch bot integration with chat, whispers, and channel points",
	fields: [
		{
			name: "clientId",
			type: "text",
		...
		},
		{
			...
			description:
				"Listen for channel point redemptions (requires broadcasterId)",
		},
		{
			name: "dmPolicy",
			type: "select",
...
const plugin: WOPRPlugin = {
	...
	version: "1.0.0",
	description:
		"Twitch chat integration with whispers and channel point redemptions",
	...
	manifest: {
		...
		requires: {
			...
			network: {
				outbound: true,
				hosts: ["irc-ws.chat.twitch.tv", "api.twitch.tv"],
			},
		},
	...
	async init(ctx: WOPRPluginContext) {
		...
		if (!config.clientId || !config.clientSecret || !config.accessToken) {
			ctx.log.warn(
				"Twitch plugin not configured. Run 'wopr configure --plugin wopr-plugin-twitch'",
			);
			return;
		}
		...
		chatManager = new TwitchChatManager(
			ctx,
			{ ...config, channels },
			apiClient,
			botUserId,
