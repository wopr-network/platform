	...
	async sendNotification(
		...
		const parser: ChannelMessageParser = {
			...
			pattern: (msg: string) => {
				...
				return (
					lower === `!accept ${shortId.toLowerCase()}` ||
					lower === `!deny ${shortId.toLowerCase()}`
				);
			},
			handler: async (ctx: ChannelMessageContext) => {
				...
				if (
					ctx.channel?.replace(/^twitch:/, "").toLowerCase() !==
					channel.toLowerCase()
				)
					return;
...
export async function handleRegisteredCommand(
	...
	try {
		...
		console.error(
			`[twitch] Error executing command ${prefix}${cmdName}:`,
			error,
		);
		await cmdCtx.reply(
	...
	for (const parser of registeredParsers.values()) {
		...
		if (matches) {
			...
			try {
				...
				console.error(
					`[twitch] Message parser "${parser.id}" threw an unhandled error:`,
					err,
				);
				return false;
			}
