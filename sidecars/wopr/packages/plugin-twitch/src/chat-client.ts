...
export class TwitchChatManager {
	...
	async connect(authProvider: AuthProvider): Promise<void> {
		...
		if (this.config.enableWhispers !== false) {
			this.chatClient.onWhisper(
				async (user: string, text: string, msg: Whisper) => {
		...
		this.chatClient.onResub((channel, user, subInfo) => {
			this.ctx.log.info(
				`Resub: ${user} for ${subInfo.months} months in ${channel}`,
			);
		});

		this.chatClient.onRaid((channel, user, raidInfo) => {
			this.ctx.log.info(
				`Raid from ${user} with ${raidInfo.viewerCount} viewers in ${channel}`,
			);
		});
	...
	async handleMessagePublic(
		channel: string,
		_user: string,
		text: string,
		msg: ChatMessage | Whisper,
	): Promise<void> {
		...
		// Skip own messages
		if (userInfo.username.toLowerCase() === this.botUsername.toLowerCase())
			return;
	...
	async sendWhisper(
		...
		if (!this.apiClient || !this.botUserId) {
			this.ctx.log.warn(
				`Whisper to ${toUsername} skipped: API client or bot user ID not configured`,
			);
			return;
		}
