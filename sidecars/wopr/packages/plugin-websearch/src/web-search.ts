...
export function isPrivateUrl(urlStr: string): boolean {
	try {
		...
		if (ipVersion === 4) {
			if (PRIVATE_CIDR_PREFIXES.some((prefix) => bare.startsWith(prefix)))
				return true;
		} else if (ipVersion === 6) {
		...
		if (mappedIpv4) {
			...
			if (PRIVATE_CIDR_PREFIXES.some((prefix) => mappedIpv4.startsWith(prefix)))
				return true;
		}
...
function buildProvider(
	...
	switch (name) {
		case "google": {
			const apiKey =
				process.env.GOOGLE_SEARCH_API_KEY ?? config.providers?.google?.apiKey;
		...
		case "brave": {
			const apiKey =
				process.env.BRAVE_SEARCH_API_KEY ?? config.providers?.brave?.apiKey;
...
function getProviderOrder(config: WebSearchPluginConfig): ProviderName[] {
	...
	if (Array.isArray(order) && order.length > 0) {
		return order.filter((n): n is ProviderName =>
			["google", "brave", "xai"].includes(n),
		);
	}
...
export function buildWebSearchA2ATools(
	config: WebSearchPluginConfig,
): A2AServerConfig {
	return {
		name: "web-search",
				...
				inputSchema: {
					...
					properties: {
						...
						provider: {
							type: "string",
							description:
								"Force a specific provider: google, brave, xai. Omit for auto fallback chain.",
						},
					},
					required: ["query"],
				},
