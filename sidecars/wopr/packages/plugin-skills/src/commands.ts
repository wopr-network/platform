...
async function cmdList(ctx: WOPRPluginContext): Promise<void> {
	...
	if (skills.length === 0) {
		...
		for (const s of skills)
			ctx.log.info(`  ${s.name} - ${s.description ?? "No description"}`);
	}
}
	...
	switch (registryCmd) {
		case "list": {
			...
			if (registries.length === 0) {
				ctx.log.info(
					"No registries. Add: wopr skill registry add <name> <url>",
				);
			} else {
...
export const skillCommands: PluginCommand[] = [
	{
		name: "skill",
		description:
			"Manage skills: list, search, install, create, remove, enable, disable",
		usage: USAGE,
