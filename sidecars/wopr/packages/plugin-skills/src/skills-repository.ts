...
export async function getSkillState(
	name: string,
): Promise<SkillStateRecord | null> {
	await initSkillsStorage();
	const repo = skillsStateRepo();
	const existing = await repo.findFirst({ id: name } as Parameters<
		typeof repo.findFirst
	>[0]);
	return existing ?? null;
}

/** Get all skill states */
export async function getAllSkillStates(): Promise<
...
export async function readAllSkillStatesAsync(): Promise<
	Record<string, { enabled: boolean }>
> {
