...
function resolveWoprMetadata(
	...
	if (typeof frontmatter.metadata === "string") {
		try {
			const parsed = JSON.parse(frontmatter.metadata) as {
				wopr?: SkillMetadata;
				clawdbot?: SkillMetadata;
			};
	...
	return (
		(frontmatter.metadata as { wopr?: SkillMetadata; clawdbot?: SkillMetadata })
			.wopr || (frontmatter.metadata as { clawdbot?: SkillMetadata }).clawdbot
	);
}
...
function resolveCommandDispatch(
	frontmatter: ParsedFrontmatter,
): SkillCommandDispatch | undefined {
	...
	if (!toolName) {
		logger.warn(
			`Skill requested tool dispatch but did not provide command-tool`,
		);
		return undefined;
	}
...
function loadSkillFromFile(
	...
	try {
		...
		const { frontmatter, warnings: fmWarnings } =
	...
	for (const { dir, source } of sources) {
		...
		for (const entry of entries) {
			...
			if (
				...
				!matchesPattern(skill.name, includeSkills)
			) {
				continue;
			}
...
export async function installSkillDependencies(
	...
	for (const step of installSteps) {
		...
		try {
			approved = await consentProvider.requestConsent(
				skill.name,
				step,
				rawCommand,
			);
			...
			logger.warn(
				`Consent provider threw for step "${step.id}" of skill "${skill.name}"; failing closed`,
				err,
			);
			return false;
		}
		if (!approved) {
			logger.info(
				`User declined install step "${step.id}" for skill "${skill.name}": ${rawCommand}`,
			);
			return false;
		}
		...
		try {
			switch (step.kind) {
				case "brew":
					if (step.formula) {
						execFileSync("brew", ["install", step.formula], {
							stdio: "inherit",
						});
					}
					break;
				case "apt":
					if (step.package) {
						execFileSync("sudo", ["apt-get", "install", "-y", step.package], {
							stdio: "inherit",
						});
					}
					break;
				case "npm":
					if (step.package) {
						execFileSync("npm", ["install", "-g", step.package], {
							stdio: "inherit",
						});
					}
					break;
				case "pip":
					if (step.package) {
						execFileSync("pip", ["install", step.package], {
							stdio: "inherit",
						});
					}
					break;
				case "script":
					if (step.script) {
						execFileSync("/bin/bash", ["-c", step.script], {
							stdio: "inherit",
						});
					}
					break;
...
export function installSkillFromGitHub(
	...
	if (
		...
		pathSegments.some((seg) => !validPathSegment.test(seg) || seg === "..")
	) {
	...
	try {
		...
		execFileSync(
			"git",
			["-C", tmpDir, "sparse-checkout", "set", normalizedSkillPath],
			{ stdio: "pipe" },
		);
