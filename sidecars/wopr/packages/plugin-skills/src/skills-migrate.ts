...
export async function migrateSkillsToSQL(
	...
	const skillsRepo = ctx.storage.getRepository<SkillStateRecord>(
		"skills",
		"skills_state",
	);
	...
	for (const [skillName, state] of Object.entries(skillsState)) {
		try {
			...
			logger.error(
				`[migration] Failed to migrate skill state "${skillName}":`,
				error,
			);
		}
	}
...
async function migrateSkillState(
	...
	logger.debug(
		`[migration] Migrating skill state "${skillName}" (enabled: ${state.enabled})`,
	);
...
export async function migrateRegistriesToSQL(): Promise<void> {
	if (!existsSync(REGISTRIES_FILE)) {
		logger.info(
			"[migration] No registries.json found, skipping registry migration",
		);
		return;
	}
	...
	for (const reg of registries) {
		...
		try {
			...
			if (!msg.includes("already exists")) {
				logger.error(
					`[migration] Failed to migrate registry "${reg.name}":`,
					error,
				);
			}
		}
