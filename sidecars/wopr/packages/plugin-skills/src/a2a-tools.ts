...
export function registerSkillsA2ATools(): void {
	if (!pluginCtx?.registerA2AServer) {
		logger.debug(
			"[skills] registerA2AServer not available in this WOPR version, skipping A2A tool registration",
		);
		return;
	}
		...
		tools: [
			{
				name: "skills.list",
				description:
					"List all discovered skills and their enabled/disabled state",
				inputSchema: {
					type: "object",
					properties: {
						source: {
							type: "string",
							description:
								"Filter by source: managed, workspace, bundled, extra",
						},
					},
