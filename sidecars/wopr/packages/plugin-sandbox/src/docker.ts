...
async function readContainerConfigHash(
	...
	const result = await execDocker(
		[
			"inspect",
			"-f",
			'{{ index .Config.Labels "wopr.configHash" }}',
			containerName,
		],
		{
			allowFailure: true,
...
export async function ensureSandboxContainer(params: {
	...
	if (hasContainer) {
		...
		if (hashMismatch) {
			...
			if (isHot) {
				...
				logger.info(
					`[sandbox] Config changed for ${containerName}, recreating...`,
				);
	...
	if (!hasContainer) {
		logger.info(
			`[sandbox] Creating container ${containerName} for session ${params.sessionKey}`,
		);
