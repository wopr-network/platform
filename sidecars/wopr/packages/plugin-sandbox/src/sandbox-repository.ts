...
function sandboxRepo(): Repository<SandboxRegistryRecord> {
	return getStorage().getRepository<SandboxRegistryRecord>(
		"sandbox",
		"sandbox_registry",
	);
}
 ...
 */
export async function removeRegistryEntrySQL(
	containerName: string,
): Promise<void> {
	await initSandboxStorage();
	const repo = sandboxRepo();
 ...
 */
export async function findRegistryEntrySQL(
	containerName: string,
): Promise<SandboxRegistryRecord | undefined> {
	await initSandboxStorage();
	const repo = sandboxRepo();
 ...
 */
export async function listRegistryEntriesSQL(): Promise<
