...
export function resolveSandboxDockerConfig(params: {
	...
	return {
		...
		containerPrefix:
			session?.containerPrefix ??
			global?.containerPrefix ??
			DEFAULT_SANDBOX_CONTAINER_PREFIX,
...
export function resolveSandboxPruneConfig(params: {
	...
	return {
		idleHours:
			session?.idleHours ?? global?.idleHours ?? DEFAULT_SANDBOX_IDLE_HOURS,
		maxAgeDays:
			session?.maxAgeDays ?? global?.maxAgeDays ?? DEFAULT_SANDBOX_MAX_AGE_DAYS,
	};
}
...
export function resolveSandboxConfig(params?: {
	...
	if (
		...
		params?.trustLevel === "semi-trusted"
	) {
	...
	let workspaceAccess: SandboxWorkspaceAccess =
		woprSandbox?.workspaceAccess ?? "none";
