...
const configSchema: ConfigSchema = {
	...
	fields: [
		...
		},
		{
			...
			description:
				"Comma-separated pubkeys allowed to DM (when DM policy is allowlist)",
		},
	],
};
...
const plugin: WOPRPlugin = {
	...
	version: "1.0.0",
	description:
		"Nostr protocol integration for WOPR — encrypted DMs and public replies",
	...
	async init(ctx: WOPRPluginContext) {
		...
		try {
			...
			if (!nsecInput) {
				ctx.log.warn(
					"No Nostr private key configured. Run 'wopr configure --plugin nostr' to set up.",
				);
				return;
			}
		...
		subscription = poolManager.subscribe(filters, {
			onevent: (event: unknown) => {
				handler
					.handleEvent(event as Parameters<EventHandler["handleEvent"]>[0])
					.catch((error: unknown) => {
		...
		ctx.log.info(
			`Nostr plugin initialized — listening on ${relayUrls.length} relays`,
		);
	},
