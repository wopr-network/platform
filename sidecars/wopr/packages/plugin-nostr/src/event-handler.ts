...
export class EventHandler {
	...
	private async handleDM(event: NostrEvent): Promise<void> {
		...
		this.ctx.logMessage(sessionKey, plaintext, {
			from: npub,
			channel: channelRef,
		});
		...
		for (const parser of parsers) {
			const matched =
				parser.pattern instanceof RegExp
					? parser.pattern.test(plaintext)
					: parser.pattern(plaintext);
			if (matched) {
	...
	private async handleMention(event: NostrEvent): Promise<void> {
		...
		const cleanContent = event.content
			.replace(new RegExp(`nostr:${ourNpub}|@${ourNpub}`, "g"), "")
			.trim();

		let response: string;
		try {
			response = await this.ctx.inject(
				sessionKey,
				`[${npub}]: ${cleanContent}`,
				{
					from: npub,
