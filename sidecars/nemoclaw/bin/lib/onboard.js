...
const { CLOUD_MODEL_OPTIONS, DEFAULT_CLOUD_MODEL, getProviderSelectionConfig } = require("./inference-config");
const { inferContainerRuntime, isUnsupportedMacosRuntime, shouldPatchCoredns } = require("./platform");
...
function patchStagedDockerfile(
  ...
  const { providerKey, primaryModelRef, inferenceBaseUrl, inferenceApi, inferenceCompat } = getSandboxInferenceConfig(
    model,
    provider,
  ...
  dockerfile = dockerfile.replace(/^ARG NEMOCLAW_PROVIDER_KEY=.*$/m, `ARG NEMOCLAW_PROVIDER_KEY=${providerKey}`);
  ...
  dockerfile = dockerfile.replace(/^ARG NEMOCLAW_INFERENCE_API=.*$/m, `ARG NEMOCLAW_INFERENCE_API=${inferenceApi}`);
  dockerfile = dockerfile.replace(
    /^ARG NEMOCLAW_INFERENCE_COMPAT_B64=.*$/m,
    `ARG NEMOCLAW_INFERENCE_COMPAT_B64=${encodeDockerJsonArg(inferenceCompat)}`,
  );
  dockerfile = dockerfile.replace(/^ARG NEMOCLAW_BUILD_ID=.*$/m, `ARG NEMOCLAW_BUILD_ID=${buildId}`);
...
function summarizeProbeError(body, status) {
  ...
  try {
    ...
    const message =
      parsed?.error?.message || parsed?.error?.details || parsed?.message || parsed?.detail || parsed?.details;
...
function probeAnthropicEndpoint(endpointUrl, model, apiKey) {
  const bodyFile = path.join(
    os.tmpdir(),
    `nemoclaw-anthropic-probe-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
  try {
    const cmd = [
      "curl -sS",
...
function fetchNvidiaEndpointModels(apiKey) {
  const bodyFile = path.join(
    os.tmpdir(),
    `nemoclaw-nvidia-models-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
  try {
    const cmd = [
      "curl -sS",
    ...
    const parsed = JSON.parse(body);
    const ids = Array.isArray(parsed?.data) ? parsed.data.map((item) => item && item.id).filter(Boolean) : [];
    return { ok: true, ids };
  } catch (error) {
...
function fetchOpenAiLikeModels(endpointUrl, apiKey) {
  const bodyFile = path.join(
    os.tmpdir(),
    `nemoclaw-openai-models-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
  try {
    const cmd = [
      "curl -sS",
...
function fetchAnthropicModels(endpointUrl, apiKey) {
  const bodyFile = path.join(
    os.tmpdir(),
    `nemoclaw-anthropic-models-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
  );
  try {
    const cmd = [
      "curl -sS",
...
function installOpenshell() {
  ...
  const futureShellPathHint = fs.existsSync(openshellPath) ? getFutureShellPathHint(localBin, process.env.PATH) : null;
...
async function startGateway(_gpu) {
  ...
  const stableGatewayImage = openshellVersion ? `ghcr.io/nvidia/openshell/cluster:${openshellVersion}` : null;
...
async function setupNim(gpu) {
  ...
  options.push({
    key: "build",
    label: "NVIDIA Endpoints" + (!ollamaRunning && !(EXPERIMENTAL && vllmRunning) ? " (recommended)" : ""),
  });
  ...
  if (hasOllama || ollamaRunning) {
    options.push({
      key: "ollama",
      label:
        `Local Ollama (localhost:11434)${ollamaRunning ? " — running" : ""}` + (ollamaRunning ? " (suggested)" : ""),
    });
  }
...
async function setupOpenclaw(sandboxName, model, provider) {
  ...
  if (selectionConfig) {
    ...
    try {
      run(`${openshellShellCommand(["sandbox", "connect", sandboxName])} < ${shellQuote(scriptFile)}`, {
...
function fetchGatewayAuthTokenFromSandbox(sandboxName) {
  ...
  try {
    ...
    const result = runOpenshell(["sandbox", "download", sandboxName, "/sandbox/.openclaw/openclaw.json", destDir], {
      ignoreError: true,
...
function buildControlUiChatUrls(token) {
  ...
  const bases = [`http://127.0.0.1:${CONTROL_UI_PORT}`, `http://localhost:${CONTROL_UI_PORT}`];
