...
export const feedbackExports = pgTable(
  "feedback_exports",
  {
    ...
    feedbackVoteId: uuid("feedback_vote_id")
      .notNull()
      .references(() => feedbackVotes.id, { onDelete: "cascade" }),
  ...
  (table) => ({
    ...
    companyProjectIdx: index("feedback_exports_company_project_idx").on(
      table.companyId,
      table.projectId,
      table.createdAt,
    ),
    companyAuthorIdx: index("feedback_exports_company_author_idx").on(
      table.companyId,
      table.authorUserId,
      table.createdAt,
    ),
