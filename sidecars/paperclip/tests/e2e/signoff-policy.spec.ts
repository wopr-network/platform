...
async function agentPatch(board: APIRequestContext, agent: AgentAuth, issueId: string, data: Record<string, unknown>) {
...
test.describe("Signoff execution policy", () => {
  ...
  test("happy path: executor → review → approval → done", async ({ page }) => {
    ...
    const step1Res = await agentCheckoutAndPatch(ctx.boardRequest, ctx.executor, issueId, ["in_progress"], {
      status: "done",
    ...
    const step3Res = await agentPatch(ctx.boardRequest, ctx.reviewer, issueId, {
      status: "done",
    ...
    const step5Res = await agentPatch(ctx.boardRequest, ctx.approver, issueId, {
      status: "done",
  ...
  test("changes requested: reviewer bounces back to executor", async () => {
    ...
    const doneRes = await agentCheckoutAndPatch(ctx.boardRequest, ctx.executor, issueId, ["in_progress"], {
      status: "done",
    ...
    const changesRes = await agentPatch(ctx.boardRequest, ctx.reviewer, issueId, {
      status: "in_progress",
    ...
    const resubmitRes = await agentCheckoutAndPatch(ctx.boardRequest, ctx.executor, issueId, ["in_progress"], {
      status: "done",
      comment: "Fixed the edge cases.",
    });
  ...
  test("comment required: approval without comment fails", async () => {
    ...
    // Executor marks done → routes to reviewer
    await agentCheckoutAndPatch(ctx.boardRequest, ctx.executor, issueId, ["in_progress"], {
      status: "done",
      comment: "Done.",
    });

    // Reviewer tries to approve without comment → should fail
    const noCommentRes = await agentPatch(ctx.boardRequest, ctx.reviewer, issueId, { status: "done" });
  ...
  test("non-participant cannot advance stage", async () => {
    ...
    const advanceRes = await agentPatch(ctx.boardRequest, ctx.approver, issueId, {
      status: "done",
  ...
  test("review-only policy: reviewer approval completes execution", async () => {
    ...
    // Executor marks done → routes to reviewer
    const doneRes = await agentCheckoutAndPatch(ctx.boardRequest, ctx.executor, issue.id, ["in_progress"], {
      status: "done",
      comment: "Ready for review.",
    });
    expect(doneRes.ok()).toBe(true);
    ...
    const approveRes = await agentPatch(ctx.boardRequest, ctx.reviewer, issue.id, { status: "done", comment: "LGTM." });
