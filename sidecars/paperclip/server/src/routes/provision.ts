        ...
        const agent = await agents.create(tenantEntityId, {
          ...
          adapterConfig: {
            ...
            promptTemplate: [
              ...
              "",
              "You have the `paperclip` skill available. Use it to interact with your company.",
              "Run `/paperclip` to see available commands, or use these directly:",
              "",
              "To read your assigned task:",
              "  curl -s $PAPERCLIP_API_URL/api/issues/$PAPERCLIP_TASK_ID -H 'x-paperclip-user-id: {{agent.id}}'",
              "",
              "To list issues in your company:",
              "  curl -s $PAPERCLIP_API_URL/api/companies/{{agent.companyId}}/issues -H 'x-paperclip-user-id: {{agent.id}}'",
              "",
              "To list agents on your team:",
              "  curl -s $PAPERCLIP_API_URL/api/companies/{{agent.companyId}}/agents -H 'x-paperclip-user-id: {{agent.id}}'",
              "",
              ...
              "",
              "Start by reading your assigned task (PAPERCLIP_TASK_ID=$PAPERCLIP_TASK_ID), then execute it.",
            ].join("\n"),
            env: {
