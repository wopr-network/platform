...
import {
  ...
  resolveInstanceSharedWorkspaceDir,
...
export function heartbeatService(db: Db) {
  ...
  async function resolveWorkspaceForRun(
    ...
    if (projectWorkspaceRows.length > 0) {
      ...
      // Fall back to instance-shared workspace so all agents on this instance
      // share the same working tree (installed deps persist across runs).
      // Sandbox isolation: our managed image runs agents as the `sandbox` user
      // rather than per-agent homes — see docs/specs/agent-sandbox-and-connected-accounts.md
