  ...
  return {
    ...
    firePublicTrigger: async (
      ...
      if (trigger.signingMode === "none") {
        ...
        const valid = normalizedBuf.length === expectedBuf.length && crypto.timingSafeEqual(normalizedBuf, expectedBuf);
      ...
      return dispatchRoutineRun({
        ...
        variables:
          isPlainRecord(input.payload) && isPlainRecord(input.payload.variables) ? input.payload.variables : null,
        idempotencyKey: input.idempotencyKey,
      });
    },
