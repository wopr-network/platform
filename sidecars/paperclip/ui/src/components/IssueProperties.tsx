...
import { User, Hexagon, ArrowUpRight, Tag, Plus, GitBranch, FolderOpen, Check, ExternalLink, X, Clock, RotateCcw, Loader2, CheckCircle2, ScanEye, Trash2 } from "lucide-react";
          ...
          .map((label) => {
            ...
            return (
              <div
                key={label.id}
                className={cn(
                  "flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-left group",
                  selected && "bg-accent"
                )}
              >
                <button
                ...
                <button
                  type="button"
                  className="p-1 text-muted-foreground hover:text-destructive rounded opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
