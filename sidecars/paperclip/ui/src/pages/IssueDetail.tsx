...
export function IssueDetail() {
  ...
  const issuePanelKey = issue ? `${issue.id}:${String(issue.updatedAt)}:${childIssuesPanelKey}` : "";
  const openNewSubIssue = useCallback(() => {
    ...
    openNewIssue({
      ...
      executionWorkspaceMode: issue.executionWorkspaceId
        ? "reuse_existing"
        : (issue.executionWorkspacePreference ?? undefined),
  ...
  const timelineEvents = useMemo(() => extractIssueTimelineEvents(activity), [activity]);
