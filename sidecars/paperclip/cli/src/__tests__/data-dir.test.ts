...
describe("applyDataDirOverride", () => {
  ...
  it("sets PAPERCLIP_HOME and isolated default config/context paths", () => {
    ...
    expect(process.env.PAPERCLIP_CONFIG).toBe(path.resolve(expectedHome, "instances", "default", "config.json"));
  ...
  it("uses the provided instance id when deriving default config path", () => {
    ...
    expect(process.env.PAPERCLIP_CONFIG).toBe(path.resolve("/tmp/paperclip-alt", "instances", "dev_1", "config.json"));
  });
