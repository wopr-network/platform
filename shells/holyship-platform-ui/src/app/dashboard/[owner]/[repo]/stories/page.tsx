...
export default function StoriesPage({ params }: { params: Promise<{ owner: string; repo: string }> }) {
  ...
  async function handleSubmit(categories: AuditCategory[], customInstructions?: string) {
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="rounded-lg border border-border bg-card p-8 text-center max-w-md">
          ...
          <h2 className="text-lg font-bold text-card-foreground mb-1">Run Analyze first</h2>
          <p className="text-sm text-muted-foreground mb-4">
            This repo hasn&apos;t been analyzed yet. Analyze it to unlock story generation.
          </p>
          <Link
  ...
  return (
    <div className="max-w-3xl space-y-6">
      ...
      {results && <AuditResults issues={results} owner={owner} repo={repo} onRerun={handleRerun} />}
    </div>
  );
}
