...
const categories: {
...
}[] = [
  {
    id: "code_quality",
    label: "Code Quality",
    description: "TODOs, dead code, large files, duplication, inconsistent patterns",
  ...
  },
  {
    id: "ecosystem",
    label: "Ecosystem",
    description: "What similar projects are doing, platform updates to leverage",
  ...
  },
  {
    id: "tech_debt",
    label: "Tech Debt",
    description: "God objects, missing types, leaky abstractions, config sprawl",
  ...
  return (
    <div className="space-y-3">
        ...
        return (
          <label
            ...
            className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 transition-colors ${
              isChecked ? `${cat.border} ${cat.bg}` : "border-border bg-card hover:border-muted-foreground/30"
            }`}
          >
            <input type="checkbox" className="sr-only" checked={isChecked} onChange={() => toggle(cat.id)} />
            <div
              className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded border transition-colors ${
                isChecked ? `${cat.border} bg-current/20` : "border-muted-foreground/40"
              }`}
            >
              {isChecked && (
                <svg
      ...
      >
        ...
        <div>
          <span className="text-sm font-semibold text-card-foreground">Custom Agent</span>
          <p className="text-xs text-muted-foreground">Your instructions — tell Holy Ship exactly what to look for</p>
        </div>
      </label>
