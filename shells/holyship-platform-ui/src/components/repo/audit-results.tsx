...
export function AuditResults({ issues, owner, repo, onRerun }: AuditResultsProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {priorityOrder.map((p) =>
          counts[p] ? (
            <span key={p} className={`rounded-full px-3 py-1 text-xs font-semibold ${priorityColors[p]}`}>
              {counts[p]} {p}
            </span>
          ) : null,
        )}
