  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <h3 className="font-bold text-card-foreground truncate">{repo.full_name}</h3>
        {analyzed ? (
      ...
      {analyzed && summaryParts.length > 0 && (
        <p className="mt-2 text-sm text-muted-foreground truncate">{summaryParts.join(" · ")}</p>
      )}

      {analyzed ? (
        <p className="mt-3 text-sm text-muted-foreground">
          <span className="text-card-foreground font-medium">{repo.inFlight ?? 0}</span> in flight{" · "}
          <span className="text-card-foreground font-medium">{repo.shippedToday ?? 0}</span> shipped today{" · "}
          <span className="text-card-foreground font-medium">{repo.openGaps ?? 0}</span> gaps open
        </p>
      ) : (
        <p className="mt-3 text-sm font-medium text-amber-400">Click to analyze &rarr;</p>
      )}
    </Link>
  );
}
