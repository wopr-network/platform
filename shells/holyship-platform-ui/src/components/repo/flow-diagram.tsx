...
function Arrow() {
	return (
		<span className="text-muted-foreground text-lg leading-none">&darr;</span>
	);
}
...
function buildMainPath(flow: DesignedFlow): {
	...
	const hasReviewToFix = transitions.some(
		(t) => t.fromState === "review" && t.toState === "fix",
	);
	const hasFixToReview = transitions.some(
		(t) => t.fromState === "fix" && t.toState === "review",
	);
...
function ReviewFixLoop({
	...
	const reviewToFix = transitions.find(
		(t) => t.fromState === "review" && t.toState === "fix",
	);
	const fixToReview = transitions.find(
		(t) => t.fromState === "fix" && t.toState === "review",
	);
...
export function FlowDiagram({ flow, pendingFlow }: FlowDiagramProps) {
	...
	const removedStateNames = pendingFlow
		? flow.states
				.map((s) => s.name)
				.filter((name) => !pendingFlow.states.some((s) => s.name === name))
		: [];
		...
		<div className="bg-muted/30 rounded-lg p-4 space-y-1">
			<div className="flex flex-col items-center gap-1">
				{path.map((state, idx) => {
					...
					const transition = displayFlow.transitions.find(
						(t) => t.fromState === state && t.toState === nextState,
					);
					const gate = transition
						? gateForTransition(
								transition.fromState,
								transition.trigger,
								displayFlow.gateWiring,
							)
						: null;
					...
					return (
						<div key={state} className="flex flex-col items-center gap-1">
							{isReview ? (
								...
								<StatePill
									name={state}
									diff={diffState(state, flow, pendingFlow)}
								/>
							)}
					...
					<div className="flex flex-wrap justify-center gap-2">
						{terminalStates.map((s) => (
							<StatePill
								key={s}
								name={s}
								diff={diffState(s, flow, pendingFlow)}
							/>
						))}
					</div>
				</div>
