...
export function PaperclipDashboard() {
	...
	useEffect(() => {
		...
		for (const [name, _inst] of provisioning) {
			const server = serverInstances.find(
				(s) => toSubdomainLabel(s.name) === name,
			);
	...
	const instances: PaperclipInstance[] = useMemo(() => {
		...
		for (const [name, inst] of provisioning) {
			const exists = serverInstances.some(
				(s) => toSubdomainLabel(s.name) === name,
			);
	...
	return (
		<div className="space-y-8">
			...
			<div>
				...
				<p className="font-mono text-xs text-indigo-400/50 tracking-wide mt-1">
					{statusSubtitle()}
				</p>
			</div>
