  ...
  return (
    ...
    >
      ...
      {instance.status !== "provisioning" && (
        <>
          ...
          <span
            ...
            onClick={() => window.open(`https://${instance.subdomain}`, "_blank", "noopener,noreferrer")}
          />
        </>
      )}
      ...
      <div className="relative z-10 flex items-center gap-3 mb-3">
        <span
          className={cn("size-2.5 rounded-full flex-shrink-0", s.dot, s.pulse && "animate-pulse")}
          aria-hidden="true"
        />
        <h2 className={cn("font-semibold tracking-tight truncate", isHero ? "text-2xl" : "text-base")}>
          {instance.name}
        </h2>
        <span className={cn("text-[10px] font-medium tracking-[0.15em] uppercase flex-shrink-0", s.text)}>
          {s.label}
        </span>
      </div>
      ...
      >
        <Settings
          className={cn("transition-transform duration-200 group-hover:rotate-45", isHero ? "size-5" : "size-4")}
        />
      </Link>
    </div>
  );
