...
export const metadata: Metadata = {
  ...
  description: "Simple pricing. $5/month per agent. No tiers. No gotchas. Pay with Stripe or crypto.",
};

const features = [
  "$5 signup credit included",
  "Managed AI agents",
  "All models via hosted adapter",
...
const usageRates = [
  { model: "GPT-4o", rate: "$0.005 / 1K tokens" },
  { model: "Claude Sonnet", rate: "$0.003 / 1K tokens" },
  { model: "DeepSeek V3", rate: "$0.001 / 1K tokens" },
  { model: "Llama 3.3", rate: "$0.0005 / 1K tokens" },
];
    ...
    <div className="min-h-dvh px-6 py-24 text-white" style={{ background: "#09090b" }}>
      <div className="mx-auto max-w-2xl space-y-12">
        <div className="text-center">
          ...
          <p className="mt-3 text-lg text-zinc-400">One plan. No tiers. No gotchas.</p>
        </div>

        <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-8 text-center">
          <p className="text-sm uppercase tracking-widest text-zinc-500">Paperclip</p>
          <p
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-zinc-400">
            Credits are consumed at transparent per-use rates. No markup surprises.
          </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/50">
                  ...
                  <th className="px-4 py-3 text-right font-medium text-zinc-400">Rate</th>
                </tr>
              </thead>
              <tbody>
                {usageRates.map((r) => (
                  <tr key={r.model} className="border-b border-zinc-800/50 last:border-0">
                    <td className="px-4 py-3 text-zinc-300">{r.model}</td>
                    <td className="px-4 py-3 text-right font-mono text-zinc-400">{r.rate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="text-xs text-zinc-500">
            Rates are approximate and may vary. See your usage dashboard for exact costs.
          </p>
        </div>
