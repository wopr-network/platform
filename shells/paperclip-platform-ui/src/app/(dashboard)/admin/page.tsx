...
function StatusBadge({ state }: { state: string | null }) {
	...
	const cls =
		colors[label] ?? "bg-zinc-500/20 text-zinc-400 border-zinc-500/30";
	...
	return (
		<div className="p-6 max-w-6xl space-y-6">
					...
					<>
						<div className="flex gap-3 items-end">
							...
							<button
								...
								onClick={handleSave}
								disabled={
									setModel.isPending || modelInput.trim() === currentModel
								}
						...
						{!currentModel && (
							<p className="text-xs text-indigo-500 mt-2">
								No model set in database. Falling back to GATEWAY_DEFAULT_MODEL
								env var.
							</p>
						)}
					</>
			...
			<Section
				...
				description="Platform-wide billing summary across all tenants."
			>
				{billingOverview.isLoading ? (
					<div className="text-zinc-500">Loading...</div>
				) : billingOverview.error ? (
					<div className="text-red-400 text-sm">
						Failed to load: {billingOverview.error.message}
					</div>
				) : (
					<div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
						...
						<div className="rounded-lg border border-indigo-500/10 bg-indigo-500/[0.03] p-4">
							...
							<p className="text-2xl font-bold font-mono text-zinc-100 mt-1">
								{billingOverview.data?.orgCount ?? 0}
							</p>
						</div>
						<div className="rounded-lg border border-indigo-500/10 bg-indigo-500/[0.03] p-4">
							<p className="text-xs text-zinc-500 uppercase tracking-wider">
			...
			<Section
				...
				description="All instances across all tenants with live status."
			>
				{allInstances.isLoading ? (
					<div className="text-zinc-500">Loading...</div>
				) : allInstances.error ? (
					<div className="text-red-400 text-sm">
						Failed to load: {allInstances.error.message}
					</div>
								...
								>
									...
									<div className="space-y-1 text-xs text-zinc-400">
										...
										<div className="flex justify-between">
											<span>Tenant</span>
											<span
												className="font-mono text-zinc-500 truncate max-w-[160px]"
												title={inst.tenantId}
											>
			...
			>
				{allOrgs.isLoading ? (
					...
					<div className="text-red-400 text-sm">
						Failed to load: {allOrgs.error.message}
					</div>
