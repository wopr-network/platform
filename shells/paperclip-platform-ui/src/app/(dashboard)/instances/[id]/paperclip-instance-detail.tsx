...
import { updateInstanceBudget } from "@/lib/paperclip-api";

export function PaperclipInstanceDetail({
	instanceId,
}: {
	instanceId: string;
}) {
...
function BudgetSection({ instanceId }: { instanceId: string }) {
	...
	async function handleSave() {
		...
		const perAgent = perAgentDollars
			? Number.parseFloat(perAgentDollars)
			: undefined;
			...
			<CardContent className="space-y-4">
				<div className="grid gap-4 sm:grid-cols-2">
					<div>
						<label
							htmlFor="budget"
							className="font-mono text-xs text-muted-foreground/60 mb-1 block"
						>
							Monthly budget ($)
						</label>
						<Input
