...
test.describe("Onboarding wizard", () => {
  test("complete onboarding — happy path", async ({ page }) => {
    ...
    await expect(page.locator('[data-onboarding-id="onboarding.name-bot"]').first()).toBeVisible();
    await page.locator('[data-onboarding-id="onboarding.name-bot"]').first().fill("TestBot");
    await page.locator('[data-onboarding-id="onboarding.continue.name"]').first().click();

    // Step 1: Pick a preset
    await expect(page.locator('[data-onboarding-id="onboarding.select-preset.discord-ai-bot"]').first()).toBeVisible();
    await page.locator('[data-onboarding-id="onboarding.select-preset.discord-ai-bot"]').first().click();
    await page.locator('[data-onboarding-id="onboarding.continue.preset"]').first().click();

    // Step 2: Launch
    await expect(page.locator('[data-onboarding-id="onboarding.launch"]').first()).toBeVisible();
    await expect(page.getByText("TestBot").first()).toBeVisible();
    await expect(page.getByText("Discord AI Bot").first()).toBeVisible();
    await page.locator('[data-onboarding-id="onboarding.launch"]').first().click();
  ...
  test("partial completion — resume from last step", async ({ page }) => {
    ...
    // Step 0: Name your WOPR
    await expect(page.locator('[data-onboarding-id="onboarding.name-bot"]').first()).toBeVisible();
    await page.locator('[data-onboarding-id="onboarding.name-bot"]').first().fill("ResumeBot");
    await page.locator('[data-onboarding-id="onboarding.continue.name"]').first().click();
    ...
    await expect(page.locator('[data-onboarding-id="onboarding.select-preset.discord-ai-bot"]').first()).toBeVisible();
    await expect(page.locator('[data-onboarding-id="onboarding.name-bot"]').first()).not.toBeVisible();
