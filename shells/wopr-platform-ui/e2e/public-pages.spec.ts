...
test.describe("Public pages (no auth required)", () => {
  ...
  test("privacy policy page renders with content and key sections", async ({ page }) => {
    ...
    // Main heading
    await expect(page.getByRole("heading", { name: "Privacy Policy" }).first()).toBeVisible({ timeout: 10000 });
  ...
  test("terms of service page renders with content and key sections", async ({ page }) => {
    ...
    // Main heading
    await expect(page.getByRole("heading", { name: "Terms of Service" }).first()).toBeVisible({ timeout: 10000 });
