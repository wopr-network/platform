...
test.describe("Admin: Tenants", () => {
  test("renders tenant list and search filters results", async ({ authedPage: page }) => {
    ...
    // Search by email
    const searchInput = page.getByPlaceholder("Search by email, name, or tenant ID...").first();
...
test.describe("Admin: Promotions", () => {
  test("renders promotions list with existing promotion", async ({ authedPage: page }) => {
    ...
    // Heading
    await expect(page.getByRole("heading", { name: "Promotions" }).first()).toBeVisible();
...
test.describe("Admin: Affiliates", () => {
  test("renders affiliate dashboard panels", async ({ authedPage: page }) => {
    ...
    await expect(page.getByText("No suppression events recorded").first()).toBeVisible();
  });
});
...
test.describe("Admin: Billing Health", () => {
  test("renders billing health dashboard with metrics", async ({ authedPage: page }) => {
    ...
    // Heading
    await expect(page.getByRole("heading", { name: "Billing Health" }).first()).toBeVisible();
...
test.describe("Admin: Rate Overrides", () => {
  test("renders rate overrides list", async ({ authedPage: page }) => {
    ...
    // Heading
    await expect(page.getByRole("heading", { name: "Rate Overrides" }).first()).toBeVisible();

    // "New Override" button
    await expect(page.getByRole("button", { name: "New Override" }).first()).toBeVisible();
...
test.describe("Admin: Route Protection", () => {
  test("non-admin user is redirected away from admin routes", async ({ authedPage: page }) => {
