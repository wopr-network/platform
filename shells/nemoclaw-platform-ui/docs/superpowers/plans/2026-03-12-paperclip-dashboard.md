# NemoClaw Dashboard Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the inherited WOPR instance list with an adaptive hero/grid dashboard that shows NemoClaw instances as cards linked to their `*.runnemo.com` subdomains.

**Architecture:** A single `NemoClawDashboard` client component replaces core's `InstanceListClient`. It uses the same `trpc.fleet.listInstances` query but renders cards instead of a table. One NemoClaw = hero card. Multiple = card grid with a `+` card for adding more. The `+` card has an inline name input — no separate create page.
...
**Spec:** `docs/superpowers/specs/2026-03-12-nemoclaw-dashboard-design.md`

**Design Language: "Warm Terminal"**
- All monospace (JetBrains Mono, already set in root layout)
- Dark-first palette: slate backgrounds, amber-400/500 primary accent (warm brass nemoclaw)
...
|--------|------|----------------|
| Create | `src/components/nemoclaw-card.tsx` | Single NemoClaw instance card — name, pulsing status dot, subdomain link, settings gear. Clickable card surface opens subdomain. Hero and grid variants. |
| Create | `src/components/add-nemoclaw-card.tsx` | The `+` card with rotating icon, inline name input, card and link variants |
| Create | `src/components/nemoclaw-dashboard.tsx` | Adaptive hero/grid layout, data fetching via tRPC, error/loading states |
| Modify | `src/app/instances/page.tsx` | Swap `InstanceListClient` for `NemoClawDashboard` |
...
| Create | `src/__tests__/nemoclaw-card.test.tsx` | Tests for NemoClawCard |
| Create | `src/__tests__/add-nemoclaw-card.test.tsx` | Tests for AddNemoClawCard |
| Create | `src/__tests__/nemoclaw-dashboard.test.tsx` | Tests for NemoClawDashboard (hero/grid/loading/error/empty) |
...
```bash
cd /home/tsavo/nemoclaw-platform-ui
...
Run: `cd /home/tsavo/nemoclaw-platform-ui && npx vitest run`
...
## Task 1: NemoClawCard Component

**Files:**
- Create: `src/components/nemoclaw-card.tsx`
- Create: `src/__tests__/nemoclaw-card.test.tsx`

- [ ] **Step 1: Write the failing tests**

```tsx
// src/__tests__/nemoclaw-card.test.tsx
import { render, screen } from "@testing-library/react";
import { NemoClawCard } from "@/components/nemoclaw-card";

const mockInstance = {
  id: "inst-1",
  name: "my-bot",
  status: "running" as const,
  subdomain: "my-bot.runnemo.com",
};

describe("NemoClawCard", () => {
  it("renders instance name", () => {
    render(<NemoClawCard instance={mockInstance} />);
    expect(screen.getByText("my-bot")).toBeInTheDocument();
  });

  it("renders subdomain as a link that opens in new tab", () => {
    render(<NemoClawCard instance={mockInstance} />);
    const link = screen.getByRole("link", { name: /my-bot\.runnemoclaw\.com/i });
    expect(link).toHaveAttribute("href", "https://my-bot.runnemo.com");
    expect(link).toHaveAttribute("target", "_blank");
  });

  it("makes the entire card surface clickable to the subdomain", () => {
    const { container } = render(<NemoClawCard instance={mockInstance} />);
    // The outer card element should be an anchor to the subdomain
    const cardLink = container.querySelector("a[href='https://my-bot.runnemo.com']");
  ...
  it("renders STOPPED status", () => {
    render(
      <NemoClawCard instance={{ ...mockInstance, status: "stopped" }} />,
    );
  ...
  it("renders ERROR status", () => {
    render(
      <NemoClawCard instance={{ ...mockInstance, status: "error" }} />,
    );
  ...
  it("applies hero variant styling when variant is hero", () => {
    const { container } = render(
      <NemoClawCard instance={mockInstance} variant="hero" />,
    );
  ...
  it("applies grid variant styling when variant is grid", () => {
    const { container } = render(
      <NemoClawCard instance={mockInstance} variant="grid" />,
    );
...
Run: `cd /home/tsavo/nemoclaw-platform-ui && npx vitest run src/__tests__/nemoclaw-card.test.tsx`
Expected: FAIL — module not found

- [ ] **Step 3: Implement NemoClawCard**
...
```tsx
// src/components/nemoclaw-card.tsx
"use client";
...
```bash
git add src/components/nemoclaw-card.tsx src/__tests__/nemoclaw-card.test.tsx src/app/globals.css
git commit -m "feat: add NemoClawCard with warm-terminal design, hero/grid variants"
```

---

## Task 2: AddNemoClawCard Component

**Files:**
- Create: `src/components/add-nemoclaw-card.tsx`
- Create: `src/__tests__/add-nemoclaw-card.test.tsx`

- [ ] **Step 1: Write the failing tests**

```tsx
// src/__tests__/add-nemoclaw-card.test.tsx
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { AddNemoClawCard } from "@/components/add-nemoclaw-card";

describe("AddNemoClawCard", () => {
  it("renders the + card with prompt text", () => {
    render(<AddNemoClawCard onAdd={vi.fn()} />);
    expect(screen.getByText(/add another nemoclaw/i)).toBeInTheDocument();
  });

  it("shows name input when clicked", async () => {
    const user = userEvent.setup();
    render(<AddNemoClawCard onAdd={vi.fn()} />);
    await user.click(screen.getByText(/add another nemoclaw/i));
  ...
  it("calls onAdd with trimmed name on Enter", async () => {
    const user = userEvent.setup();
    const onAdd = vi.fn();
    render(<AddNemoClawCard onAdd={onAdd} />);
    await user.click(screen.getByText(/add another nemoclaw/i));
  ...
  it("shows loading state when adding is true", () => {
    render(<AddNemoClawCard onAdd={vi.fn()} adding={true} />);
    expect(screen.getByText(/creating/i)).toBeInTheDocument();
  });

  it("renders as subtle link variant without card border", () => {
    const { container } = render(
      <AddNemoClawCard onAdd={vi.fn()} variant="link" />,
    );
...
Run: `cd /home/tsavo/nemoclaw-platform-ui && npx vitest run src/__tests__/add-nemoclaw-card.test.tsx`
Expected: FAIL — module not found

- [ ] **Step 3: Implement AddNemoClawCard**
...
```tsx
// src/components/add-nemoclaw-card.tsx
"use client";
...
export function AddNemoClawCard({
  onAdd,
  adding = false,
  variant = "card",
}: {
  ...
  if (!expanded) {
    return variant === "link" ? (
      ...
      >
        ...
        <span className="font-mono text-xs tracking-wide">Add another NemoClaw</span>
      </button>
    );
  }
  ...
  return (
    ...
    >
      <input
        ref={inputRef}
        type="text"
        placeholder="Name your NemoClaw"
        value={name}
...
```bash
git add src/components/add-nemoclaw-card.tsx src/__tests__/add-nemoclaw-card.test.tsx
git commit -m "feat: add AddNemoClawCard with amber-accent design, rotating + icon"
```

---

## Task 3: NemoClawDashboard Component

**Files:**
- Create: `src/components/nemoclaw-dashboard.tsx`
- Create: `src/__tests__/nemoclaw-dashboard.test.tsx`
...
```tsx
// src/__tests__/nemoclaw-dashboard.test.tsx
...
vi.mock("@core/lib/brand-config", () => ({
  getBrandConfig: () => ({ domain: "runnemo.com", productName: "NemoClaw" }),
...
import { NemoClawDashboard } from "@/components/nemoclaw-dashboard";
...
describe("NemoClawDashboard", () => {
  ...
  it("shows loading state while fetching", () => {
    ...
    renderWithQueryClient(<NemoClawDashboard />);
  ...
  it("shows empty state when no instances exist", () => {
    ...
    renderWithQueryClient(<NemoClawDashboard />);
    expect(screen.getByText(/no nemoclaws yet/i)).toBeInTheDocument();
  ...
  it("renders hero card for single instance", () => {
    ...
    renderWithQueryClient(<NemoClawDashboard />);
    expect(screen.getByText("my-bot")).toBeInTheDocument();
    expect(screen.getByText(/my-bot\.runnemoclaw\.com/)).toBeInTheDocument();
...
Run: `cd /home/tsavo/nemoclaw-platform-ui && npx vitest run src/__tests__/nemoclaw-dashboard.test.tsx`
Expected: FAIL — module not found

- [ ] **Step 3: Implement NemoClawDashboard**
...
```tsx
// src/components/nemoclaw-dashboard.tsx
"use client";
...
import { AddNemoClawCard } from "./add-nemoclaw-card";
import { NemoClawCard, type NemoClawInstance } from "./nemoclaw-card";

export function NemoClawDashboard() {
  ...
  const createMutation = trpc.fleet.createInstance.useMutation({
    onSuccess: () => {
      refetch();
      toast.success("NemoClaw created!");
    },
    onError: (err: unknown) => {
  ...
  const instances: NemoClawInstance[] = useMemo(() => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-24 text-muted-foreground/50">
        ...
        <span className="font-mono text-sm tracking-wide">Loading your NemoClaws...</span>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      {isHero && (
        <div className="flex flex-col items-center gap-8 py-10">
          <NemoClawCard instance={instances[0]} variant="hero" />
          <AddNemoClawCard
            onAdd={handleAdd}
      ...
      {!isHero && instances.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((inst) => (
            <NemoClawCard key={inst.id} instance={inst} variant="grid" />
          ))}
          <AddNemoClawCard
            onAdd={handleAdd}
            adding={createMutation.isPending}
      ...
      {instances.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-2 py-24 text-muted-foreground/40">
          <p className="font-mono text-sm">No NemoClaws yet.</p>
...
```bash
git add src/components/nemoclaw-dashboard.tsx src/__tests__/nemoclaw-dashboard.test.tsx
git commit -m "feat: add NemoClawDashboard with adaptive hero/grid layout"
```
...
- [ ] **Step 1: Update instances page to use NemoClawDashboard**
...
Run: `cd /home/tsavo/nemoclaw-platform-ui && npx tsc --noEmit`
Expected: No errors
...
git commit -m "feat: wire NemoClawDashboard into instances pages, redirect /new"
```
